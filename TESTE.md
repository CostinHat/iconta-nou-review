...
- **`core/test_sonda_web.py` — NOU, 7 teste (27.08.2026, R75)**: sonda e în `RITMURI`, trece prin `cron.ruleaza`, **cere pagina** și **compară ora de pornire**; constantele nu pot fi argumente implicite (forma care a făcut calea de eșec neprobabilă). Plus două probe pe viu și o calibrare pe o unitate inexistentă.
- **`core/test_nume_anaf.py` — NOU, 4 teste (27.08.2026)**: coloanele există; captarea e pe TOATE căile, nu sub `if seteaza_nume`; lista de firme poartă cele două câmpuri pe toate rolurile; ecranul păstrează pragul de vechime.
