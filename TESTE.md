...
- **`core/test_tenant_stergere.py` — NOU, 13 teste (27.08.2026, R72 + R50)**: lista celor 13 tabele cu `tenant_id` se confruntă cu `information_schema` la fiecare rulare (clichet în ambele direcții); `gdpr_sterge` trebuie să cheme aceeași funcție (AST); ordinea de ștergere citită ca secvență de operații; urma nu se poate șterge pe sine; previzualizarea numără rândurile. Nu șterge nimic ca să probeze — efectul s-a probat separat, pe firme sintetice.
