...
- ultim: C-4 COMPLET — TRANȘA 3 (anuale + tranziția de an) ÎNCHISĂ 06.08 tura 3. Livrat (bug reachable): FIX D101 crash pe ca_an_precedent_eur (declaratii_api îl injecta, calcul_d101 îl respingea) -> gate IMCA art.18^1 (sub 50 mil euro nu se aplică; peste prag fără P47 eroare clară) — commit a3463a5, gard test_d101_imca_ca_precedent.py (RED+API-path+DUK+mutație). Verificat conform: D392 NU se depune (intenționat C4_date.md:108), decembrie/granița de an corectă (D300/D112/D394/D406 12/2026), D205=dividende (salarii în D112).
- urmator: C-4 ÎNCHISĂ (T1 salarizare + T2 coerență/TVA + T3 anuale/tranziție). Reziduu = DECIZII DE PRODUS (nu blochează; greenlight Costin pt campanie ulterioară): (1) perioadă fiscală parțială / regim dual pe an — T2 micro→profit mid-an (D101 pro-rata) + T1 înființare mid-an (bilanț parțial); cere câmp data_infiintare + împărțire de an + comutare regim + enforcement prag CA (L1). (2) D207 nerezidenți (NR1, art.231) neimplementat = declarație nouă. (3) IMCA pe calea DB (pull VT/Vs/I/A). + reziduu tura 2: marjă în D300, praguri L1/L2/L3/L10. Toate = GARZI 06.08 tura 3 + tura 2.
- pasi:
