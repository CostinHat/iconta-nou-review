...
- ultim: T-5 forfetar agricol (item e) GATA 06.08 tura 2 — divergenta REALA reparata: calcul_d300 deriva TVA din COTA si ignora antetul -> compensatia forfetara 8%% (S3->P1: antet tva=2000, cota linie 0) cadea tacit in alte_a cu avertisment generic care NU numea suma; P1 putea pierde deducerea art.315^1 al.17 -> TVA de plata supraevaluata. FIX: TVA orfan (antet - randuri pe cota) la achizitii primite semnalat CANTITATIV (ca precedentul 9%% deductibil), FARA auto-deducere (nu se forteaza - lipsa flag Registrul agricultorilor, decizie produs) si FARA schimbare XML -> DUK-valid. Gard test_d300_forfait_agricol.py: RED probat pe cod vechi + bidirectional (sub-raportare/supra-corectie) + DUK; mutatie=scoaterea masurarii pica. Commit d1a2e4d; suita 1465 passed, verificator 0.
- urmator [PREDARE tranșa 2 — CLASIFICAT LA SURSA 06.08 tura 2; proba contrarie stabilita per item]: (b) D390 P1 = COVERAGE-SEED, NU bug: codul cade deja c.cui->tert_cui (bug reparat 16.07, d390.py pull), deci o factura primita cu tert_cui prefix UE (ex DE...) -> tip A corect; cere doar seed. Blast-radius pe seed partajat (schimba D300/D394 P1) -> foloseste schema efemera in test. (c) D301 N1 = COVERAGE-SEED: pull citeste tabelul d301_operatiuni (NU facturi; d301.py:239), gol in seed; generator deja DUK-valid. Pragul L10 (10.000 EUR obligatie art.317) NEENFORCED = decizie produs (clasa L3/395k). (d) D300 marja S1/S2 = DECIZIE PRODUS (§2.3 oprire): tva_marja/tva_marja_turism STANDALONE, neimportate in d300 (main.py posteaza doar ciorne in inregistrari); nicio operatiune pe marja nu ajunge in D300 ('neintegrat=observatie'). Integrarea cere model nou (flag/tabel marja) -> greenlight Costin. (e) T-5 forfetar = GATA (commit d1a2e4d). (f) limite L1/L2/L3/L4/L10/L12 = CLASA MIXTA: L4 (plafon TVA la incasare) ENFORCED period-aware (common.py:393) + deja testat; L1/L2/L3/L10 NEENFORCED (regim/perioada/platitor fixate MANUAL in seed; GARZI:1555, C2_firme) = C-5/decizie produs -> prag±1 NU e C-4 (CONFIRMA lectia transei 1); L12 (storno/retur) structural, fara ±1.
- pasi:
  ...
  STARE = OPRIT §2.3 pct.6 (buget context) la graniță curată (06.08 tura 2). Livrate verzi cumulat: seed 11 firme + T-1…T-7; fix trimestrial D300/D394; T-6 taxare inversă; T-5 forfetar orfan (item e, commit d1a2e4d, gard + RED + mutatie + DUK). PREDARE clasificata la sursa: D390=coverage-seed, D301=coverage-seed, marjă=decizie produs, limite=L4 enforced+testat / L1-L3-L10 C-5 / L12 structural (vezi urmator).
