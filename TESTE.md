...
| comportament la apăsare + text lung, desktop și mobil | `frontend_test/vizual/interactiune_scan.py` | apasă butoanele (ne-destructiv), umple casetele cu text lung+diacritice; prinde erori de consolă, layout rupt, revărsare, ținte <24px | `acoperire_vizuala.json` (se comite) |
...
- urmator: **F3 etapa 3** (documente primare: achiziție IC + servicii UE tip 5 → taxare inversă, sursă D301/D390) apoi 4-8. STARE = NEINCEPUT
- pasi Faza 0:
...
- pasi:
  ...
  3. **A12 — pro-rata doar pe achizitii mixte (BLOCAT: decizie de date).** `core/d300.py:569,581-588`
     ...
     PART B (UI) GATA 19.09: select `destinatie_tva` per LINIE pe ecranul de VALIDARE a facturii primite
     din SPV (`facturi_ecran.primitaDetaliu`), default 'taxabil', trimis ca `destinatii[]` in ordinea
     liniilor -> `factura_primita_valideaza` -> `repo_facturi.actualizeaza_destinatii_linii` (UPDATE in
     ordine, ORDER BY id). Regula DS noua = cap.28 (clasificare per-linie pe ecran de validare a
     documentelor importate) + gard verificator CLASIF_SELECT (select `pr-dest` fara aria-label).
     VERIFICAT LA SURSA (calea flat/operatiuni e complet separata de primitaDetaliu; cele 4 operatiuni
     care creeaza facturi sunt regimuri speciale, niciuna achizitie art.300-general -> (a) e suficienta).
     Garduri `core/test_a12b_destinatie_linie.py` (4, end-to-end XML real + mutatie ORDER BY DESC -> RED).
     STARE = A12 INCHIS (PART A + PART B)
...
- pasi:
  ...
  5. Ecran propriu, cu temeiul pe fiecare constatare + firmele NEVERIFICATE numite. Inregistrat in
     harta ecranelor. Poarta vizuala pe ecranul atins (axe/mobil/interactiune), cu
...
- pasi:
  ...
  · [ÎN CURS] 2b — locuri fără temei, pe CLUSTER, cu validarea ta: deducere personală [√ 17.09] → tichete → concedii medicale → apoi celelalte 10 module.
...
| cota profit 16% + IMCA | d101 | test_d101.py | √ 23.09 (bump: IMCA versionata pe an - 1% pana in 2025, 0,5% pt 2026 (OUG 89/2025), reverificat la sursa. cota 16% verificata CF art.17 + COTE impozit_profit, test cu temei; IMCA CF art.18^1 IMPLEMENTAT: formula 1%x(VT-Vs-I-A) negativ->0 + prag 50mil euro + wiring P47/comparatie P48 + PROBA DUK. Gap minor: cota din COTA_STANDARD literal, nu cota() - rutare follow-up) | FISCAL | CF art.17 (cota 16%); CF art.18^1 (IMCA); OUG 8/2026 | test_golden_lant_formule_oficiale test_cota_profit_16pct_din_cota_cu_temei test_imca_formula_1pct_din_vt_vs_i_a test_datoreaza_imca_prag_50mil_euro test_imca_wiring_p47_si_comparatie_p48 test_imca_d101_duk_valid |
...
| amortizare | d101 | test_d101.py | √ 01.09 (bump: baza `plafon_mijloc_fix` s-a schimbat sub bifa — data ei era gresita si s-a corectat la sursa; re-verificat, nu re-datat formal. RE-VERIFICAT LA SURSA 01.09.2026, ceruta de `test_bifele_nu_stau_pe_o_baza_schimbata`: baza `plafon_mijloc_fix` s-a schimbat sub bifa — data ei era 01.01.2026, iar marcajul din forma consolidata a Codului fiscal spune «(la 25-02-2026, Litera b), Alineatul (2), Articolul 28 ... a fost modificata de Punctul 7., Articolul 6 din OUG 8/2026)». Valoarea 5.000 lei NEschimbata; s-a corectat DATA de la care se aplica, iar pragul se citeste acum din registru, nu din literali duplicati (R108). Testul de prag confirma acum valoarea LA O DATA: 2.500 pana la 24.02.2026, 5.000 de la 25.02.2026. Bifa initiala 03.08: tratamentul amortizarii in d101 aliniat CF art.28: amortizare FISCALA P11 dedusa in P16, amortizare CONTABILA P28 adaugata inapoi in P34; prag MF amortizabil 5000 lei art.28 alin.2b/OUG8-2026. Golden cu temei. DATORIE separata MF/D406: degresiva/accelerata necalculate - xfail test_datorie_mf_metode_amortizare, impact pe contabil/SAF-T nu pe d101) | FISCAL | CF art.28 (amortizarea fiscala); OUG 8/2026 (prag 5000) | test_amortizare_ajustare_fiscala_art28 test_mf_prag_amortizabil_5000_art28 |
...
| pro-rata deducere | d300 | test_d300.py | √ 19.09 (bump: A12 - test_pro_rata_ajustare_deductibila_art300 rescris sa foloseasca achizitie MIXTA; CF art.300 alin.(5) - pro-rata DOAR pe achizitiile mixte, nu pe tot deductibilul; reverificat la sursa in campania A12. CF art.300, regim mixt: deducere pro-rata cand nu se tin evidente separate. D300 CORECT: R31_2 = Ajustari conform pro-rata (Rd.33 struct v12) pe baza liniilor MIXTE; pro_rata=80% pe mixt -> R31=-42; pro_rata=100 fara ajustare; taxabil neatins, scutit exclus; proba DUK) | FISCAL | CF art.300 (deducere regim mixt); structura D300 Rd.33 (ajustari pro-rata) | test_pro_rata_ajustare_deductibila_art300 test_pro_rata_100_fara_ajustare test_pro_rata_d300_proba_duk_valid |
...
| rotunjire aritmetica (A91b) | d390 | test_d390.py test_limita_text_anaf.py | √ 03.08 (TEMEI CORECTAT 14.09.2026, proba neatinsa: A91b nu exista in D390Validator.jar - zero potriviri in tot jarul; e regula D112, care a respins rotunjirea bancara ACOLO. Faptul (half-up) ramane. Vezi DECIZII.md (42). VERIFICARE - fara fix. d390._int (d390.py:59) = ROUND_HALF_UP (schimbat de la round() bancar 27.07). ANAF cere half-up nu bancar (DUK regula A91b (D112) - referinta validator de la ALT formular, nu act normativ §3.1). Deja gardat dublu: identitate cross-generator (test_rotunjirea_e_identica_intre_generatoare importa d390._int) + scan anti-round() bancar (test_toate_generatoarele_rotunjesc_aritmetic). Adaugat proba d390 pe valoare: _int(0.5)=1/2.5=3/112.5=113 (mutant bancar 0/2/112)) | FISCAL | DUK regula A91b (D112) (rotunjire aritmetica: regula care a respins rotunjirea bancara PE D112; in D390Validator NU exista - verificat la sursa 14.09.2026, vezi DECIZII.md (42)) | test_d390_rotunjeste_aritmetic_nu_bancar_A91b test_rotunjirea_e_identica_intre_generatoare test_toate_generatoarele_rotunjesc_aritmetic |
...
| impozit dividend | decontari_asociati | test_impozit_dividend.py | √ 23.09 (bump: test_cota_dividend_si_lichidare actualizat - LICHIDARE 10% fix CF art.97 alin.5, NU cota dividendelor, reverificat la sursa. bump: cota 2025 corectata 8%%->10%% (OUG 156/2024 art.LXIV+LXV, de la 01.01.2025; confirmat d205_struct_anaf.txt:6 "8%%/2024,10%%/2025,16%%/2026") - reverificat la sursa, concluzia 03.08 ca "2023-2025=8%%" era gresita pe 2025. FIX cote istorice. Mecanism period-aware corect (common.cota), DAR datele gresite: intrarea pre-2026 era 10%% (petic "else 10") - 10%% NU a fost niciodata cota pe dividende (e impozit pe VENIT art.78) -> 2023-2025 primeau 10%% in loc de 8%%. REPARAT: 3 intrari verificate la sursa - 5%% (2016-2022, Legea 227/2015+OUG 50/2015), 8%% (2023-2024, OG 16/2022), 10%% (2025, OUG 156/2024), 16%% (2026, Legea 141/2025). Teste care cimentau 10%% actualizate la 8%%. Proba 2020->5%%, 2024->8%%, 2026->16%%) | FISCAL | CF art.97 alin.(7); OUG 50/2015 (5%% din 2016); OG 16/2022 (8%% din 2023); Legea 141/2025 (16%% din 2026) | test_impozit_dividend_period_aware_din_cote test_cota_dividend_si_lichidare_sursa_din_cote |
| contributii PFA (praguri CAS/CASS pe sm) | d212 | test_d212_reper.py | √ 23.08 (bump: garda anti-stale a semnalat corect - `test_d212_apare_in_graful_salariu_minim` s-a schimbat pe 23.08, la calificarea cheilor grafului (R17). RE-VERIFICAT LA SURSA in aceeasi tura, nu bumpuit ca sa treaca poarta: `anaf_surse/cod_fiscal_227_2015_consolidat.txt` confirma art.170 alin.(1) = baza CASS cumulata, PLAFONATA la 72 salarii minime, fara trepte; iar marcajul de consolidare de sub el spune literal `(la 01-01-2026, Alineatul (1), Articolul 170 ... a fost modificat de Punctul 19., Articolul XII din LEGEA nr. 239 din 15 decembrie 2025)`, deci exact temeiul pe care clusterul il corectase; nota actului adauga ca pct.19 se aplica veniturilor anului 2026. CAS art.148: pragurile 12 si 24 salarii minime, confirmate in text. Functia care s-a schimbat e cea de GRAF, nu cea fiscala - `test_d212_reper_din_cota_nu_literal` e neatinsa. VERIFICARE - calcul CONFORM - calcul CONFORM. CAS 25%% praguri 12/24 sm (CF art.148), CASS 10%% LINIAR 6 sm..plafon 60/72 sm (CF art.170 alin.1 - treptele doar pt venituri pasive, nu PFA), impozit 10%% pe net-CAS-CASS. sm period-aware. REPARAT citare temei: plafonul 72 sm 2026 era atribuit gresit "Legea 141/2025" -> corect Legea 239/2025 art.XII pct.19 (valoarea 72 sm corecta). Obs: calea 2026 dormanta in productie (poarta an=2025), de ridicat la 2027) | FISCAL | CF art.148-149 (CAS), art.170 alin.(1) (CASS PFA liniar), art.68-69 (venit net); Legea 239/2025 art.XII pct.19 (plafon CASS 72 sm 2026) | test_d212_reper_din_cota_nu_literal test_d212_apare_in_graful_salariu_minim |
...
| Etapă | Stare |
|---|---|
| 1. Migrare / preluare | **F1 √ 20.09** (Σ=17000, stoc, 3 salariați) · **F2 √ 21.09** (Σ=17000, mijloc fix) · **F3 √ 21.09** (Σ=5000, simplu, fără salariați) |
| 2. Configurare firmă | **F1 √ 20.09** (micro/TVA-lunar/fără-IC) · **F2 √ 21.09** (profit/TVA-trimestrial) · **F3 √ 21.09** (micro/NEPLĂTITOR+art.317/cu-IC → D301/D390/D100/D406)** |
...
- **`core/test_raport_z_unic.py` — NOU, 4 teste (26.08.2026, R61)**: unicitatea raportului Z pe `NUI + nr_raport`, în **amândouă** sursele, plus poarta de perioadă pe `import-amef`. Aserțiunea e pe **ordinea** apelurilor în AST (garda înaintea scrierii), nu pe prezența lor — calibrare pe ambele forme: apel absent, și apel după INSERT.
- **`core/test_portal_acces.py` — NOU, 6 teste (26.08.2026, R62)**: izolarea între cabinete pe **amândouă** căile de legare a unui cont, dispariția scrierii directe a adresei, reconfruntarea adresei la confirmare, urma pe toate cele șase acte. Calibrarea e pe forma reparației de instanță: o cale gardată, cealaltă nu.
...
- **`core/test_ruta_fara_apelant.py` — 6 teste (26.08.2026 R70; al șaselea 27.08.2026)**: o rută nouă fără apelant nu trece; `_BASELINE` ca fotografie, clichet pe mulțime în ambele direcții. Calibrare pe dispecerizarea prin tabel — forma care a doborât al doilea detector. **Al șaselea**: cele cinci rute fără ecran își declară lipsa în cod (`[api_intern_v1]` pe linia decoratorului), RED-proof pe o rută care are ecran.
- **`core/test_reluari_decizie.py` — NOU, 5 teste (27.08.2026)**: o restanță nouă deblocată de DECIZIE nu poate trece 5 commituri de registru cu `reluări: 0`. Fotografie cu 19 existente (R9 la 133 de commituri), clichet în ambele direcții. Calibrări: R33, care ARE contor, nu intră în clasă; restanțele proaspete nu sunt raportate.
- **`core/test_sonda_web.py` — NOU, 10 teste (27.08.2026, R75)**: sonda e în `RITMURI`, trece prin `cron.ruleaza`, **cere pagina** și **compară ora de pornire**; constantele nu pot fi argumente implicite (forma care a făcut calea de eșec neprobabilă). Plus două probe pe viu și o calibrare pe o unitate inexistentă.
- **`core/test_nume_anaf.py` — NOU, 6 teste (27.08.2026, T36 + R77)**: coloanele există; captarea e pe TOATE căile, nu sub `if seteaza_nume`; lista de firme poartă cele două câmpuri pe toate rolurile; ecranul păstrează pragul de vechime.
- **`core/test_refuz_tacut.py` — NOU, 9 teste (27.08.2026)**: clichet pe scrierile care pot refuza fără să arate motivul (16, măsurat după calibrare pe instanțe reale), pe citirile tăcute (70, declarate), și pe existența plasei din `api.js`. Calibrări: `catch {}` prins, `insertAdjacentHTML` NU e acuzat, `try/finally` nu intră în clasă, o acoladă dintr-un șir nu mută blocul.
- **`core/test_nume_anaf.py` — +2 teste (27.08.2026, R77)**: alegerea are **amândouă** ramurile și amândouă **scriu** — consemnarea `nume_ales` e una singură și **nu** are voie să stea sub ramura „anaf", altfel „păstrez denumirea mea" redevine tăcere (citit din arborele funcției, nu din text) · o citire ANAF mai nouă decât alegerea **redeschide** întrebarea, deci nu se pune nici la infinit, nici o singură dată.
- **Niciun test nou pentru R78, și se spune de ce**: ce s-a schimbat e **așezarea** — un buton pe fiecare rând din listă. Verificatorul nu prinde așezarea, iar o gardă pe poziția în pixeli ar fi zgomot la prima schimbare de font. S-a probat cu Playwright, în aceeași fereastră în care s-a măsurat defectul (`frontend_test/vizual_scoatere.py`), și **asta rămâne o probă, nu o gardă**: o mutare viitoare a butonului sub pliu n-ar fi prinsă de nimic. Ce **e** gardat, fiindcă se poate: `frontend_test/vizual/axe_firme.py` ține ecranul de firme sub axe-core, cu anti-vacuu — `axe_scan.py` merge doar pe cele 5 ecrane din `nav_ecrane`, iar lista de firme nu e printre ele.
- **`core/test_nume_anaf.py` — +1 test (27.08.2026 seara, R77 partea a doua)**: a **tasta** o denumire diferită de cea de la ANAF e tot o alegere, nu doar a apăsa un buton — amândouă căile trec prin aceeași funcție de consemnare (AST: apel, nu text), iar pe firmele fără `nume_anaf` nu se consemnează nimic. RED-proof pe sursa reală: 4 mutații, 4 roșii.
- **`core/test_nume_firma_unic.py` — +3 teste (27.08.2026 seara, R81)**: cele două denumiri ale unei firme (portofoliu vs fiscală) nu divergează mai mult — clichet pe date la **4 din 17**, în ambele direcții, cu anti-vacuu; plus ecranul le arată distinct și numește care pleacă pe hârtie (structural, pe id-uri și pe funcția care le compară).
- **`core/test_ruta_fara_apelant.py` — +1 test, +1 redenumire (27.08.2026 seara, R80)**: `test_gardul_isi_spune_NUMITORUL` recalculează acoperirea și pică dacă `_OARBE` a rămas în urmă; testul principal se cheamă acum `..._DINTRE_CELE_VIZIBILE`, iar la rulare scrie *„acoperire reală: 360 din 411 (88%)"*. Cerut de Costin: verdele trebuie să poarte numitorul.
- **`core/test_ancore_rute.py` — NOU, 4 teste (27.08.2026 seara, R80)**: clasa de rute despre care detectorul din R70 nu poate afirma nimic nu mai crește tăcut — clichet **51 din 411**, în ambele direcții, cu anti-vacuu pe textul JS și calibrare pe instanța cunoscută (`PUT /tenants/{tenant_id}`, rămasă azi fără apelant fără ca R70 s-o vadă). Instrumentul: `scripts/scan_ancore_rute.py`.
- **`core/test_nume_firma_unic.py` — NOU, 7 teste (27.08.2026)**: numele unic per cabinet, pe creare ȘI pe redenumire; redenumirea verifică și CUI-ul; normalizarea ignoră majuscule și spații dar NU forma juridică; clichet în ambele direcții pe perechea rămasă din trecut.
- **`core/test_tenant_stergere.py` — NOU, 20 teste (27.08.2026, R72 + R50 + R79)**: lista celor 13 tabele cu `tenant_id` se confruntă cu `information_schema` la fiecare rulare (clichet în ambele direcții); `gdpr_sterge` trebuie să cheme aceeași funcție (AST); ordinea de ștergere citită ca secvență de operații; urma nu se poate șterge pe sine; previzualizarea numără rândurile. Nu șterge nimic ca să probeze — efectul s-a probat separat, pe firme sintetice. **Plus trei teste pe orfanul-OM (27.08 seara)**: niciun cont de client activ fără nicio firmă (invariant pe date, azi 0) · ștergerea îl **dezactivează**, nu îl șterge (ordinea operațiilor, pe AST) · previzualizarea îl numește cu email. **Plus două pe orfanul-RÂND (27.08, R79)**: niciun `INSERT` în `audit_log` nu poartă `tenant_id` fără sub-interogarea care îl trece pe `NULL` când firma nu mai există (AST, deci repară clasa) · clichet pe date la **69** de orfani, ca să nu mai crească.
- **`core/test_joburi_supravegheate.py` — NOU, 12 teste (27.08.2026, R74)**: joburile se citesc din SISTEM (`crontab` + timere systemd), nu dintr-o copie scrisă în test; fiecare are prag în `cron.RITMURI`; interpretorul fiecărei unități trebuie să existe pe disc (baseline cu clichet în ambele direcții pentru cele trei stricate); anti-vacuu pe amândouă sursele. A înlocuit setul bătut de mână din `test_heartbeat.py`.
- **`core/test_esec_trimitere_email.py` — NOU, 12 teste (27.08.2026, R73)**: un `except` care prinde un `trimite_email_html` trebuie să cheme `esec_secundar`; cele trei căi de acces alertează, bun-venitul nu. Structural pe AST, fără baseline (măsurat: 4 din 17 apeluri sunt prinse de `try`, toate conforme). Calibrare pe forma subtilă (`log(...)` fără urmă) și pe direcția inversă (`try/finally` nu e raportat). RED-proof pe `main.py` real, mutat în memorie: 4 mutații, 4 roșii.
- **Rândul de deasupra a anulat un rând scris cu o oră înainte** (27.08.2026): *„Niciun test pentru R73, și se spune de ce: reparația e o schimbare de apel, iar un clichet care s-o țină n-a fost construit."* Era adevărat când s-a scris și fals după ce Costin a cerut gardul, în aceeași tură. **Se șterge, nu se lasă lângă** — două rânduri care se contrazic în același registru nu informează pe nimeni; cine citește nu poate ști care e cel de azi.
- **`core/test_compozitie_fluturas.py` — NOU, 38 de teste (30.08.2026, lista 5)**: componentele netului trăiau doar în `fluturas_pdf`, deci se vedeau numai în PDF-ul descărcat. Gardul ține **sursa unică**: fiecare câmp declarat în `CAMPURI_COMPUSE` se **mută** — i se schimbă valoarea — și se cere ca ieșirea să se schimbe, deci se verifică **efectul**, nu că numele apare undeva. Direcția inversă e obligatorie și e acolo: șapte câmpuri martor care NU intră în compoziție trebuie să lase ieșirea neschimbată, altfel proba pozitivă n-ar dovedi nimic. Plus: motivul scris pentru un câmp **neafișat** e verificat, nu crezut (dacă ajunge totuși la om, cade) · numele declarate trebuie să fie chiar chei ale statului (AST) · fluturașul nu mai are voie să-și recompună rândurile. **Regresie pe o divergență reală**: un salariat cu NUMAI tichete culturale nu-și vedea secțiunea, iar totalul disponibil diferea între hârtie și ecran. RED-proof: 4 mutații, 4 roșii.
- **`core/test_balanta_pe_ecran.py` — NOU, 8 teste (30.08.2026, lista 5)**: singura rută spre cifrele balanței întorcea un **fișier**. Gardul ține ruta de date, sursa unică a totalurilor (AST: PDF-ul nu mai adună singur), și — partea pentru care există — **cele trei stări ale închiderii**. Pe o lună fără nicio înregistrare toate perechile sunt `0 = 0`, deci verdictul naiv ar fi „se închide" despre o lună în care **nu s-a verificat nimic**; testul cere `nimic_de_verificat`. Calibrare în amândouă direcțiile, plus toleranța ca toleranță, nu amnistie: sub un ban trece, peste un ban nu. RED-proof: 3 mutații, 3 roșii.
- **`core/test_declaratii_componente.py` — NOU, 30 de teste (30.08.2026, lista 5)**: pasul 1c măsurase **0 din 92** de ieșiri care își arată componentele. Gardul face imposibil ca vreuna din cele nouă să iasă din acoperire în tăcere — ori are hartă, ori are motiv scris. Modul de eșec pentru care e scris: **o hartă care arată spre un atribut inexistent nu crapă, întoarce zero rânduri — iar zero rânduri arată exact ca o declarație goală**. De aceea clasa de rezultat a fiecărui motor se citește **din modul**, nu se scrie în test. Plus: coloanele declarate monetare trebuie să existe printre cele produse (altfel `bani()` nu s-ar aplica niciodată) · plafonul de 500 de rânduri se **declară**, nu taie tăcut · un tip nemapat spune „nu știu", nu întoarce listă goală. **A prins o eroare a mea la prima rulare**: copiasem coloanele monetare de la D100 pentru D710, dar `ObligatieRect` are `suma_dat_i`/`suma_dat_c` și o `@property` pe care `dataclasses.asdict` n-o vede. RED-proof: 6 mutații, 6 roșii.
- **`core/test_poarta_citire_istorica.py` — +1 test (30.08.2026)**: poarta de citire-istorică a trecut de la 13 la **14** apelanți, iar gardul avertiza pe drept că *„al 14-lea nu e o scăpare, e varianta (c) pe furiș"*. Nu era o clasă nouă de acces: ruta de date a balanței e **a doua ieșire** a unui artefact deja înăuntru. Testul nou cere ca orice intrare viitoare să fie **ori din cele 13 de la deschidere, ori declarată pereche a uneia dintre ele** — deci lărgirea deciziei (b) cere o propoziție scrisă, nu un nume adăugat într-un `frozenset`.
- **`core/test_cartea_mare.py` — NOU, 8 teste (30.08.2026, lista 3)**: `core/fisa_cont.py` producea Fișa de cont 14-6-22 — înlocuitorul legal al Cărții mari (14-1-3) — din iulie, **pe date reale** (6 firme, 43 de conturi), și n-avea nici rută, nici ecran. Gardul ține ruta, cele trei părți pe care le consumă ecranul, și — partea pentru care există — **`temei_completitudine` până în răspuns**: *o fișă care omite o notă arată identic cu una completă*. Plus conversia la JSON în amândouă direcțiile (`Decimal`/`date` convertite, restul **neatins** — altfel „a convertit" ar putea însemna „a stricat tot"). RED-proof: 4 mutații, 4 roșii.
- **`core/test_declaratii_componente.py` — +5 teste, −1 rescris (30.08.2026, R105)**: D112 a intrat în hartă în ziua în care motorul ei a căpătat obiect de rezultat. Testul care cerea ca motivul absenței să fie **adevărat** (`core.d112` să NU aibă clasă de rezultat) a **picat, corect**, și s-a inversat: acum cere ca declarația să-și desfacă cifra pe amândouă felurile de poziții. Nou și important: **confruntarea** — suma impozitului asiguraților = obligația `602`, CAS = `412`, CASS = `432` —, cu calibrare inversă (un asigurat inventat trebuie să rupă egalitatea) și cu proba că **CNP-ul nu pleacă în componente**, fiindcă obiectul se persistă în bază. *Proba „absența motivată nu arată ca o declarație goală" s-a mutat pe caz sintetic: se proba pe `d112`, singura intrare reală, și ar fi rămas fără obiect — aceeași lecție ca METODA §29.*
- **Trei clichete mutate, toate cu cauza măsurată (30.08.2026)**: orbirea detectorului **51 → 55**, GRI **45 → 49** (în toate cele **trei** locuri care o citesc — gardă, verificator, R80 —, fiindcă există un gard care le compară), și `core/fisa_cont.py` **scos** din PIN-ul modulelor nelegate. Prima creștere **nu e o regresie a aplicației**: cele patru rute `/jurnal` care au orbit **au apelanți**; ce a crescut e incapacitatea detectorului de a o dovedi, fiindcă ancora lui — cuvântul `jurnal` — a trecut de prag odată cu ecranul nou. *Măsurat prin comparație directă cu HEAD, nu dedus.*
- **`core/test_categorie_marime.py` — NOU, 13 teste (30.08.2026, R3)**: categoria de mărime nu exista în aplicație — nici câmp, nici derivare, nici măcar numele —, iar cele două rute de bilanț produceau situații financiare **fără să poată ști dacă e cea datorată**. Gardul ține cele două lucruri pe care o implementare naivă le-ar fi ratat, amândouă în normă: **încadrarea cere DOUĂ exerciții consecutive** (pct.13 alin.(2)-(3)) și **pragul se trece pe DOUĂ criterii din trei** (pct.9). Modul de eșec pentru care există: *fără exercițiul precedent, categoria e `nedeterminata`, NU „micro"* — micro e cea mai mică, deci exact valoarea spre care alunecă o implementare care tratează necunoscutul ca zero. Plus: fiecare prag își poartă `Temei`-ul **lipit de valoare** (forma din `common.COTE`; prima formă îl ținea ca cheie-soră și `scan_constante` a numărat șase constante nesursate — pe drept), iar cifrele se confruntă cu textul citat în chiar temeiul lor. RED-proof: 6 mutații, 6 roșii.
- **`core/test_ancore_rute.py` + `core/test_ruta_fara_apelant.py` — reparate, nu replafonate (30.08.2026, R80)**: după **a treia** creștere din aceeași zi produsă de vocabular, nu de comportament, ancora a devenit **segment de cale**. Orbirea **55 → 6**, GRI **55 → 7**, ACCEPTAT **327 → 375**, **ROSU rămâne 0**. Cele 6 rămase sunt exact rutele al căror singur segment literal e `tenants` — adică **instanța fondatoare a restanței rămâne oarbă**: reparația n-a acoperit-o, a curățat în jurul ei. Calibrarea care proba ordinea `EXCLUS`-înaintea-lui-`GRI` a rămas fără obiect după reparație și **a picat pe un succes** — mutată pe caz sintetic, a patra instanță a METODA §29 în aceeași zi.
- **`core/test_clichete_generate.py` — NOU, 12 teste (31.08.2026)**: tabelul clichetelor vii din `PREDARE_LANT.md` era scris cu mâna sub propoziția *«se recalculează, nu se citesc de aici»*. Recalculate: **trei din patru corecte — exact cele trei plafonate**; a patra, umbra interdicției 77, singura declarată nedeplafonată, circula în aceeași zi în trei valori prin trei documente. Tabelul e acum **bloc generat** (`scripts/scan_ramas.py --clichete-md`), iar gardul îl compară pe **structură** (cod cu cod, METODA §23) și peste asta caracter cu caracter. Direcția pe care egalitatea de cifre n-o vede are test propriu: un clichet **măsurat și nescris**. Calibrare în ambele direcții pe forma reală — cifră mutată · rând șters · marcaj șters · blocul probat că nu poartă oră (altfel ar pica la fiecare rulare și cineva l-ar scoate). Al zecelea test interzice cifra umbrei în **proza** predării, și **m-a prins pe mine** la prima rulare verde: paragraful care povestea derapajul purta chiar cifrele interzise. RED-proof: 5 roșii din 10 pe documentul nereparat, 12 verzi după. Singura scutire — tabelul cifrelor invalidate, unde valorile vechi TREBUIE să stea — e tăiată structural, cu anti-vacuu propriu și cu proba că o cifră din proza obișnuită e tot prinsă.
- **`core/test_verificator_al_treilea_rezultat.py` — NOU, 18 teste (31.08.2026)**: `verificator_neconformitati.sh` tipărea *„FAIL NC-07 JWT_SECRET lipseste din proces!"* când `sudo` nu e disponibil neinteractiv — **secretul era acolo**, lipsea dreptul de a te uita. Reparația e un **al treilea rezultat** (`NEVERIF`), nu drepturi în plus. Generalizarea, **măsurată pe scriptul real înainte de a repara**, a scos încă trei forme latente ale aceleiași clase plus un `EXIT=0` peste `FAIL: 3`; iar ipoteza că `psqlv` minte a fost **infirmată** (`sudo -n -u postgres psql` merge). A patra formă greșea în **direcția opusă**: glob gol → `wc -l` = 0 → `PASS` pe mulțime goală. Fiecare `NEVERIF` poartă un **cod stabil**, iar gardul asertează pe codul parsat, nu pe proza mesajului (METODA §23). **RED-proof: 8 mutații, 8 roșii — dar prima rulare a dat 5 din 7**, fiindcă două apărări erau acoperite de ramuri vecine care ajungeau la același verdict din alt motiv; de acolo au ieșit codurile și un test propriu pentru ramura portantă. *Proba prin mutație a găsit un gol în GARD, nu în cod (interdicția 76) — și un bug al meu: `grep -rq -- "$m" "$RAD" --include=...` intra în `.git`, fiindcă `--` oprește parsarea opțiunilor.*
- **`core/test_verificator_al_treilea_rezultat.py` — +2 teste (31.08.2026)**: ancora pe marker a lui NC-02 a fost **retrasă** prin decizie, fiindcă murise la o rescriere legitimă (`55a57f6`). Retragerea e justificată de un singur fapt — verificarea **comportamentală** de deasupra ei acoperă ce voia să apere —, deci acel fapt e acum gardat: NC-02 trebuie să producă **exact un** rând. Zero = retragerea a rămas fără temei și nimeni nu verifică NC-02; două = ancora pe marker s-a întors pe furiș, sub formă de „îmbunătățire". *Un temei de retragere care nu e gardat se erodează tăcut.*
- **`core/test_pereche_act_articol.py` — NOU, 12 teste (31.08.2026, faza 2, R106)**: prima operațiune a fazei 2. Întrebarea, care n-avea instrument: *documentul pe care îl citează un temei conține articolul pe care îl numește?* Măsurat pe registrul de cote — **34 de temeiuri, 26 verificabile, 15 confirmate, 6 NEGĂSIT, 5 CIOT**. Cele șase au aceeași formă, confirmată la sursă: articolul e al **Codului fiscal**, actul citat e cel care l-a **modificat** (`COTE.impozit_dividend` are patru temeiuri și niciunul nu se confruntă). Clichet pe număr **și pe identitate** — cel pe număr n-ar vedea una reparată și alta stricată în aceeași tură —, plus prag de JOS pe confirmate. Calibrare în ambele direcții, cu **cele trei feluri de «nu pot spune» deosebite** (`NEGASIT` / `CIOT` / `FISIER_LIPSA`). **RED-proof: 3 mutații, 3 roșii — dar prima rulare a dat 2 din 3**: proba anti-supra-captare căuta titluri într-un fragment normalizat pe spații, iar tiparul de titlu e ancorat pe linie. Rescrisă pe **efect** (articolul următor poartă un marcaj din 2030; dacă apare, s-a împrumutat). *Un gard care se uită la forma greșită nu e un gard.*
- **`core/articol_in_act.py` — NOU, logică extrasă din `scripts/vigoare_articol.py` (31.08.2026)**: era neimportabilă, sub `sys.argv`, deci am re-scris-o de două ori într-o oră și amândouă copiile au dat cifre greșite. Confruntate înainte de mutare: **zero dezacorduri pe 26 de perechi**. Extragerea a scos **a cincea reparație**, un defect viu: o tăiere laxă potrivea forma de CITARE dinăuntrul unui marcaj de consolidare și trunchia articolul acolo — `CF art. 78` de la **16.719 la 532 de caractere** și de la **șase ani de modificare la unul**. Direcția greșelii era cea liniștitoare: articolul părea mai stabil decât e. Test pe AST: scriptul trebuie să IMPORTE modulul, ca să nu existe iar două implementări.
- **`core/test_pereche_act_articol.py` — +2 teste, clichete rescrise (31.08.2026, R106 RETRASĂ)**: cele șase perechi „negăsite" nu erau un defect de date, ci **convenția de modelare declarată la interdicția 50** — `Legea 141/2025 art. 97` înseamnă *CF art. 97, așa cum l-a modificat Legea 141/2025*. Registrul scrisese și modul de eșec: *„un instrument care le-ar lua literal … n-ar găsi nimic"*. Instrumentul îl reproducea. Reparat **instrumentul**, nu datele: `document_tinta` rezolvă convenția înainte de căutare. **GĂSIT 15 → 24 · NEGĂSIT 6 → 0 · CIOT 5 → 2, cu zero temeiuri schimbate.** Test de regresie pe convenție (mutație: dacă `document_tinta` o uită, **4 teste cad**) plus direcția inversă — `OUG 89/2025 art. III` și `art. LXVI` sunt articole **proprii** și nu au voie să ajungă la Codul fiscal, altfel convenția ar înghiți tot și ar părea că merge. `ART_DE_COD_FISCAL` are acum **un singur loc**, importat de `test_vigoare_articole_registru`. *Reparația pe date fusese scrisă și probată; au oprit-o două gărzi vechi — `test_forma_consolidata_nu_e_sursa_pentru_valoare_cu_succesor` și `test_fiecare_articol_din_registru_e_masurat_la_sursa`.*
- **`core/test_reverificare.py` — NOU, 11 teste (31.08.2026, interdicția 55)**: categoria de reverificare, calculată din două axe mecanice, cu tabelul 3×3 al lui Costin. Miezul nu e că fiecare temei are o clasă, ci că **niciun NECUNOSCUT nu primește prag** — plus direcția inversă, ca `NECUNOSCUT` să nu devină scuză: o pereche complet clasificată trebuie să aibă prag. Tabelul se cere **complet** și **monoton pe amândouă axele** (o rescriere care păstrează cifrele dar le încurcă ordinea trece un test de egalitate și pică aici). `INFORMATIV` e cerut **vid**, fiindcă nu se poate atribui mecanic — testul cade în ziua în care apare o cale, și e corect să cadă. Fereastra de 3 ani e probată că **taie** (altfel orice articol vechi ar părea volatil pe vecie), iar gaura tabelului — trei modificări în același an — e astupată **declarat**, spre VOLATIL. Distribuția e pinată: o mutare tăcută a clasificării e o mutare a pragurilor de reverificare. Ultimul test cere ca **nicio valoare să nu devină verificată mai rar** decât azi (`mai_larg == 0`), cu anti-vacuu pe direcția opusă (`mai_strict > 0`, altfel tabelul n-ar schimba nimic).
- **`core/test_constante_nesursate.py` — +4 teste (31.08.2026, a cincea direcție oarbă)**: `in_domeniu` întreba „poartă o valoare din registru?" **doar despre valorile implicite ale parametrilor**; o constantă de MODUL cu aceeași valoare îi scăpa — așa a scăpat `PLAFON_MF_2026 = 5000.0`, chiar valoarea curentă din registru, într-un fișier cu **zero** intrări în inventar. Măsurat: **248 din 412** de fișiere în afara domeniului. Regula cere acum **nume fiscal ȘI valoare de registru** — calibrat pe populația reală: cu valoarea singură intra `nucleu.py` (`_SCRYPT_N = 16` se potrivește cu cota de profit), adică șase constante de criptografie ca datorie fiscală permanentă. **Lărgirea adaugă ZERO clasa C** (C rămâne 133): cele patru valori nou-văzute sunt clasa **E**, sursate în proză. Calibrare în ambele direcții, sintetic și pe fișiere reale, plus un test care scrie **ce nu prinde**: `intrastat.PRAG_2026`, valoare fiscală care nu e în registru — dacă intră vreodată, testul cade și cere actualizarea.
- **`core/test_prag_mijloc_fix_unic.py` — NOU, 6 teste (01.09.2026, R108)**: pragul de mijloc fix stătea în **trei** locuri, cu trei adevăruri — canonicul cu data greșită și necitit, `obiecte_inventar` cu data corectă, importul **fără dată**. Unificat pe registru: `prag_mf(la_data)` e singura poartă. Gardul asertează pe **AST** (`5000`, `5000.0`, `Decimal("5000")` sunt aceeași valoare — un test pe șiruri ar vedea trei lucruri diferite), cere ca valoarea canonică să **aibă consumatori** în graf (starea în care s-a putut strica în tăcere), și probează că încadrarea unui bun de 3.000 lei se schimbă chiar la 25.02.2026. Necunoscutul a primit nume în același commit: `prag_mf_cunoscut()` deosebește „verificat la sursă" de „moștenit" pentru date anterioare lui 2015 — fără el, unificarea ar fi mutat minciuna dintr-un loc în altul. **Proba mecanică a reușitei n-am scris-o eu**: `dependenti_act` întorcea zero consumatori, acum întoarce patru, iar `reverificare` a mutat singur valoarea din `MISCATOR/NECUNOSCUT` în `MISCATOR/CALCULAT`.
- **`core/test_constante_nesursate.py` — testul scris ca să cadă a căzut (01.09.2026)**: forma de pe 31.08 cerea ca `intrastat.py` să fie în AFARA domeniului și spunea în mesaj *„dacă pragul a fost adăugat în registru, scoate testul ăsta"*. Pragul a intrat (Costin), fișierul a intrat în domeniu, testul a picat — **exact cum fusese scris să facă**. Rescris ca să păzească **limita**, nu instanța: o valoare fiscală scrisă direct în cod rămâne invizibilă pentru orice regulă ancorată pe registru, iar azi **nu există instanță cunoscută** — afirmat, nu presupus. Clichetul a crescut **133 → 134**, declarat, cu ce anume (`PRAG_ATENTIE = 0.80`, pragul nostru de avertizare, nu o valoare din lege) și cu drumul de coborâre (o clasă pentru operaționalele din `core/`, care nu există azi). A doua constantă nou-văzută n-a intrat în clichet: lista de precizie a clasei D era **enumerată, nu derivată**, și a fost completată cu `0.1` după ce s-a măsurat că e o singură instanță în tot repo-ul.
- **`core/test_prag_per_articol.py` — NOU, 6 teste (01.09.2026, R109)**: pragul calculat de interdicția 55 alimentează acum raportul lunar. Miezul e **incluziunea pe 13 date**, nu pe una — o slăbire care apare abia peste patru luni e tot o slăbire, iar o probă într-o singură zi n-ar vedea-o. Volumul, măsurat înainte și după: **0 → 0 azi, 0 → 3 de la o lună** (dividende, micro, impozit pe venit — clasa VOLATIL/DEPUS), **nicio alertă pierdută la niciun punct**. Cele două capcane pe care le-a scos măsurătoarea au garduri proprii: `NECUNOSCUT` **nu** scoate o cotă din pază (ar însemna reconfirmare niciodată — încălcarea regulii pe ușa din dos), iar un prag **mai larg** decât podeaua se ignoră cu motivul scris, probat pe caz sintetic fiindcă azi tabelul nu produce așa ceva. Plus compatibilitate (fără al treilea argument, comportament identic) și anti-vacuu în două direcții. *Un test vechi a picat corect: cerea pragul în subiectul alertei, dar nu mai există UN prag — proba s-a mutat pe rând, unde chiar diferă.*
- **`core/test_citate_verbatim.py` · `core/test_identitate_acte.py` · `core/test_vigoare_articole_registru.py` — trei clichete atinse (01.09.2026, R110)**: aducerea actului de la emitent a mutat **verbatim 10 → 11** (câștig, clichet ridicat ca să nu se piardă tăcut). Celelalte două gărzi **au cerut să termin ce am început, și au avut dreptate**: identitatea actelor a cerut 121 → 123 — fișierele poartă așezarea Monitorului Oficial (titlu fără număr în antet, număr la picior), pe care tiparul ei n-o citește; am urcat clichetul **numind cele două fișiere**, și **n-am scris** un tipar pentru forma MO fiindcă în tot corpusul există un singur exemplar cu acel antet, iar un tipar pe eșantion de unu nu dovedește acoperire. Vigoarea articolelor a cerut ca art. 1 al ordinului să fie **măsurat la sursă**: l-am citit întreg (2.899 de caractere, șase articole, niciun marcaj) și am scris `None` — dar **dezacordul dintre mine și instrument e scris lângă intrare**, nu netezit: `vigoare_articol.py` refuză documentul cu `0 titluri de articol`, fiindcă cere cuvântul „Articolul" iar MO scrie „Art. 1. —". *Refuzul e în direcția sigură; motivul pe care îl dă e fals* — R111.
- **`core/test_pereche_act_articol.py` — `CLICHET_CIOT` 2 → 3 (01.09.2026, R111)**: **poarta a respins prima încercare de commit**, corect. A treia pereche neconfruntabilă e însă de **alt fel** decât primele două: documentul citat **nu e un ciot**, e actul întreg adus de la emitent — îl declară ciot `_TITLURI_NUMARATE`, care cere cuvântul „Articolul" când Monitorul Oficial scrie „Art. 1. —". Deosebirea e scrisă **lângă clichet**: *R107 = corpus adus parțial · R111 = formă pe care instrumentul n-o citește*, altfel numărul crescut ar spune „corpusul s-a înrăutățit", ceea ce e fals. **N-am reparat tiparul aici**, și motivul e scris: `titluri()` decide starea CIOT pentru tot corpusul, care hrănește `reverificare`, care hrănește raportul lunar — schimbare de contract, nu o linie, deci tura lui R111 cu volumul măsurat înainte și după. Plus direcția periculoasă a reparației înseși: numărând „Art." prea larg, o trimitere în proză devine titlu și un document trece din CIOT în GĂSIT — *din refuz în răspuns fals*.
- **`core/test_frecventa_document_care_raspunde.py` — NOU, 14 teste (01.09.2026, R111)**: frecvența nu se mai citește dintr-un document care nu poate răspunde. Miezul nu e că un anume fișier dă `NECUNOSCUT`, ci că verdictul se cere pe **proprietatea măsurată** a sursei — zero marcaje în tot corpul ei —, nu pe un nume de fișier. **Direcția inversă e proba care contează**: `OUG 89/2025 art. III` trebuie să rămână `STABIL`, fiindcă documentul lui *are* 18 marcaje; fără ea, un criteriu care ar fi întors mereu „nu poate răspunde" ar fi trecut primul test pe o clasificare golită. Convenția e probată că are **o singură implementare** (se compară `_cheie` cu `S.cheie_articol` pe fiecare temei) și că **niciun articol nu se rezolvă condiționat** de actul care îl citează — chiar greșeala care a produs totul. Capcana din `marcaje()` are caz sintetic în ambele direcții: un marcaj tăiat de linii noi trebuie găsit, iar normalizarea n-are voie să facă tiparul lacom. Volumul e pinat cu **numele** celor șapte, nu cu numărul. **Mutație, patru direcții, toate omoară teste**: scoaterea verificării documentului (2 căzute) · un criteriu care înghite tot (3) · `291` scos din mulțime (4) · `marcaje` fără normalizare (4).
- **`core/test_reverificare.py` · `core/test_prag_per_articol.py` — două pinuri mutate, cu socoteala (01.09.2026, R111)**: distribuția `VOLATIL/DEPUS` 9 → 13 · `STABIL/DEPUS` 12 → 7 · `NECUNOSCUT` 11 → 12, iar volumul la o lună 3 → 7. **Amândouă au căzut corect** — asta fac pinurile. Socoteala e scrisă lângă ele, nu doar cifrele noi: +4 cotele de TVA mutate din STABIL, −1 `Legea 201/2025 art. I` mutat în NECUNOSCUT. *Nicio valoare fiscală nu s-a schimbat; s-a schimbat ce poate afirma instrumentul despre ele.* Tabelul măsurat la legarea lui R109 rămâne scris ca **măsurătoare de atunci**, cu nota că s-a mutat în aceeași zi — o cifră istorică nu se rescrie, se datează.
- **`core/scan_data_curenta.py` + `core/test_data_curenta.py` — NOI, instrument + 8 teste (01.09.2026, interdicția 3)**: prima măsurătoare a clasei „un calcul fiscal care citește data curentă", care era NEÎNCEPUTĂ. Instrumentul separă **două populații** — 26 de funcții care *cad* pe `today()` (generatoare latente) și 4 apeluri care *omit* data (clasa activă) — plus contra-cifra care dă scara: 88 de apeluri de producție dau data. **Calibrarea e scrisă pe modul MEU de eșec, nu pe al codului**: am măsurat greșit de trei ori într-o singură alegere, toate numărând forma în loc de efect (numai apeluri pe nume → „1"; numai argumente cu cuvânt-cheie → „68"; tipar de nume prea larg → o funcție de email clasată fiscală). Fiecare greșeală are probă. **Iar prima formă a probei pentru atribut a fost respinsă de propria mutație**: număra toate apelurile de `cota` și trecea și cu sonda oarbă, fiindcă `cota(...)` pe nume există oricum — acum cere ca `d100.py`, care cheamă *exclusiv* `_common.cota(...)`, să apară în inventar. *O calibrare care nu izolează proprietatea nu o testează.* Excepțiile sunt **numite cu motivul** și au test anti-orfan în ambele direcții. **Mutație, trei direcții, toate omoară teste.**
- **`core/test_supervizor.py` — NOU, 13 teste (01.09.2026, supervizorul)**: cele două tării și confirmarea care rămâne scrisă. Miezul nu e că tipul de azi are o tărie, ci că **nu are**, și că asta nu produce niciun efect — nici tăcere, nici confirmări cerute. Un tip **necunoscut** ridică; unul cunoscut dar neatribuit dă `None`, iar `cere_confirmare()` întoarce `False`. **Mecanismul e probat pe un tip SINTETIC cu tărie atribuită** — altfel tot fișierul ar fi verde pe o mulțime în care nimic nu e încă atribuit, iar în ziua atribuirii nimic n-ar fi fost verificat (METODA §29). Amprenta are probe în **ambele** direcții: stabilă la rescrierea prozei, mișcată de **orice** cifră. Eticheta de tip se cere pe **toate cele patru** căi de retur ale comparației — o cale ratată ar face supervizorul să sară constatarea **tăcut**. Plus: confirmarea refuză un motiv gol **înainte** să atingă baza de date, iar modulul e probat că nu conține nicio cale de blocare. **Mutație, trei direcții, toate omoară teste**: o tărie implicită (2 căzute) · amprentă pe proză (3) · ștampila scoasă (1). *Un caz de probă a fost corectat de propriul anti-vacuu: «ambele zero» e tăcut prin construcție, deci nu exercita calea verde deloc.*
- **șase gărzi au respins prima încercare de commit al supervizorului (01.09.2026)** — și fiecare cerea un lucru pe care un modul nou îl datorează, nu un lucru stricat: *afirmații tipate* (o înregistrare citită dintr-un jurnal poartă cheia `motiv`; intrată în BASELINE **cu motivul**, fiindcă `afirmatii.FELURI` e un nomenclator despre ce afirmă aplicația *despre datele firmei*, iar actul unui om n-are fel acolo) · *registrul de excepții* (aceeași cifră, altă unitate — se repară singură odată cu prima) · *module nelegate* (supervizorul și migrarea lui, intrate în **PIN cu motivul scris**: cele două lucruri care l-ar cabla sunt explicit ale lui Costin și nedecise) · *ștergerea unei firme* (`supervizor_confirmari` poartă `tenant_id` și trebuia clasificată — **se șterge** cu firma: sunt decizii despre datele ei) · *inventarul de gărzi* și *tabelele traseelor*, amândouă blocuri generate, regenerate. **Niciuna nu era un fals pozitiv.** *Un modul nou nu e „gata" când trece testele lui — e gata când trece gărzile care nu știau că vine.*
- **`core/test_fara_probe_imagine.py` — NOU, 7 teste (03.09.2026, cerut de Costin)**: *„o gardă care refuză introducerea de fișiere imagine ca probă vizuală. Fără ea, §27 rescris rămâne o intenție și capturile revin la prima tură de interfață."* Granița nu e *„ce e o probă"*, care e o judecată, ci **„intră în index?"**, care e mecanic: capturile făcute în timpul turei rămân permise, salvate în repo — nu. Mulțimea imaginilor din index e **pinată** (30 de probe + 5 de produs + 2 date încărcate), cu clichet în **amândouă** direcțiile: una nouă pică, dar și una **dispărută** pică, cerând să fie scoasă din listă. A doua interdicție e pe **mecanism**, nu pe fișiere: niciun cod nu compară două imagini — `PIL`/`pixelmatch`/`imagehash`/`cv2`/`skimage` citite din **AST** (un `# import PIL` din comentariu nu declanșează), `toHaveScreenshot`/`toMatchImageSnapshot` citite ca text în JS, **cu motivul scris lângă gardă** fiindcă n-avem parser de JS (METODA §23). Măsurat azi: **0** ocurențe. **RED-proof pe mecanismul real, nu pe listă fabricată**: captură creată → `git add -f` → gardă roșie cu mesajul ei → scoasă. *Calibrarea pe listă fabricată e și ea acolo, în ambele direcții, dar singură n-ar fi dovedit că garda vede indexul adevărat.*
- **`core/test_metoda_vie.py` · `core/test_perimetru.py` — două gărzi doborâte de curățarea arborelui, amândouă având dreptate (03.09.2026)**: prima cerea ca fiecare cale citată în METODA să existe, iar §27 rescris **numește**, pe drept, unealta scoasă — reparat cu blocul declarat `CAI-SCOASE`, pentru care gardul cere **exact opusul**: căile din el trebuie să lipsească, deci o reapariție a lui `baseline_scan.py` îl face roșu. *Structural, pe delimitatori — nu se caută cuvântul „scos" prin proză, ceea ce §23 interzice.* A doua avea un anti-vacuu (`assert nt`) care **se sprijinea pe sediment**: cele 299 de fișiere neurmărite pe care arborele le purta permanent; în ziua în care s-a curățat, a picat. *Un anti-vacuu care depinde de dezordinea din jur măsoară dezordinea, nu instrumentul.* Acum testul **își produce singur** condiția — creează un fișier neurmărit, probează ambele direcții, îl șterge.
- **`frontend_test/vizual/acoperire_hash.py` — gardul acoperirii vizuale cerea 16 ecrane din 18 (05.09.2026, R160)**: `ecrane_asteptate()` citește `nav_ecrane.ECRANE` cu regex, ca gardul să nu importe un modul care are nevoie de browser. Regexul era `ECRANE\s*=\s*\[(.*?)\]` — **negreedy**, deci se oprea la **prima** paranteză dreaptă din text. Iar prima nu e capătul listei: e cea din comentariul `# [LOTUL 12, R142]`, scris deasupra intrării «emitere». Consecința, măsurată: `emitere` și `operatiuni` puteau lipsi din artefactul vizual fără ca `test_toate_ecranele_scanate` să cadă — **din chiar tura care le adăugase**, fiindcă acea tură adăugase și comentariul care taie lista. *Nu e o gardă slabă: e o gardă care se uită exact unde nu e problema, a treia instanță a clasei în registru.* Reparat: blocul se ia până la un `]` la **început de rând**, iar comentariile se scot **înainte** de căutarea numelor — altfel un nume citat într-un comentariu ar intra în lista pe care gardul o cere. Plus **anti-vacuu**, care lipsea cu totul: dacă blocul nu se găsește sau iese gol, funcția **ridică** — un `[]` întors tăcut ar face gardul să nu ceară nimic. **Calibrare**: 16 → 21 (16 + `emitere` + `operatiuni` + cele trei ecrane mutate azi în `ECRANE`, al căror JS l-a atins lotul 15). Artefactul vizual s-a refăcut pe toate 21, **zero violări**. *Gărzi NOI: niciuna — campania cere „fără gărzi noi", iar asta e o reparație de gard, nu una nouă.*

- **`core/test_d406_fereastra.py` (05.09.2026)** — 15 teste pe fereastra și antetul D406, gardând
  lanțul R165 / R165c / R166 / R166b: fereastra urmează perioada fiscală TVA pe toate cele cinci
  feluri (L, T, S→T, A→T, neplătitor→T) și calibrat **pe efect**, nu pe nume (T chiar acoperă de
  trei ori cât L) · tipul depunerii **derivat** din întinderea perioadei, cu oprire pe o întindere
  fără cod în nomenclator, în loc de default tăcut · antetul citit ca **arbore** (`ElementTree`),
  nu prin `"..." in xml`, conform METODA §23 · contractul rutei de validare verificat pe **AST**:
  `an`/`luna` trebuie să treacă prin rezultatul generatorului, fiindcă atâta timp cât veneau din
...
- **Cheia de API și suspendarea, probate pe EFECT (14.09.2026)** —
  ...
  `/api/v1/firme`, cu martor pe o cheie inventată · se arată o singură dată · în DB stă sha256 ·
...
- **Coduri de validator, verificate la sursă (14.09.2026)** — `core/test_coduri_validator.py`,
  **4 probe**, peste `scripts/scan_coduri_validator.py`: fiecare `DUK regula <cod>` se confruntă cu
  ...
  1 în `core/d301_operatiuni_api.py` — datorie deschisă, nu reparată) · calibrare în cinci direcții
  pe univers fabricat, inclusiv marca de formular (`DUK regula A91b (D112)`) și proza fără cifră ·
  anti-vacuum pe cele trei corecții din ziua asta. *Instrumentul spune singur unde e orb: pe coduri
  scurte prezența într-un jar de câțiva MB e coincidență, deci greșește spre indulgență.*
