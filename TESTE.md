...
- ultim: C-5 CATALOG DE GOLURI cap.6 (date_test/C5_catalog.md) — livrate P1 (blocari: clase 5-6 izolare = gard structural test_izolare_structurala.py 243 rute×metoda + marcaje mutat; clase 1-4 catalog rol/perioada/date-lipsa/patru-ochi), P6 (cens complet .oblig 34 ecrane, reluare sweep 24.07: 10 goluri dir.1 + 5 dir.2 + bug-uri tacere B7-B16), P2 (date lipsa extins: stopuri fiscale majoritar exemplare; bug B17 d406 UM tacut), P3 (~140 raise-uri core/ fiscale: ~55% explicit reconciliere+cota-TVA model, ~25% telegrafic input-guards; ZERO tacere fiscala). Commituri 412d5df→be2602e, backup/lant-2026-08-06.
- urmator: C-5 continua in ordinea aprobata — P4 (rute main.py HTTPException auth/rol/izolare/conflict 401/403/404/409/423) → P5 (rute validare/date 400/422 + passthrough str(e)) → P7 (validari UI form + consolidarea 'ce nu e static-enumerabil'). Sizing cunoscut: 490 HTTPException (~90% telegrafic, 136× 'tenant inexistent sau fara acces', ~144 str(e)/business passthrough, ~18 cu indrumare). Reziduu decizii Costin (fix = campanii separate): rescriere mesaje telegrafice; fix bug-uri tacere B1-B17 + info-leak operatiuni:381; markeri literali ' *' → .oblig; d406 B17 UM. C-4 INCHISA (vezi istoric mai jos).
- pasi:
