...
| **sub-aproximare pe UN nume, când obiectul are mai multe** | graful de consumatori potrivea numele din calea inventarului. Dar `registru_evidenta_fiscala` ține același `Temei` și în `TEMEI` (dicționar) și în `TEMEI_PROFIT` (constanta pe care o citește codul), iar `scan_citate` raportează primul — `dir()` e alfabetic. Verdict: `NECUNOSCUT` pe o valoare cu doi cititori. *O sub-aproximare arată exact ca o absență* — și, spre deosebire de una zgomotoasă, nu se aude. **Regula:** când cheia e un NUME, se întreabă întâi câte nume are obiectul (R171) |
| **gard care compară cu o podea ce nu acoperea populația** | `test_nicio_valoare_nu_devine_verificata_MAI_RAR` a raportat „10 valori ar fi verificate MAI RAR". Cele 10 nu fuseseră niciodată sub podeaua globală: canalul citește registrul de cote, iar ele trăiesc în modulul regulii. „Mai larg decât podeaua" însemna, pentru ele, *primesc un prag pentru prima oară*. Premisa unui gard e o afirmație despre **populație**; când populația se lărgește, premisa se reverifică, nu clichetul (R171) |
| **mutația care nu se curăță lasă instrumentul mutat** | proba de mutație pe fișier (`cp` de rezervă → `sed -i` → `cp` înapoi) a lăsat un **`__pycache__` compilat din varianta MUTATĂ**. Sursa avea filtrul, codul rulat nu — iar `inspect.getsource` citește FIȘIERUL, deci confirma sursa și întărea iluzia. Am măsurat de patru ori „Fișa de cont arată o ciornă" și era instrumentul meu, nu aplicația. **Semnalul care trebuia să oprească din prima:** *același SQL, rulat de mână pe aceeași conexiune, dădea alt rezultat decât funcția* — când cele două nu pot să difere, dar diferă, primul suspect e ce EXECUTĂ, nu ce scrie. **Regula:** orice mutație pe fișier se încheie cu ștergerea `__pycache__`, nu cu restaurarea fișierului (R96) |
