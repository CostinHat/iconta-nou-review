...
**COMANDA DE REPORNIRE (actualizata 21.08):** „Inchiderea lunii pe `facturi` e LIVRATA (core/
inchidere_luna.py, rute, ecran, gard). Toate caile care schimba evidenta fiscala a lunii sunt legate:
creare (`creeaza_factura`, prin care trece SI stornarea) si stergere. Nu exista cale de editare a
sumelor unei facturi emise - corectia se face prin storno, care e o factura noua. Restul scrierilor
(serie, tip, notificare_stop, transformat_in_id) sunt metadate si nu ating evidenta lunii. Ce ramane
pe firul asta, daca se dovedeste ca ajuta: legarea si a altor domenii de inchidere (banca, note). Apoi:
