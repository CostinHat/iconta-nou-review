...
Stare la 2026-08-15. HEAD = **acest commit** (www canonic 301 + HEAD->GET pe rutele GET, in middleware _edge_canonic_head + gard core/test_edge_canonic_head.py), IMEDIAT peste **2ed7bf6** (SEO: robots allow-list + titlu/meta/OG landing + X-Robots-Tag noindex /public/plata). Ruleaza `git log -1` pentru hash. Proba RUNNING = start-time > commit (se CITESTE la predare, nu se presupune): `systemctl show iconta-nou -p ExecMainStartTimestamp` + `git log -1`. App+prod pe 127.0.0.1:8010 (serviciul systemd `iconta-nou`, ExecStart uvicorn --port 8010; EnvironmentFile = ~/.iconta/db.env + ~/.iconta/api_keys.env - JWT_SECRET e in api_keys.env, o instanta efemera fara el REFUZA sa porneasca). Publicare origin/main + backup/lant-<data> prin post-commit (neconditionat) + restart la fiecare commit.

Lantul recent, DEJA COMIS: plimbarea Firma Grea (0054a4a -> 50cec89): 58e2aed (render CSS #5/#6 + favicon + canal note_rezultat #7), 8a965f4 (UX coada #2/#3/#4), 434efa3 (gard diacritice frontend #8), 087b33a (versionare asseturi cu gard #1), 5eeb9f2 (registre), 50cec89 (39 ghiduri cu front-matter, 79 pe disc = 79 servite = 79 sitemap); apoi SEO: 2ed7bf6 (robots allow-list + landing title/meta/OG + plata noindex). Peste ele, lotul www/HEAD (acest commit).

DE ANCORAT (√ TESTE.md): core/test_edge_canonic_head.py (6 teste) a intrat in git in ACEST commit - bifa √ se ancoreaza la hash-ul lui la commitul URMATOR (gardul anti-stale test_agenda citeste HEAD; test_agenda trece si fara √, dar inventarul A cere randul). Fisier NOU necitat = OK pt test_agenda; de adaugat randul de inventar cu √ ancorat.
...
Nimic in lucru. Lotul www/HEAD (middleware _edge_canonic_head: www->non-www 301 pastrand calea + HEAD->GET pe rutele GET, ca sa nu mai dea 405; gard core/test_edge_canonic_head.py) e in ACEST commit. Lotul SEO precedent (robots/landing/plata) e comis in 2ed7bf6. Frontul „ghid parcat” e INCHIS (39 publicate in 50cec89, ~/ghid_pending gol). De ce niciun alt front nu s-a miscat: lotul a avut raza fixa (cele 2 cereri crawler ale lui Costin); fronturile ramase asteapta fiecare o DECIZIE/INPUT (compensare/e-TVA = produs; D212 2026 = date; D300-ANAF = raspuns ANAF; D406/F116/D101G = scope/infra/instalare).
