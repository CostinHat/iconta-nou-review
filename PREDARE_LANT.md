...
Stare la 2026-08-15. HEAD = **acest commit** (bifa Inventar A √ pentru core/test_edge_canonic_head.py ancorata la 02869c0 in TESTE.md + inregistrarea gardului edge crawler/SEO in GARZI - registrul ramasese in urma cu un commit), IMEDIAT peste **02869c0** (www canonic 301 + HEAD->GET in middleware _edge_canonic_head + gard core/test_edge_canonic_head.py), la randul lui peste **2ed7bf6** (SEO: robots allow-list + titlu/meta/OG landing + X-Robots-Tag noindex /public/plata). Ruleaza `git log -1` pentru hash. Proba RUNNING = start-time > commit (se CITESTE la predare, nu se presupune): `systemctl show iconta-nou -p ExecMainStartTimestamp` + `git log -1`. App+prod pe 127.0.0.1:8010 (serviciul systemd `iconta-nou`, ExecStart uvicorn --port 8010; EnvironmentFile = ~/.iconta/db.env + ~/.iconta/api_keys.env - JWT_SECRET e in api_keys.env, o instanta efemera fara el REFUZA sa porneasca). Publicare origin/main + backup/lant-<data> prin post-commit (neconditionat) + restart la fiecare commit.
...
Nimic in lucru. Ultima actiune (acest commit): inchiderea firului de inventar - √ ancorat la 02869c0 pentru core/test_edge_canonic_head.py + inregistrarea gardului edge in GARZI. Lotul www/HEAD (middleware _edge_canonic_head: www->non-www 301 pastrand calea + HEAD->GET pe rutele GET, ca sa nu mai dea 405; gard core/test_edge_canonic_head.py) e comis in 02869c0. Lotul SEO precedent (robots/landing/plata) e comis in 2ed7bf6. Frontul „ghid parcat” e INCHIS (39 publicate in 50cec89, ~/ghid_pending gol). De ce niciun alt front nu s-a miscat: lotul a avut raza fixa (cele 2 cereri crawler ale lui Costin); fronturile ramase asteapta fiecare o DECIZIE/INPUT (compensare/e-TVA = produs; D212 2026 = date; D300-ANAF = raspuns ANAF; D406/F116/D101G = scope/infra/instalare).
