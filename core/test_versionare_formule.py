...
def test_vanzare_marja_dispecer_versionat():
    ...
    assert tm.vanzare_marja(1000, 600, 21, la_data=date(2026, 6, 1)) == tm._vanzare_marja_2018(1000, 600, 21)
    assert tm._VARIANTE_VANZARE_MARJA[0][1] is tm._vanzare_marja_2018
    with pytest.raises(ValueError):
        tm.vanzare_marja(1000, 600, 21, la_data=date(2000, 1, 1))
...
def test_marja_turism_special_dispecer_versionat():
    ...
    assert tt.marja_turism_special(*a, cota=21, la_data=date(2026, 6, 1)) == tt._marja_turism_special_2018(*a, cota=21)
    assert tt._VARIANTE_MARJA_TURISM[0][1] is tt._marja_turism_special_2018
    with pytest.raises(ValueError):
        tt.marja_turism_special(*a, cota=21, la_data=date(2000, 1, 1))
