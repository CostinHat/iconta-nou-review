# -*- coding: utf-8 -*-
"""REPOSITORY — instructiunile SQL ale lui `core/d100_reconciliere.py`.

[P7 · valul D4, 13.09.2026] Statele in `core/d100_reconciliere.py`, modul care facea DOUA straturi deodata. Textul
...
def sql(cur, q, inc, sf):
    cur.execute(q, (inc.isoformat(), sf.isoformat()))
    return cur.fetchone()


def sql_2(cur, q, inc, sf):
    cur.execute(q, (inc.isoformat(), sf.isoformat()))
    return cur.fetchone()
