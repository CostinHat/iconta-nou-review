...
| nr | funcționalitate | ecran sau rută | acțiune | declarație | stare probare |
|---|---|---|---|---|---|
| 341 | admin activitate cabinet | `GET /admin/activitate/cabinet/{firm_id}` · rol `verificat-în-corp` | citire / afisare — `admin_activitate_cabinet()` | nu |**neprobat** — reevaluat 04.09.2026 (lot 12): lotul 9 a trimis cererea cu un token `admin_firma`, iar ruta cere `superadmin`, deci a primit **`403`** și **n-a ajuns niciodată la verificările ei proprii**. Ce s-a măsurat atunci e poarta de rol, nu ruta. Confirmat prin reapăsare azi. Se reprobează în **lotul 14**, cu token de rol potrivit (`DECIZII.md` 32)|
...
| 345 | Cifre agregate din public.eveniment_public: pe eveniment, pe zi, pe pagina de provenienta | `GET /admin/analytics` · rol `superadmin` | citire / afisare — `admin_analytics()` | nu |**neprobat** — reevaluat 04.09.2026 (lot 12): lotul 9 a trimis cererea cu un token `admin_firma`, iar ruta cere `superadmin`, deci a primit **`403`** și **n-a ajuns niciodată la verificările ei proprii**. Ce s-a măsurat atunci e poarta de rol, nu ruta. Confirmat prin reapăsare azi. Se reprobează în **lotul 14**, cu token de rol potrivit (`DECIZII.md` 32)|
| 346 | admin anunt creeaza | `POST /admin/anunturi` · rol `superadmin` | creare sau executie — `admin_anunt_creeaza()` | nu |**neprobat** — reevaluat 04.09.2026 (lot 12): lotul 9 a trimis cererea cu un token `admin_firma`, iar ruta cere `superadmin`, deci a primit **`403`** și **n-a ajuns niciodată la verificările ei proprii**. Ce s-a măsurat atunci e poarta de rol, nu ruta. Confirmat prin reapăsare azi. Se reprobează în **lotul 14**, cu token de rol potrivit (`DECIZII.md` 32)|
...
| 463 | Ecranul `static/js/ecrane/activitate_cabinet.js` — 2 rute chemate: `/asistenti/echipa/centralizator`, `/asistenti/echipa/jurnal` | `static/js/ecrane/activitate_cabinet.js` | interacțiune de ecran (atribuire pe fișier) | da |neprobat — parcurs 04.09.2026 (lot 12), 0 câmpuri și 0 selecturi în DOM: centralizator și jurnal, numai afișare|
...
| 465 | Ecranul `static/js/ecrane/admin_activitate.js` — 2 rute chemate: `/admin/activitate/cabinet/{}`, `/admin/activitate/cabinete` | `static/js/ecrane/admin_activitate.js` | interacțiune de ecran (atribuire pe fișier) | nu |neprobat — parcurs 04.09.2026 (lot 12), 1 câmp, și e FILTRU (caută cabinet); butoanele («Suspendă») acționează pe un rând ales, nu pe conținutul unui formular|
| 466 | Ecranul `static/js/ecrane/admin_analytics.js` — 1 rute chemate: `/admin/analytics` | `static/js/ecrane/admin_analytics.js` | interacțiune de ecran (atribuire pe fișier) | nu |neprobat — parcurs 04.09.2026 (lot 12), 0 câmpuri și 0 selecturi în DOM: panou de cifre publice, numai afișare|
...
| 469 | Ecranul `static/js/ecrane/ansamblu.js` — 1 rute chemate: `/ansamblu` | `static/js/ecrane/ansamblu.js` | interacțiune de ecran (atribuire pe fișier) | nu |neprobat — parcurs 04.09.2026 (lot 12), 0 câmpuri și 0 selecturi în DOM: prezentarea de ansamblu, derivată din `/ansamblu`|
| 470 | Ecranul `static/js/ecrane/asistent.js` — 2 rute chemate: `/control-fiscal`, `/raportari/contor` | `static/js/ecrane/asistent.js` | interacțiune de ecran (atribuire pe fișier) | nu |neprobat — parcurs 04.09.2026 (lot 12), pe cont REACTIVAT prin ruta aplicației (`POST /asistenti/{uid}/reactiveaza`) și dezactivat la loc în `finally`, cu starea verificată în bază: 7 carduri, **0 câmpuri și 0 selecturi**, 0 erori JS. Bara a treia (motivațională, doar la rolul `angajat`) se randează cu cifre reale. Cardurile lui deschid ecrane deja probate pe contul de cabinet|
...
| 472 | Ecranul `static/js/ecrane/cabinet.js` — 14 rute chemate: `/asistenti`, `/asistenti/echipa/centralizator`, `/asistenti/echipa/jurnal`, `/asistenti/echipa/semafor`, `/cabinet/consolidare`, `/coada`, `/control-fiscal`, `/eu/educatie`, `/eu/educatie/patru-ochi/vazut`, `/eu/patru-ochi`, `/migrare/status`, `/raportari/contor`, `/tenants`, `/termene` | `static/js/ecrane/cabinet.js` | interacțiune de ecran (atribuire pe fișier) | da |neprobat — parcurs 04.09.2026 (lot 12), desktopul cabinetului: 0 câmpuri pe pagină. Cardurile deschid ferestre, iar ferestrele sunt unitățile probate separat|
| 473 | Ecranul `static/js/ecrane/capacitate.js` — 1 rute chemate: `/capacitate` | `static/js/ecrane/capacitate.js` | interacțiune de ecran (atribuire pe fișier) | nu |neprobat — parcurs 04.09.2026 (lot 12), 0 câmpuri și 0 selecturi în DOM. Singurul buton («Adaugă asistenți») NAVIGHEAZĂ la ecranul Asistenți — nu ia conținutul niciunui formular|
| 474 | Ecranul `static/js/ecrane/control.js` — 2 rute chemate: `/control-fiscal`, `/control-fiscal/{}` | `static/js/ecrane/control.js` | interacțiune de ecran (atribuire pe fișier) | nu |neprobat — parcurs 04.09.2026 (lot 12), 0 câmpuri și 0 selecturi în DOM: semaforul portofoliului, numai afișare|
| 475 | Ecranul `static/js/ecrane/control_verdict.js` — 2 rute chemate: `/control-fiscal/{}/audit-preluare`, `/tenants/{}/facturi/{}/contabilizeaza` | `static/js/ecrane/control_verdict.js` | interacțiune de ecran (atribuire pe fișier) | nu |neprobat — parcurs 04.09.2026 (lot 12), 0 câmpuri și 0 selecturi în DOM: corpul verdictului, randat din `/control-fiscal/{}`|
...
| 485 | Ecranul `static/js/ecrane/operatiuni_ecran.js` — 0 rute chemate | `static/js/ecrane/operatiuni_ecran.js` | interacțiune de ecran (atribuire pe fișier) | ? |neprobat — parcurs 04.09.2026 (lot 12), fișierul ecranului «Operațiuni speciale»; geamănul lui `#fa-operatiuni` (#447) a dat `campuri=0`|
...
| 495 | Ecranul `static/js/ecrane/supervizor.js` — 1 rute chemate: `/supervizor` | `static/js/ecrane/supervizor.js` | interacțiune de ecran (atribuire pe fișier) | nu |neprobat — parcurs 04.09.2026 (lot 12), 0 câmpuri și 0 selecturi în DOM. E și în inventarul porții vizuale (`nav_ecrane.ECRANE`), deci primește deja cele trei unelte vizuale|
| 496 | Ecranul `static/js/ecrane/termene.js` — 1 rute chemate: `/termene` | `static/js/ecrane/termene.js` | interacțiune de ecran (atribuire pe fișier) | da |neprobat — parcurs 04.09.2026 (lot 12), 0 câmpuri și 0 selecturi în DOM: scadențele, grupate pe dată; butoanele filtrează|
| 497 | Ecranul `static/js/ecrane/tipare.js` — 2 rute chemate: `/tipare`, `/tipare/ai` | `static/js/ecrane/tipare.js` | interacțiune de ecran (atribuire pe fișier) | nu |neprobat — parcurs 04.09.2026 (lot 12), 0 câmpuri și 0 selecturi în DOM, pe starea GOALĂ («încă nu sunt tipare de arătat»). «Generează analiză AI» NU s-a apăsat: `/tipare/ai` cheamă `core.ai_client`, adică un furnizor din afara aplicației, contra cost|
