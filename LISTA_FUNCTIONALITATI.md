...
| 470 | Ecranul `static/js/ecrane/asistent.js` — 2 rute chemate: `/control-fiscal`, `/raportari/contor` | `static/js/ecrane/asistent.js` | interacțiune de ecran (atribuire pe fișier) | nu |neprobat 04.09.2026 (lot 12) — **fără subiect**: desktopul asistentului cere rolul `angajat`, iar singurul cont cu rolul ăsta din bază (`asistent@prisma-cont.test`) e INACTIV, deci `sesiune_pentru_user` îl refuză. Nu e „fără defect”, e un ecran fără cont viu. Se probează după reactivarea lui prin calea aplicației (`/asistenti/{id}/reactiveaza`)|
