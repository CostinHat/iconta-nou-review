---
title: "Sancțiuni pentru nedepunerea declarației de mențiuni ONRC"
description: "Limitare importantă: sursele verificate nu conțin un temei ONRC distinct pentru mențiuni; cea mai apropiată sancțiune reală este cea din Codul de procedură fiscală, pentru declarația de mențiuni la ANAF."
published: 2026-09-24
modified: 2026-09-24
poarta: v1
---

# Sancțiuni pentru nedepunerea declarației de mențiuni ONRC

Atenție la formulare: „declarația de mențiuni" circulă în două registre diferite — cea depusă la Oficiul Registrului Comerțului (ONRC), pentru modificări ale actului constitutiv/datelor firmei în registrul comerțului, și cea depusă la organul fiscal (ANAF), pentru modificări ale datelor de înregistrare fiscală. Regimul registrului comerțului nu mai e dat de Legea nr. 26/1990 — aceasta a fost abrogată prin Legea nr. 265/2022 (art. 140 alin. (1) lit. a)), care reglementează acum înregistrarea în registrul comerțului. Sursele verificate aici nu conțin o sancțiune ONRC distinctă pentru nedepunerea unei declarații de mențiuni, așa că acest ghid redirecționează onest spre sancțiunea reală, verificată, din Codul de procedură fiscală — cea mai apropiată prevedere disponibilă, dar despre mențiunile fiscale, nu despre cele de la ONRC.

## Temeiul legal

::: ghid-temei
„ART. 336 Contravenții
(1) Constituie contravenții următoarele fapte, dacă nu au fost săvârșite în astfel de condiții încât să fie considerate, potrivit legii, infracțiuni:
a) nedepunerea de către contribuabil/plătitor la termenele prevăzute de lege a declarațiilor de înregistrare fiscală, de radiere a înregistrării fiscale sau de mențiuni;
[...]
(2) Contravențiile prevăzute la alin. (1) se sancționează astfel: [...]
d) cu amendă de la 1.000 lei la 5.000 lei pentru persoanele juridice încadrate în categoria contribuabililor mijlocii și mari și cu amendă de la 500 lei la 1.000 lei, pentru celelalte persoane juridice, precum și pentru persoanele fizice, în cazul săvârșirii faptei prevăzute la alin. (1) lit. a), b) și i) - m)."
— Legea nr. 207/2015 (Codul de procedură fiscală), art. 336 alin. (1) lit. a) și alin. (2) lit. d) (sursă: anaf_surse/legea_207_2015_consolidat.txt)
:::

Ce se poate confirma din acest temei — valabil pentru declarația de mențiuni **fiscală**, nu pentru cea de la ONRC:

- Nedepunerea la termen a declarației de mențiuni la organul fiscal este **contravenție**, distinctă de eventuale sancțiuni de la ONRC pentru aceeași modificare.
- Amenda pentru majoritatea firmelor (altele decât contribuabilii mijlocii și mari) este **500-1.000 lei**; pentru contribuabilii mijlocii și mari, **1.000-5.000 lei**.
- Termenul de referință pentru depunerea declarației de mențiuni fiscale este de 15 zile de la producerea modificării (art. 88 din același cod).

## Ce se greșește în practică

- Se presupune că depunerea mențiunii la ONRC scutește automat de depunerea declarației de mențiuni la ANAF (sau invers) — sunt proceduri și registre distincte, chiar dacă privesc aceeași modificare a firmei.
- Se caută sancțiunea ONRC în Codul de procedură fiscală — acesta reglementează doar relația cu organul fiscal, nu regimul registrului comerțului.
- Se ignoră total termenul de depunere presupunând că „nu se verifică" — nedepunerea rămâne contravenție indiferent de frecvența controalelor.

## Ce face iConta.eu

La data acestui ghid, iConta.eu nu are un flux automatizat de generare sau depunere a declarațiilor de mențiuni, nici către ANAF, nici către ONRC — profilul firmei din aplicație se actualizează manual, pentru uzul intern al contabilității, iar obligațiile declarative efective către cele două instituții rămân în sarcina angajatorului/contabilului.

[iConta.eu](/)
