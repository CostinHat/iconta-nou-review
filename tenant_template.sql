...
-- [d205_v1 27.07.2026] Tabelul exista din 2026 DOAR in tenant_002 (creat ad-hoc,
-- nemirrorat in template). Consecinta: D205 crapa pe tenant_001, tenant_003 si pe
-- ORICE firma noua. Gardul F165 (test_audit_schema) NU l-a prins: verifica doar
-- directia template->tenant, iar tabelul lipsea chiar din template -> absenta lui
-- din tenanti nu era 'lipsa', iar prezenta in tenant_002 era 'extra' tolerat.
CREATE TABLE IF NOT EXISTS TENANT_PLACEHOLDER.d205_beneficiari (
    id SERIAL PRIMARY KEY,
    an integer,
    tip_venit text DEFAULT '08'::text,
    nume text,
    cnp text,
    rezid integer DEFAULT 1,
    suma_bruta numeric DEFAULT 0,
    impozit numeric DEFAULT 0,
    creat timestamp without time zone DEFAULT now()
);
ALTER TABLE TENANT_PLACEHOLDER.d205_beneficiari OWNER TO iconta_user;
