...
## 11.08.2026 — Certificare-comportament: read-side behavioral + DS browser firma-level (cluster 4-5)
...
## 11.08.2026 — Certificare sub-ecrane adanci (39 ecrane) + bonuri portal (Costin)
CRAWLER ADANC (frontend_test crawl, prin nginx https://iconta.eu): fiecare ecran (13 cabinet + 25 firma + emitere) -> enumerat + deschis individual FIECARE sub-element interactiv (buton/tab/formular non-nav, non-distructiv), verificat pe DOM randat: render, 0 erori consola, 0 violari CSP, clase DS. ~130 functionalitati cu pytest/DUK trecute prin ACEEASI parcurgere in browser (cele cu UI).
REZULTAT: covarsitor curat. Findings:
- CLASA DS-INPUT (browser a prins ce verificatorul static rateaza): formulare care folosesc clase ad-hoc pe input/select in loc de .camp-input canonic (DS §2.1 "exclusiv .camp-input"): Produse "+ Adauga produs" = .pr-input (denumire/pret/UM/select, compact inline "contrast_ferestre_v1"); Asistenti "Vizualizeaza" = select .asi-per-sel. Vizual similare (aceeasi bordura #b9c2cf) dar diferite (fara height 40px/font-family/color; radius hardcodat). Par INTENTIONATE; verificatorul (0) le tolereaza. RAPORTAT, nereparate (decizie DS-owner; refactor la .camp-input risca layout).
- CLASA CONSOLE-400: butoane de actiune care fac request ce da 400 cand lipsesc precconditii (Casa "Adauga" gol; Salariati "Raspunsuri REGES"/"Fisier plata card SEPA" fara config/IBAN; Acces client "Previzualizeaza portalul" fara client). App-ul se RECUPEREAZA (ecran intact, fara crash). Igiena UX minora, raportat.
...
## 18.08.2026 — Audit tenant_006 (cont.): D100 micro pe fapt de venituri
Verificat picioarul declaratiilor la XML+DUK: D301 valid, D406 valid (68895B, avertisment conturi 731-738 excluse din norma A), D101/D112 genereaza (gol - firma micro/fara salariati), D205/D390 refuza pe gol. Gasit: semaforul arata D100 micro restanta fara a verifica baza de venituri, dar D100 pe zero e STRUCTURAL invalid la DUK (generatorul refuza) -> restanta falsa. Reparat cu d100_fapt (simetric d390_fapt/d112_fapt): trimestru inchis fara venituri -> "nu se datoreaza", nu restanta. tenant_006/003 corectate (restante moarte -> neaplic consistent cu generatorul); tenant_002 T1 (are venituri) pastrata. Gard RED(mutatie)->GREEN. Commit: aceasta tura.

## 18.08.2026 — Audit tenant_006 (cont.): a11y contrast Control fiscal + import blockages
Provocat vizual blocajele de import (cerute in comanda, sarite anterior): salariati CNP invalid + solduri neechilibrate -> ambele model (camp marcat, motiv vizibil nu title-only, consecinta, buton salvare dezactivat, diacritice). axe pe Vector fiscal/Date firma = contrast 0. Contrastul izolat pe Control fiscal (17 perechi, 2 tokeni pe panoul #e9edf3): reparat scoped (#2f6fa6 coduri, #5c6675 sub-text), token global neatins. axe contrast=0 dupa (captura privita). Gard extins mutatie-probat, DS v2.43, versioneaza_assets. Commit: aceasta tura.

## 18.08.2026 — Audit tenant_006 (cont.): field-level error marking + front D390 verificat
Front 3 (field-marking) LIVRAT: eroareCamp marca doar mesajul langa camp, nu inputul; acum adauga contur rosu (camp-invalid + aria-invalid), app-wide (7 ecrane). Capcana: bordura globala !important (contrast_ferestre_v1, specificitate 0,6,1) - overrideul reproduce selectorul + clasa (0,7,1). Captura privita Date firma (2 campuri rosii, dispar la corectare). Gard mutatie-probat. Front 2 (D390<->d301) verificat la sursa: ramane decizie - d301_operatiuni n-are TVA+tara furnizorului pe care D390 cod A le cere; fixul e alegere de produs pe fluxul datelor. Commit: aceasta tura.

## 18.08.2026 — Audit tenant_006 (cont.): front 2 (D390<->d301) rezolvat pe corectitudine
Front 2 investigat: D390 SE POATE produce manual (UI clasificare are Tip A - achizitie bunuri IC cu tara+cod furnizor; probat: linie manuala -> XML valid nr_opi=1, DUK valideaza codul TVA). Deci nu blocaj de corectitudine. Fix: refuzul D390 "pe zero" SEMNALEAZA acum operatiunile din d301_operatiuni si indruma spre adaugarea manuala, in loc de mesaj generic fals. Mirror al refuzului D301<->facturi. Auto-derivarea d301->D390 ramane decizie (recomandare NU: caz de margine art.317, cere migrare DB + camp partener pe ecranul D301). Gard mutatie-probat. Commit: aceasta tura.

## 18.08.2026 — Audit tenant_006 (cont.): auto-derivare d301->D390 cod A/S CONSTRUITA (decizia Costin)
Costin a cerut construirea auto-derivarii (peste recomandarea executorului). Livrat cap-la-coada: (A) migrare DB d301_operatiuni + furnizor (tara/cod/den), 19/19 scheme, test_audit_schema verde; (B) ecran D301 + 3 campuri furnizor optionale + indicator grila; (C) generator d390 citeste d301 (tip 1/3->A, 5->S; 2/4 excluse), injectat ca linii pre-tipizate, cu reconciliere a-doua-cale independenta (_pull_d301); (D) refuz-pe-zero rafinat (indruma completarea furnizorului). Proba: tenant_006 op tip 1 cu furnizor DE -> D390 auto-derivat DUK VALID (baza 52261, cod A); captura privita ecran D301. Gard mutatie-probat (cele doua cai coincid). Commit: aceasta tura.
...
## 18.08.2026 — Campanie formulare _DOAR_API: d311 (2/6)
D311 (TVA datorata dupa anularea codului de TVA) capata formular manual in ecranul Declaratii. Spre deosebire de d710 (lista), d311 e PANOU-CAMP: data anularii + motiv (din oficiu / la cerere) + 3 situatii oficiale (livrari / achizitii cu taxare inversa / livrari cu TVA la incasare exigibila dupa anulare), fiecare cu baza+TVA; subtotalurile si totalul de plata se calculeaza (total LIVE la tastare). Parametri cititi din semnatura d311.genereaza + structura oficiala (structura_D311, XSD v1.02). Mesajele erori_generare rescrise in limba contabilului (fara Data_A/d_anul1/OB_51); test_d311 re-ancorat; gard test_d311_formular mutatie-probat. Scos din _DOAR_API (d307 pus la loc, prins de test_live_accesibil). CSV F207->LIVE + login.js GRUPE_FUNC regenerat (verificatorul respinsese primul commit pe count 145 vs 146). Proba Playwright reala (ALFA MICRO, declarant setat via API): DUK valid, axe 0, mobil OK. Commit 238deb0. Urmatoarea: d307 (lista, ca d710).

## 18.08.2026 — Campanie formulare _DOAR_API: d307 (3/6)
D307 (ajustare/corectie/regularizare TVA) capata formular manual in ecranul Declaratii. Model LISTA (ca d710, nu panou-camp ca d311): contabilul adauga operatiuni de ajustare, fiecare cu tip (A=transfer active/cedent, L=leasing/finantator, C=anulare cod TVA/beneficiar), cod fiscal + denumire operator, suma TVA (poate fi <=0 la regularizare). tvaA/L/C + total de control se calculeaza; total afisat live. Parametri cititi din semnatura d307.genereaza + structura oficiala (D307Validator.jar). Mesajele erori_generare rescrise in limba contabilului (fara denO/codO/operatiuni/d_anulare); gard test_d307_formular mutatie-probat. Scos din _DOAR_API (DOAR d307, vecinii intacti). CSV F217->LIVE + GRUPE_FUNC regenerat. Proba Playwright reala (ALFA MICRO): DUK valid, axe 0, mobil fara overflow. Urmatoarea: d107 (informativa sponsorizari, lista beneficiari).

## 18.08.2026 — Campanie formulare _DOAR_API: d107 (4/6)
D107 (informativa privind beneficiarii sponsorizarilor / mecenatului / burselor private) capata formular manual in ecranul Declaratii. Model LISTA (ca d710/d307): contabilul adauga beneficiari, fiecare cu denumire, cod fiscal (CUI/CNP), adresa si TREI sume - acordata (Val1), reportata (Val2), dedusa (Val3). Anexa beneficiarilor neindividualizati (entit1) e o sub-sectiune `<details>` colapsata (caz rar): apare lista cu nume/cod/adresa DOAR daca suma reportata a lor (Val2_NI) > 0 - exact regula validatorului ANAF (entit1 exista <=> Val2_NI>0). Totalurile pe cele trei coloane (TVal1/TVal2/TVal3) + suma de control (totalPlata_A) se calculeaza si se afiseaza (coincid cu serverul, Regula 14.2: acordata 8.000 + reportata 2.000 + dedusa 6.000 = control 16.000). ZERO clase noi (aceleasi dec-man-rand/dec-man-form/dec-xml/camp ca d307). Parametri cititi din semnatura d107.genereaza + structura oficiala (D107Validator.jar v1, anaf_surse structura_D107_2024). Mesajele erori_generare rescrise in limba contabilului (fara denE/cifE/Val1/Val2_NI/entit1/cod_oblig); valideaza_cerere d107 rescris la fel (nu mai expune `manual.beneficiari`); test_d107 re-ancorat + gard test_d107_formular mutatie-probat (sed reintroduce denE -> RED). Scos DOAR d107 din _DOAR_API (tipuri: 12). CSV F211->LIVE + GRUPE_FUNC regenerat (148) + registre IN ACELASI commit. Proba Playwright reala (ALFA MICRO, an 2024, 2 beneficiari Asociatia Alfa+Fundatia Beta): DUK VALID, XML Val1=5000/TVal1=8000/2x<entit>, axe contrast+eticheta 0, mobil Pixel5 body=393 fara overflow. Captura privita. Urmatoarea: d177 (redirectionare profit ONG).

## 18.08.2026 — Campanie formulare _DOAR_API: d177 (5/6)
D177 (cerere de redirectionare a unei parti din impozitul pe profit catre entitati nonprofit / unitati de cult, art.25(4)i/t CF) capata formular manual in ecranul Declaratii. Cea mai bogata forma din campanie: un ANTET cu plafoanele (suma maxima redirectionabila = min(20% impozit profit, 0,75% cifra afaceri) / redirectionata anterior / ramasa) + perioada fiscala (dataInceput/dataSfarsit, prefill 01.01-31.12.an), apoi o LISTA de beneficiari (ca d107/d307): fiecare cu tip (1=cult/PJ fara scop lucrativ, 2=alti beneficiari, 3=mecenat/PF, 5=UNICEF/org internationale; 4=bursa privata nepermis de validator), cod fiscal (CUI, ori CNP la mecenat - eticheta se schimba dupa tip), denumire, IBAN (RO+22), suma, acord de informare (checkbox), contract (obligatoriu la tip 1/2/3, ascuns la UNICEF). Total alocat vs. suma ramasa afisat; suma de control = 0 (D177 informativa). ZERO clase noi. Etichete tipB/acord CITITE LA SURSA (anaf_surse structura_D177_2026, Regula 2). Mesajele erori_generare + valideaza_cerere rescrise in limba contabilului (fara tipB/cuiB/denB/ibanB/sumaB/contractB/sumaMax/sumaRest/tipPlatitor/`manual.beneficiari`); test_d177 re-ancorat + gard test_d177_formular mutatie-probat (sed reintroduce sumaRest -> RED). Scos DOAR d177 din _DOAR_API (tipuri: 13). BUG REAL prins de DUK pe traseu (nu de formular): dispatch-ul trimitea luna=6, dar validatorul (R4.1) cere luna XML = luna din dataSfarsit (12 la an calendaristic) -> reparat in genereaza (luna derivata din dataSfarsit; 12 daca e nul), aserție de regresie in gard. CSV F210->LIVE + GRUPE_FUNC (149) + registre IN ACELASI commit. Proba Playwright reala (ALFA MICRO, an 2025, plafon 10000, 1 beneficiar tip 2 Asociatia Binele, IBAN valid, suma 10000): DUK VALID, XML tipB=2/sumaRest=10000/1x<beneficiar>/luna=12, axe 0, mobil Pixel5 body=393. Captura privita. Urmatoarea: d207 (nerezidenti, ultima din campanie).


## 18.08.2026 — Perimetru extins 006: F5 nume intern D301 + F6 a11y app-wide (ratchet)
Doua clase gardate app-wide, provocate de auditul 006 dar reparate pe tot ecranul. **F5 (31e4cbb):** D301 expunea nume interne de camp in mesaje (denO/codO etc.) -> re-ancorat la limba contabilului + gard ratchet care refuza reaparitia numelor interne in erori_generare pe toate declaratiile. **F6 (5abaf77):** audit axe app-wide -> 7 ecrane gardate pe contrast/eticheta/landmark, baseline pus. Principiul „perimetru extins": cand simptomul e o CLASA, gardul acopera tot, nu doar instanta care l-a scos la iveala.

## 18.08.2026 — Ghiduri: 51 noi + corectie TollRo + retragere 6 duplicate (301)
**8dd0b4b:** 51 ghiduri noi (front-matter) publicate; corectie TollRo -> data corecta 1 octombrie 2026. Discrepanta 51 vs. 45 asteptate ridicata ca needs-input; tinta clarificata = 170. **ff1cffb:** 6 ghiduri duplicate cu sufix „-contabil" retrase, cu redirect 301 permanent -> total 164 (nu 170; cele 6 erau duplicate reale, nu lipsa). Fara continut orfan (redirect, nu 404).

## 18.08.2026 — Audit tenant_006 D390: doua goluri de raportare gardate
**ade5ab6 + registre bc9f65f.** Auditul D390 pe 006 a scos doua goluri: (Q1a) o operatiune putea intra din DOUA surse (facturi_ic + d301 derivat) -> avertisment de dubla-sursa; (Q2) achizitia „primita fara CUI" nu era numita corect la formarea liniei. Ambele = avertismente neblocante (nu pierd operatiunea obligatorie), gardate pe operatiuni reale. `_sursa` marcheaza provenienta. §2.2 + §11 (GARZI, ISTORIC_TENANTI) in commit separat de registre.

## 18.08.2026 — Audit tenant_006 D301: ruta de adaugare refuza platitorii de TVA
**78aaf27 + registre d40d356.** Ecranul D301 lasa un platitor de TVA sa adauge operatiuni (D301 e pentru NEplatitori/achizitii IC sub plafon). Ruta d301_operatiuni_api.adauga refuza acum platitorul, cu mesaj in limba contabilului. Corelat: vectorul 006 avea platitor_tva gresit -> corectat (platitor_tva=False, inreg_art317=True) si verificat pe ecran ca obligatiile se schimba (Regula 14). Gard pe date reale.

## 18.08.2026 — Sweep §8: eligibilitatea vectorului pe toate rutele de intrare manuala
**6e034b4 + corectie vector 06a2d01.** Dupa garda D301-platitor, sweep pe toate caile de intrare manuala (D300/D390): verifica ca fiecare cere eligibilitatea din vectorul fiscal (perioada_tva_tip fara default tacit), nu doar D301. Corectie vector 006 in acelasi lant. §5 (ramas de remediat) golit: remediabilele reparate inainte de raport (feedback Costin: §5 = gol sau doar imposibilitati explicate).

## 17-18.08.2026 — Coada de validare: traseu de vizualizare a continutului
**5a21ba4.** Cardul „De validat" lista elementele dar niciunul nu se deschidea la click (verificat pe 003 si 006, D301 iunie). Lipsea traseul, nu era intentionat. Backend GET /coada/{id}/continut (read-only) + validat.js: deschideContinut cu decodare UTF-8-safe `decodeURIComponent(escape(atob(...)))` (R14 a prins mojibake „BucureÈ™ti" -> reparat). Gard test_coada_vizualizare. Playwright: 4 capturi (dashboard OFF -> ecran coada -> dialog -> dupa depunere).

## 19.08.2026 — Audit 006/R24.1: checksum VIES al codului de TVA la introducere
**734f912.** D390 pe 006 respins de DUKIntegrator R24.1 (cod TVA „811234567" nu respecta algoritmul „DE"). Aceeasi clasa cu preview<->salvare: calea d301-derivata NU trecea prin _facturi_ic, deci codul gresit ajungea neverificat pana la respingerea DUK - aceeasi regula, verdicte diferite in etape diferite. Reparat: algoritmul codului de TVA (offline DE/FR/HR) se verifica ACUM in d301_operatiuni_api.adauga -> avertisment neblocant (simetric cu cifra de control CUI RO). Achizitia IC obligatorie nu se pierde. Gard test_d301_vies_la_introducere RED-probat. R14: form op DE811234567 -> avertisment „va fi respins de DUK (R24.1)".

## 19.08.2026 — temei_307: excluderea D301->D390 devine auditabila per operatiune
**a0bafbb.** Euristica veche „tip 4 + cod -> serviciu" + confirma_local (confirmare per-firma opaca) SCOASE, inlocuite (nu dublate; zero cod mort) de temei_307: fiecare operatiune D301 tip 4 (art. 307 alin. 3/5/6) poarta CARE alineat se aplica, cerut la introducere (respins fara temei; nu se deduce). Excluderea din D390 devine AUDITABILA: excluse_d301 -> motiv numit + temei citat (simetric cu diag-ul facturilor); temei NULL = SEMNAL, nu verde. Migrare pe toate cele 19 scheme tenant (migrare_d301_temei_307.py; migrare_d301_confirmat.py stearsa). UI: dropdown temei pe tip 4 (declaratii.js), vizualizare coada. Acceptare 006/iunie: D390 = 1 linie cod A DE136695976 baza 52.261; INV-DE-88 in excluse cu semnal neconfirmat; tip 4 fara temei respins. Trei respingeri pre-commit (diacritice / „DUK regula" / citari-istorice in registre) rezolvate.

## 19.08.2026 — GARD D394 trimestrial + inchiderea misdiagnosticului 003
**9e839e4.** Constatarea „D394/003 emite septembrie in loc de trimestru + cifre necorespunzatoare" investigata read-only si inchisa ca MISDIAGNOSTIC (cod corect, ZERO reparatie). Verificat la sursa: (1) OPANAF 2194/2025 lit. c -> luna trimestrial = 03/06/09/12, deci 09=T3 e codificarea ANAF, nu „septembrie"; (2) fereastra_tva(.,T) = trimestrul intreg [2026-07-01,2026-10-01), iulie/august nu se pierd; (3) 5900 baza + 889 TVA = 6789 = factura din august, culeasa corect. Gard core/test_d394_trimestrial_perioada.py (4 teste, RED-probat prin mutatia fereastra T -> luna-ancora -> 3 pica): freeze pe eticheta (trim*3∈{3,6,9,12}) + fereastra (trimestru intreg, augustul inauntru). Inchidere documentata in PREDARE_LANT.md („NU redeschide" + cele 3 puncte). D300/D394/D406 impart acelasi tipar corect perioada.luna+fereastra_tva.

## 20.08.2026 — Patru-ochi pe cabinet solo: indicatorul nu mai minte + fundatura de aplicabilitate
**9d963d6 + igiena/registre f4caced.** Coada de validare pe cabinetul Prisma 1968 (un singur validator competent, patron Elena): subbara afisa „Validarea in doi asistenti ✓" in timp ce, pe ACELASI ecran, cardul zicea „De depus · 3 declaratii de depus" — contradictie cu sine, prinsa in captura `po_0_inainte_dashboard.png`. Cauza: enforcement-ul (`coada_api.aproba`) folosea DEJA `activ ∧ posibil`, dar `GET /eu/patru-ochi` intorcea doar flagul brut `{activ}`, iar UI-ul (`validat.js:52`, `cabinet.js:393`, `_indicatorPatruOchi`) decidea pe el. Aceeasi familie cu preview↔salvare: doua definitii ale aceleiasi stari, in doua straturi. **Regula scrisa (DECIZII 20.08, GARDATA):** patru-ochi are doua axe care nu se confunda — POLITICA (`activ`, explicita, a patronului, persistenta, nu se re-cere) x APLICABILITATE (`posibil`, aritmetica, calculata live, obligatorie); `efectiv = activ ∧ posibil`, dintr-o SURSA UNICA (`coada_api.patru_ochi_stare`) consumata si de `aproba` si de endpoint — front si back nu mai pot diverge fiindca nu mai exista doua definitii. Indicatorul are TREI stari: in vigoare (bifa, `--verde-inchis`), pornit-dar-suspendat (`--ardezie`, FARA bifa si FARA verde — verdele e afirmatia „controlul functioneaza"), oprit (absent). **Defectul principal era indicatorul, nu butonul lipsa:** un cabinet care crede ca are control in doi si n-are e mai rau decat o coada blocata. **FUNDATURA gasita raspunzand la intrebarea „ce se intampla cu declaratiile deja in coada":** afirmatia din propunerea de dimineata („deadlock imposibil in operare normala") era FALSA — formula veche a lui `posibil` („>=1 pregatitor + >=1 validator + >=2 oameni") devenea `true` cand un cabinet solo angaja un asistent DOAR cu `poate_pregati`, iar cele 3 declaratii pregatite de unicul validator deveneau neaprobabile de NIMENI, fara iesire din aplicatie. Reparat: `posibil ⇔ >=2 VALIDATORI activi`; nu se pierde nicio protectie (un om fara drept de validare nu putea oricum aproba nimic). **Alegerea pe declaratiile din coada la trecerea `posibil` false->true: RETROACTIV** — trec la al doilea validator, pregatitoarea nu le mai poate auto-aproba (enforcement fara stare, calculat la clic, deci auto-vindecare in ambele sensuri, fara flag per-item; a le „grandfather-iza" ar fi lasat tocmai munca nevalidata sa treaca in ziua in care patronul a pornit controlul). **Trecerea granitei NU e tacuta, in trei locuri:** subbara (crom persistent = STAREA, la fiecare deschidere a spatiului de lucru), dialogul „Acorzi dreptul de validare lui X (Nivel 2)?" din Asistenti (= MOMENTUL, prin `patru_ochi_intra_in_vigoare` intors de `set_permisiuni`), si textul cozii — care era el insusi mincinos: zicea „Patru-ochi e dezactivat" despre o politica doar SUSPENDATA, adica il lasa pe patron sa creada ca i s-a stins setarea; acum trei variante, cu golul + cauza + iesirea (DS cap.6). **Reparatie reala:** intrebarea „cine poate aproba in acest cabinet" era pusa in TREI locuri cu trei interogari proprii (`coada_api` aplicabilitate, `asistenti_api._nr_validatori` educatie, `notificari_api.validatorii_cabinetului` destinatari) -> toate deriva din `coada_api.validatori_activi`. Garzi RED-probate: `core/test_patru_ochi_efectiv.py` (14 teste, 8 pica pe HEAD, intre care fundatura pe latura COMPORTAMENTALA — direct pe `aproba`, nu doar aritmetic) + `core/test_a11y_contrast_tokens.py` extins (ambele culori >=4.5:1 pe bara #dfe4ea: `--verde-inchis` 4.57, `--ardezie` 9.85; starea suspendata nu poate purta un token verde). Mutatii prinse: prag `>=2`->`>=1` (4 pica), `notificari_api` cu interogare proprie (1 pica), `--ardezie`->`--verde-inchis` (2 pica). DS v2.60. Proba live pe 1968 (`frontend_test/proba_patru_ochi.py`), trei stari pe acelasi ecran, capturi PRIVITE: `po_0_inainte_*` (bifa mincinoasa, cod vechi simulat prin mutatie temporara restaurata) -> `po_1_suspendat_*` (text onest, rgb(43,52,64), card „De depus") -> `po_2_efectiv_*` (bifa adevarata, card „De validat", ZERO butoane de aprobare pentru pregatitoare, cele 3 marcate „ai pregatit-o tu — o valideaza altcineva", iar „altcineva" exista acum). axe 0 desktop + Pixel 5 (body=393), tinta indicator 54px >= AA 2.5.8. **Igiena de date (f4caced):** asistentul 6248 activat TEMPORAR pentru starea 2 si restaurat exact (activ=f, competente f/f/f); flagul `patru_ochi_activ` pe 1968 STINS dupa reparatie (era ON doar ca proba vizibila) — ramura „suspendat" ramane acoperita de gard si de capturile comise, nu de o stare lasata in baza.

## 20.08.2026 — D390: checksum-ul VIES se aplica pe TOATE liniile, nu doar pe cele din facturi
**9d963d6 (acelasi bundle).** Continuarea directa a lui 734f912 (19.08): acolo s-a adaugat verificarea codului de TVA la INTRODUCERE in ecranul D301; aici s-a inchis clasa la SURSA. `checksum_vies` era chemat DOAR de `_facturi_ic` (latura auto-derivata din facturi); bucla liniilor MANUALE din `calcul_d390` verifica exclusiv lungimea `codO` (>12), nu si cifra de control — deci o operatiune introdusa de contabil in ecranul D390 sau derivata din D301 intra in declaratie cu un cod de TVA nevalidat, iar contabilul afla abia din respingerea de la DUK regula R24.1. Aceeasi informatie, doua masuri, dupa calea de intrare. Fixul e in `calcul_d390`, la sursa COMUNA a ambelor cai (nu la fiecare punct de scriere — ar fi fost a treia copie), cu acelasi diagnostic, aceeasi categorie `checksum` si acelasi caracter **NEBLOCANT** ca pe facturi (motivat explicit in docstringul lui `d390.valideaza`: o operatiune obligatorie raportata cu cod invalid e mai buna decat una disparuta tacit; partenerul e NUMIT, DUK decide). Exceptie deliberata gardata: `codO` gol pe tip A/S nu e raportat ca invalid — A si S il pot omite legal (OPANAF 394/2017 anexa 2, NOTA 1), iar lipsa nu e greseala de cifra de control. Verificat la sursa si gasit CURAT: `salveaza_reclasificare` reclasifica o pereche `(tara, cod)` EXISTENTA, nu introduce cod nou de partener — deci un singur fix inchide clasa, cum se anticipase. Gard `core/test_d390_checksum_manual.py` (5 teste, 2 pica pe HEAD: linia manuala DE136695975 fara niciun diagnostic, si cele doua cai cu verdicte DIFERITE pe acelasi cod). Non-regresie pe date reale: linia lui tenant_006 (iunie 2026, cod A, DE136695976, baza 52.261) ramane curata, zero fals-pozitiv, D390 in continuare DUK-valid. RAMAS NUMIT (nu tacut, nu gaura de corectitudine): formularul de linie manuala D390 (`d390_clasificare_api.manual_adauga`) nu da inca avertismentul la INTRODUCERE, cum face ecranul D301 — corectitudinea e inchisa (avertismentul apare la generare, cu partenerul numit), simetria „contabilul afla DEVREME" nu; de facut cand se atinge ecranul, fiindca raspunsul rutei e azi aruncat de `declaratii.js:371` (backend-only ar fi cod mort).

## 20.08.2026 — Perimetrul unei firme se DECLARA in registru si se gardeaza (ISTORIC_TENANTI + F1)
**(bloc 006 + core/test_perimetru_firma_declarat.py).** Intrebare de metoda: pe ce fatete se inchide rândul unei firme si merita parcurse toate pe 006? Verificat la sursa: schema `tenant_006` are 47 de tabele si date in TREI (`firma_profil` 1, `d301_operatiuni` 3, `plan_conturi` 185 = nomenclatorul OMFP); nici `facturi` n-are date — linia D390 vine exclusiv din derivarea D301. Firma nu e incompleta din neglijenta: e PURTATOAREA regimului „neplatitor micro cu achizitii intracomunitare". Doua precizari care schimbau raspunsul: (a) in `ISTORIC_TENANTI.md` rândul NU se inchide — antetul spune „un rand per (tura x tenant), fiecare tura ADAUGA un rand"; ce se inchide e PERIMETRUL, scris in coloana „Ramas"; criteriul agreat e pe functionalitate x fatetele APLICABILE, nu firma x toate fatetele. (b) Nu se parcurg fatetele pe modulele goale, dar firma NU se exclude: F3 e definita pe date POPULATE si F7 compara cifrele cu faptele, deci pe tabele goale dau verde fiindca n-au ce contrazice = falsul sentiment de acoperire interzis in GARZI.md; iar excluderea ar lasa regimul IC netestat (006 e singura purtatoare), pe cand umplerea cu date straine de regim — solutia corecta la t005/t011/t012, SUB-EXERCITATE — l-ar dilua. Marcarea corecta nu e niciuna din cele doua propuse („rand inchis pe fateta IC" = eroare de categorie, IC nu e fateta ci perimetru; „firma exclusa din matrice" = pierzi regimul), ci a treia: **declaratia de perimetru pe antetul sectiunii firmei**, cu doua laturi (IN perimetru = proza; IN AFARA = lista de tabele, citabila mecanic). Scrisa pe 006 (35 de module numarate la sursa; scriptul REFUZA sa scrie daca vreunul are date) + gardata de `core/test_perimetru_firma_declarat.py` (4 teste): blocul nu poate LIPSI dintr-o sectiune de firma, iar tabelele declarate trebuie sa EXISTE si sa ramana GOALE — daca firma se umple, declaratia devine rosie in aceeasi zi. Introdus **fara baseline**, deliberat: registrul avea o SINGURA sectiune de firma (001/002/003 = doar mentiuni in proza; subsolul spune „se completeaza pe masura ce fiecare tenant e atins"), deci poarta e verde din prima si gardul musca la prima tura care creeaza sectiunea urmatoarei firme — un baseline gol ar fi fost greutate moarta, unul ne-gol o lista de amnistie. RED-probat cu trei mutatii (linie stearsa / `plan_conturi` 185 randuri declarat gol / tabel inexistent), toate restaurate exact; al patrulea test e anti-vacuu. Limita declarata: apara sectiunile care EXISTA; „nescris deloc" ramane pe §11. MODEL_AUDIT_TENANT F1 numeste gardul in linia Acoperire (§5 neschimbat: {F3,F4,F5,F7}).
