...
import { esc } from "./api.js?v=1dccbc985b";  // esc canonic (cap.10): strip-html data-lossy inlocuit
  ...
  function randeazaDesktop() {
    ...
    if (areBaraJos) {
      ...
      if (u.rol === "superadmin") {
        ...
        import("./api.js?v=1dccbc985b").then(({ api }) => api.get("/admin/activitate/cabinete")).then((r) => {
    ...
    if (u.rol === "angajat") {
      ...
      import("./api.js?v=1dccbc985b").then(({ api }) => api.get("/eu/calitate")).then((cal) => {
...
async function _clopotActualizeazaBadge(container) {  // [p66_badge_ref]
  ...
  try {
    const { api } = await import("./api.js?v=1dccbc985b");
      ...
      items.forEach((n) => {
        ...
        it.addEventListener("click", async () => {
          ...
          // Format link: "control-fiscal:{tid}". tid malformat -> log + fallback (ramai pe ecran, nu ecran alb).
            ...
            try {
              const { randeazaControl } = await import("./ecrane/control.js?v=d09ca9ab5a");  // ?v=1 aliniat cu cabinet/asistent — fara versiune ar instantia o a doua copie a modulului
