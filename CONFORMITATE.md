...
- **DOUĂ INSTANȚE NOI, cu răspunsul măsurat — 27.08.2026.** Costin le-a găsit citind sloturile T36 și a cerut verificarea: *„două rute ating tabele neașteptate […] pot fi artefacte ale instrumentului, dar merită verificate."* **Amândouă SUNT artefacte**, verificat pe rută (instrucțiunile SQL ale funcției, ca noduri de AST, nu prin citire):
  - `POST /tenants/{id}/activare` → *„`tenants` (DELETE)"*. `comuta_activ` execută **două** instrucțiuni: `UPDATE public.tenants SET activ` și `INSERT INTO public.audit_log`. **Niciun DELETE.** `DELETE`-ul e al lui `tenant_stergere.sterge`, alt membru al aceluiași modul.
  - `PUT /tenants/{id}` → *„`user_tenants` (INSERT)"*. `actualizeaza_tenant` execută **trei** instrucțiuni, toate pe `public.tenants`. `INSERT`-ul în `user_tenants` e al lui `provision_tenant`, alt membru al lui `tenant_provisioning`.
- **de ce merită păstrate**: sunt **două cazuri cu răspuns cunoscut**, exact ce-i lipsește unui instrument per-rută la calibrare. Când R53 se construiește, el trebuie să spună *nu* pe amândouă — iar dacă spune *da*, n-a măsurat ruta, a măsurat tot modulul.
- **și partea care nu se repară cu o etichetă**: eticheta *„PLAFON, nemăsurat pe rută"* e onestă, dar **omul tot trebuie să verifice de mână** — Costin a trebuit s-o ceară, iar eu a trebuit s-o măsor. Un plafon declarat mută costul, nu-l scoate.
...
- **ECRANUL SPUNE ACUM CARE ACT E REVERSIBIL — 27.08.2026**, după observația lui Costin de la sloturile T36: *„trei din cele patru rute sunt ireversibile în feluri diferite, iar ecranul nu spune care e care. Doar dezactivarea are drum de întoarcere, și e singura care nu pare periculoasă."* Măsurat pe ecran, erau **două** lipsuri, nu una:
  - pe ramura în care firma **se poate** șterge, **alternativa reversibilă nu era oferită deloc** — singurul buton de acolo era ștergerea, deci un cabinet care voia doar s-o scoată din listă era împins spre actul ireversibil;
  - niciunul dintre acte nu-și spunea **felul**.
  Acum: refuzul se deschide cu *„Act ireversibil"*, dezactivarea se numește *„(reversibil)"* pe amândouă ramurile, iar nota de sub buton spune de la început că sunt **două acte diferite în același loc**. Verificat pe ecranul randat, desktop și Pixel 5: **axe 0 încălcări**, fără revărsare, fără țintă sub 24px.
...
- **reluări**: 0
- **stare**: **DESCHISĂ**
- **unde a ajuns**: clasa e **MĂSURATĂ** din 30.08.2026; ce rămâne e decizia pe fiecare instanță
- **deschisă pe commit**: `89f11f0`
- **măsurat la**: 2026-08-30 (a doua oară, după reparațiile listei 5) · **pe commit**: `4b6d664`
- **REMĂSURAT DUPĂ LISTA 5 (30.08.2026): clasa a scăzut de la 54 la 41 de câmpuri, de pe 16 rute pe 15.** Scăderea e **13**, și se descompune exact: **2** (registrul-jurnal) + **12** (statul de plată) − **1** (`retinut_tichete`, rămas tăcut *cu motiv scris*). **Partea care contează e ce NU s-a mișcat:** clasa propriu-zisă — cea de pe **ecrane interne**, în afara artefactelor fazei 1 — e tot **32 de câmpuri pe 13 rute**, iar cele **8** ale previzualizării de scoatere sunt tot acolo, sub `R99`. Adică 41 = 8 (R99) + 1 (declarat) + 32 (clasa). *Reparația a golit exact felia pe care decizia de ordine o numise „adâncimea listei 5", și nimic altceva — ceea ce e o confirmare a despărțirii, nu un efect colateral.*
- **AL PATRULEA COȘ, adăugat azi: „randat prin COMPOZIȚIE PE SERVER" — 25 de câmpuri.** Când ruta trimite o listă **deja compusă** (fiecare rând cu eticheta și suma lui) iar ecranul o parcurge generic, câmpul ajunge la om **fără ca ecranul să-l numească** — deci o căutare pe NUME nu-l vede. E același tipar cu `randat_generic`, doar că parcurgerea s-a mutat pe server. **Nu e o scutire pe cuvânt:** numele vin din chiar constanta codului care compune (`stat_plata_api.CAMPURI_COMPUSE`), iar `core/test_compozitie_fluturas.py` dovedește **prin mutație, câmp cu câmp**, că fiecare schimbă ce vede omul — plus proba inversă, pe câmpuri care n-au voie să schimbe nimic. Fără coșul ăsta instrumentul ar fi raportat drept tăcute exact câmpurile pe care reparația le-a dus pe ecran — adică ar fi greșit **în direcția opusă celei declarate**, a doua oară după `randat_generic`.
- **CALIBRAREA S-A MUTAT DE PE STAREA VIE PE UN CAZ SINTETIC, și motivul e o clasă (METODA §29).** Forma dinainte cerea, cu `assert`, ca cele **șase instanțe fondatoare** să fie GĂSITE ca tăcute — adică ancora pozitivă erau chiar defectele de reparat. **A picat efectiv**, la randarea coloanelor registrului-jurnal: instrumentul a devenit inutilizabil fix în tura în care începea munca pe care el o ordonase, și nu pe un defect al lui, ci pe un succes. *O calibrare pozitivă ancorată pe instanțele care urmează să fie reparate se autodistruge la prima reparație.* Acum calibrarea e pe un caz construit, care acoperă toate patru coșurile în amândouă direcțiile; **instanțele fondatoare au rămas ca RAPORT**, nu ca aserțiune — instrumentul tipărește pentru fiecare dacă mai e tăcută sau a fost reparată, și azi toate șase scriu `randat — REPARAT`. Fără raportul ăla, mutarea pe sintetic ar fi șters singura urmă că cele șase au existat.
- **ce blochează**: **CLASA ARE CIFRĂ, și nu e caz izolat.** Instrument: `scripts/scan_r97_livrat_tacut.py`, care ia fiecare `api.get(...)` din `static/js/` — **perechea o declară ecranul, nu o ghicesc eu** —, cheamă ruta **VIU** și confruntă cheile răspunsului, la toate adâncimile, cu ce apare în JS. **Domeniu:** 175 de apeluri, 129 de perechi distincte chemate, **106 măsurate** (restul: 36 cu interpolare nesubstituibilă, 23 refuzate — 19 rute de administrator, 2×404, 1×422, 1×405). **REZULTATUL, pe trei praguri:** **TĂCUT SIGUR — numele nu apare NICĂIERI în `static/js` — 54 de câmpuri, pe 16 rute**; **randat prin container** (părintele e parcurs cu `Object.keys/entries`, deci copiii ajung pe ecran fără să fie numiți) — **37 de câmpuri, scoase din clasă**; **candidați** (apar altundeva, dar nu în ecranul care le cere) — 267 de câmpuri, pe 41 de rute. **90 din cele 106 rute măsurate n-au niciun câmp tăcut sigur** — deci clasa e reală și **concentrată**, nu difuză. **Din cele 16 rute, una — previzualizarea scoaterii unei firme, 8 câmpuri — a ieșit din inventarul ăsta și are număr propriu, `R99`**, prin decizia lui Costin din 30.08: *„îngropată printre 66 de câmpuri se pierde."* **Rămân deci 46 de câmpuri pe 15 rute în clasa asta.** **Cele mai mari instanțe:** `stat-plata` **12** (mai mult decât se știa — pe lângă cele 7 componente ale netului tac și `cm_brut`/`cm_net`/`cm_zile`, adică **concediul medical**, plus `valoare_tichete`/`retinut_tichete`) · `control-fiscal/{id}` **10 din 216** (printre ele `regim_tva_anaf` și `facturi_necontabilizate`) · `GET /tenants/{id}` **5 din 13** (`nume_ales`, `balanta_importata_la`, `plan_importat_la`, `salariati_importati_la`) · registrul-jurnal **2 sigure** (`note[].nr_curent`, `note_fara_document`) plus 2 candidate (`total_debit`, `total_credit`, care apar în alt fișier JS, nu în cel al jurnalului).
- **CIFRA S-A CORECTAT ÎN JOS ÎN ACEEAȘI ZI, și direcția erorii era inversul celei declarate**: prima formă a instrumentului a raportat **66 de câmpuri pe 17 rute**. Instrumentul își declarase direcția — *„«randat» se caută pe nume, deci e supra-numărat, deci clasa e plafon inferior"* —, iar un tipar o **întorcea**: un ecran care parcurge un container **generic** (`Object.keys(rd)`, `Object.entries(marcaje)`) afișează cheile **fără să le numească**, deci căutarea pe nume le dădea drept tăcute. Verificat în cod, nu dedus: `randuri_de_sters.*` de pe ecranul de scoatere și cele **9** `marcaje.*` de pe ecranul de contracte **se afișează**. *O cifră care greșește în direcția opusă celei declarate e mai rea decât una imprecisă: cine o citește o corectează mental în partea greșită.* Reparat prin `randat_generic()`, care rezolvă și **un** nivel de alias; **ce nu rezolvă, declarat**: alias pe două niveluri, parcurgere printr-o funcție ajutătoare, sau parcurgere în alt fișier — acolo rămâne supra-numărare.
- **CUM SE DESCOMPUNE, după decizia de ordine din 30.08** (`DECIZII.md` 12): *„R97 pe artefacte nu e o listă separată, e adâncimea listei 5."* Deci cele **54 de câmpuri pe 16 rute** se despart, și fiecare parte merge unde îi e locul: **8 câmpuri, 1 rută** → **R99** (previzualizarea scoaterii, scoasă la suprafață cu număr propriu) · **14 câmpuri, 2 rute** → **adâncimea listei 5**, fiindcă alea sunt singurele care poartă artefacte din lista lui 1a (statul de plată **12**, registrul-jurnal **2**) · **rămân 32 de câmpuri, pe 13 rute — clasa R97 propriu-zisă, toată pe ECRANE INTERNE**, care nu intră în niciun criteriu al fazei 1.
- **ce NU crește din asta**: **lista 5 nu capătă poziții noi.** Cele două artefacte atinse de clasă erau **deja** în ea; ce s-a schimbat e **adâncimea** uneia — netul avea 7 câmpuri tăcute cunoscute, are **12**, printre cele noi `cm_brut`/`cm_net`/`cm_zile`, adică **concediul medical**, și tichetele. *Marginea despre care lista 5 spunea „nu se poate ști" e măsurată, și e zero poziții.*
- **calibrare**: în amândouă direcțiile, ca **aserțiune** — instrumentul pică dacă nu trece. **Pozitivă:** cele șase câmpuri ale instanțelor cunoscute (`nr_curent`, `total_debit`, `total_credit`, `deducere_tineri`, `deducere_copii`, `cas_suprataxa`) trebuie **găsite** — toate șase găsite. **Negativă:** trei câmpuri care **se randează** (`descriere`, `sold_final`, `operatiuni`) n-au voie să apară — toate trei corect absente. Plus anti-vacuu pe domeniu (>100 de apeluri, >20 de fișiere JS).
- **ce a scris sonda**: **doar `public.audit_log`, +128.** `tenants`, `users`, `declaratii_coada`, `inregistrari`, `inregistrari_linii`, `facturi`, `salariati`, `declaratii_depuse`: **neatinse**, prin snapshot înainte/după. *Se măsoară fiindcă „un GET nu are voie să scrie" nu e „un GET nu scrie" — iar printre rutele chemate e `/tenants/{id}/scoatere`.*
- **condiția de deblocare**: prima jumătate e **făcută** — clasa are cifră pe domeniu declarat. **Ce rămâne:** fiecare instanță ori **reparată**, ori **declarată cu motiv scris**, iar cifra de 66 nu se poate întoarce în sus fără să fie observată. Se închide când `scripts/scan_r97_livrat_tacut.py` raportează **0 câmpuri tăcute sigur** pe rutele care poartă artefacte, iar restul au motiv scris; și când numărul e ținut de un **clichet**, ca să nu crească tăcut la următorul câmp adăugat într-un răspuns. *Ordinea rămâne cea scrisă la deschidere: întâi măsurătoarea — acum e —, apoi reparațiile.*
...
### R99 — Previzualizarea scoaterii unei firme arată ce s-a GĂSIT, dar nu ce s-a VERIFICAT

- **felul**: VERIFICARE
- **cine deblochează**: INTERN
- **unde intră**: E1 · scoasă din inventarul clasei **R97** prin decizia lui Costin, 30.08.2026 · **PRAG 3**
- **reluări**: 0
- **stare**: **DESCHISĂ**
- **deschisă pe commit**: `7dbda7c`
- **măsurat la**: 2026-08-30 · **pe commit**: `7dbda7c`
- **de ce are număr propriu**: decizia lui Costin — *„nu e artefact și n-are loc într-o listă de artefacte, dar e previzualizarea celui mai distructiv act din aplicație. Îngropată printre 66 de câmpuri se pierde."* Deci iese din inventarul lui R97 și se numără separat, chiar dacă e aceeași clasă mecanică.
...
- **condiția de deblocare**: `_d112_genereaza` întoarce, pe lângă XML, un obiect de rezultat cu pozițiile pe care le scrie (asigurații și contribuțiile lor), iar D112 intră în `COMPONENTE` ca oricare alta. Se închide când `test_toate_cele_noua_sunt_acoperite_ori_motivate` trece cu `FARA_COMPONENTE` **gol**, iar componentele D112 se văd pe ecran pe o firmă reală.
...
**Decizie cerută [atunci].** **TVA la încasare pe factura primită** — punctul 2 din blocul AAA. **A PRIMIT RĂSPUNS pe 29.08.2026: varianta (ii)**, și e construită — vezi BLOC FFF mai jos. Textul de mai jos se păstrează fiindcă e măsurătoarea pe care s-a luat decizia.
...
#### BLOC FFF — factura PRIMITĂ, construită (29.08.2026, PROBĂ)

**FFF1 — nota se scrie la `/valideaza`, în același act**, după ce clasificarea e pusă pe factură, ca
nota s-o poată citi. **Contul de cheltuială e OBLIGATORIU.** *Schimbare de comportament, declarată:*
până azi câmpul era `cont or None`, deci validarea trecea fără el; de azi **refuză**, cu `422` și cu
motivul ca **afirmație tipată** (`cod=CONT_CHELTUIALA_OBLIGATORIU`), nu ca propoziție. Un refuz al
notei **nu anulează** validarea: recunoașterea cheltuielii e actul omului, iar contarea e consecința ei.

**FFF2 — clasa ambiguă e refuzată automat, și numai ea.** Furnizor la încasare **și** firmă în regim
normal → automatul refuză, cu `cont_tva=4428`, `temei=Cod fiscal art. 297 alin. 2` și
`iesire=contabilizare_manuala`, toate **ca date**. Restul claselor se contează automat — calibrat în
direcția cealaltă: o primită normală rămâne pe **4426**.

**Reparația care vine odată cu ea, și e o cifră greșită închisă:** generatorul citea
`firma_profil.tva_la_incasare` — regimul **propriu** — și îl aplica pe amândouă direcțiile, fără să
citească vreodată `facturi.furnizor_tva_incasare`, deși coloana există, ecranul o scrie la
`/valideaza` și **D300 rutează pe ea**. De azi, pe primită contează **și** regimul furnizorului.

**FFF3 — aceeași idempotență și a-doua-cale ca la EEE3**, din același loc.

**FFF4 — probă pe date reale**, pe chiar cele **2** facturi ale clasei (`scripts/proba_contare_reala.py`,
în tranzacție cu ROLLBACK):

| factura | furnizor la încasare | firmă la încasare | automat | manual |
|---|---|---|---|---|
| `tenant_004` #9 | DA | NU | **refuzat** `TVA_LA_INCASARE_MANUAL` | **contată**, conturi `371 · 401 · 4428` |
| `tenant_017` #8 | DA | NU | **refuzat** `TVA_LA_INCASARE_MANUAL` | **contată**, conturi `371 · 401 · 4428` |

*Exact ce cerea varianta (ii): automatul nu începe un lanț pe care nu-l poate termina, omul îl poate
duce — iar când îl duce, cifra coincide cu ce declară D300.*

**MĂSURĂTOAREA PE DIRECȚII, care era câmpul „ce NU vede măsurătoarea"**: din 28 de facturi
declarabile fără notă, **10 primite** (48.200,00 lei TVA) și **18 emise** (54.060,00 lei). Se reface
la fiecare rulare a sondei.

- **CONDIȚIA, DESPĂRȚITĂ — se scrie, fiindcă altfel închiderea ar arăta mai largă decât e.** Condiția
  avea două jumătăți: *(a)* validarea produce nota în același act — **ÎNDEPLINITĂ**, construită și
  probată; *(b)* nu mai există factură primită validată fără notă — **NEÎNDEPLINITĂ**, și nici nu
  putea fi în tura asta: stocul istoric e obiectul lui **R89**, iar decizia din 29.08 e că **nu se
  atinge**. Restanța se închide pe (a); (b) se poartă mai departe, la R89, cu numele ei.
...
#### BLOC DDD — cheia nu mai numără orb (29.08.2026, PROBĂ)

**Ce s-a construit, și unde stă.** Contarea unei facturi are de azi **un singur loc**:
`core/contare_facturi.py`. Ruta manuală nu mai are SQL propriu — gardul o verifică pe **AST**: zero
apeluri `execute` în corpul ei. Cele trei acte care produc note (emiterea, validarea unei primite,
ruta manuală) cheamă **aceeași** funcție, cu un singur steag de diferență: `automat`.

**DDD1 — `contare_existenta` în locul lui `COUNT(*)`.** Întrebarea nu mai e *„există o notă?"*, ci
*„există o notă **de contare**?"*. Semnătura, ca definiție mecanică: un cont de **terț**
(4111 · 401 · 404) **și** un cont de **fond** sau de **TVA**. O notă de plată (`401 = 5121`) sau de
încasare (`5311 = 4111`) atinge terțul și trezoreria — deci nu e contare. O notă de casă
(`5311 = 707`) atinge fondul, dar nu terțul.

**PROBAT PE DATELE REALE, pe chiar cele 3 facturi măsurate la BBB** (`scripts/proba_contare_reala.py`,
rulează pe portofoliul viu și face **ROLLBACK**; numărările de rânduri se întorc identice și se
tipăresc):

| factura | nota care ocupa cheia | `contare_existenta` | contabilizarea |
|---|---|---|---|
| `tenant_004` #9 | #1 `banca` — `401=5121` | **None** | **trece** → `371=401` 2.000,00 · `4428=401` 420,00 |
| `tenant_013` #14 | #29 `casa` — `5311=4111` | **None** | **trece** → `4111=707` 300,00 · `4111=4427` 63,00 |
| `tenant_017` #8 | #1 `banca` — `401=5121` | **None** | **trece** → `371=401` 2.000,00 · `4428=401` 420,00 |

*Toate trei erau refuzate cu „factura are deja înregistrare" — un mesaj fals. Fals-pozitivul e închis.*

**DDD2 — plasa, pe criteriul STRICT.** Înainte de a scrie o notă **automată**, se caută și note
**fără** `factura_id`, în luna facturii, cu valoarea pe terț egală cu totalul, la ban. Găsită →
automatul **nu scrie**, semnalează. **De ce strict și nu larg**: criteriul larg (baza, sau suma
tuturor liniilor) a produs **3 fals-pozitive** pe datele de ieri — două note de încasare care se
potrivesc cu factura pe care chiar o încasează, și o amortizare care nimerește un total. *Un automat
oprit pe încasări n-ar mai fi o plasă, ar fi un obstacol.* Pe ruta manuală, plasa **avertizează** în
loc să oprească: omul are ce n-are automatul — contextul.

**DDD3 — reparația la SURSĂ, și e mai importantă decât plasa.** O notă scrisă din `POST /jurnal` care
contează **evident** o factură primește `factura_id` **la scriere**. „Evident" e îngust deliberat:
semnătură de contare · exact **una** dintre facturile declarabile ale lunii se potrivește pe total ·
factura aia n-are deja contare. **Două potriviri → nu se leagă niciuna** — *o legătură greșită e mai
rea decât lipsa ei: ar face o factură să pară contată de altcineva.* Contractul e scris în
`PLAN_ARHITECTURA.md`, fiindcă schimbă ce întoarce ruta.

**DDD4 — ce ÎNCHIDE și ce NU.** *Se spune, fiindcă întrebarea a fost pusă explicit.*
- **Fals-pozitivul e închis complet.** O notă care nu e contare nu mai blochează contarea, pe nicio
  cale. Probat pe cele 3 instanțe reale, cu calibrare în ambele direcții și RED-proof.
- **Fals-negativul e MICȘORAT, nu închis.** Rămân: notele scrise **înainte** de azi, care n-au cheie
  și nu se leagă retroactiv · cazul cu două potriviri, unde legarea refuză deliberat · o contare
  făcută pe `461`/`462`, pe care semnătura n-o vede.
- **Și a apărut o consecință care n-a existat înainte**: refuzul ștergerii numește o ieșire —
  „dezleagă nota" — pe care aplicația **n-o are**. → **R90**, deschisă.

#### BLOC EEE — factura EMISĂ (29.08.2026, PROBĂ)

**EEE1 — nota se scrie în `creeaza_factura`, nu în rută.** Citit înainte de scris, nu presupus:
`emite_factura` și `storneaza` cheamă chiar funcția asta, deci punctul unic acoperă **toate cele
patru** drumuri de emitere — `POST /facturi`, `POST /facturi/emite`, stornarea, transformarea
proformei. *Legat de o singură rută, regula R36 ar fi fost adevărată pe un drum din patru.*

Tiparul NIR, punct cu punct, probat: validarea se termină **înainte** de prima scriere · **o
tranzacție**, fără commit propriu · nota intră **`ciorna`** (patru-ochi neatins, R47) · `sursa` numește
**actul** (`facturi`) · referință inversă prin `factura_id`. **Rollback complet**: o eroare
nedeclarată în generarea notei anulează și factura — gardat cu o mutație. **Iar un refuz DECLARAT
oprește nota, nu emiterea**: factura se creează, motivul pleacă în răspuns ca afirmație tipată.
*Altfel o factură validă n-ar mai putea fi emisă dintr-un motiv de contabilitate.*

**EEE2 — ștergerea refuză motivat**, cu ieșirea numită **ca dată** (`iesire: storno`), nu doar în
propoziție. **EEE3 — a doua chemare a rutei manuale e NO-OP**: `200` cu `stare='deja_contata'`, nu
`422`. *Refuzul era corect, prezentarea lui nu — un comportament corect care arată ca o eroare învață
pe cineva să se ferească de el.*

**EEE4 — probă pe date reale** (`tenant_017`, în tranzacție cu ROLLBACK): factura emisă nouă produce
nota **automat**, `status=ciorna`, `sursa=facturi`, `factura_id` legat, liniile `4111=704` 1.000,00 și
`4111=4427` 210,00 · a doua contare → `deja_contata` · ștergerea → refuzată, cu ieșirea `storno`.

**CE NU ACOPERĂ, DECLARAT** (ZZ4 cere ca absența să fie **scrisă**, nu dedusă): facturile intrate prin
`main._factura_din_parsat` — importul din SPV și încărcarea manuală de XML — **nu** trec pe aici, ci
printr-un `INSERT` propriu. O factură **emisă** care se întoarce din SPV a fost emisă în altă parte;
pentru ea, sosirea documentului nu e faptul economic. Rămâne necontată și **vizibilă ca atare** în
verdictul de TVA (R35).
...
- **ce blochează**: nota propusă de `salarii_contare.note_lunare` nu reproduce, pe cele două poziții pe care le calculează **singură**, ce declară D112 pentru aceeași lună. Două cauze distincte, mai jos — una e o cheltuială care lipsește cu totul, cealaltă o cifră care diferă.
- **CAUZA A — cheltuiala cu biletele de valoare nu se înregistrează.** `642 = 5328` e **0,00** pe toate cele 5 luni, iar declarația poartă **800 · 800 · 25.140 · 920 · 840**. Motivul, citit în cod: `note_lunare` cheamă `calcul_salariu` **fără** argumentele de tichete (`tichet_valoare`, `tichet_zile`, `tichet_vacanta`, `tichet_cultural`, `tichet_cresa`) — alea se pasează doar înăuntrul lui `d112.pull`. Deci `tichete_nominal` iese 0, iar `monografie_salariu` nu mai produce linia. **Nu e o cifră greșită, e o cheltuială reală care lipsește din notă.**
- **CAUZA B — salariile brute realizate diferă de cele declarate**, pe aceleași 5 luni: **−300,00 · −300,00 · +5.400,00 · −199,04 · −200,24** (nota vs declarat). **Semnul alternează**, deci nu e rotunjire; iar luna cu abaterea mare (iunie) e chiar cea cu cel mai mult concediu medical și cu tichete de 25.140. Cauza exactă nu e stabilită — și nu se stabilește azi.
- **de ce NU s-a reparat în aceeași tură**: cerut explicit (QQ3: *„dacă apar, nu le repara acum, doar le documentează ca restanță nouă"*). Și e o alegere bună independent de cerere: cauza A e o listă de argumente lipsă, dar cauza B cere să se știe **care** dintre cele două cifre e cea corectă contabil — iar aia se măsoară, nu se ghicește.
- **ce NU vede măsurătoarea**: firmele care n-au nici tichete, nici CM, nu pot arăta niciuna dintre cele două cauze. Din 30 de perechi măsurabile, **25** sunt de felul ăsta — deci absența divergențelor acolo **nu e o probă** că restul aplicației e în regulă, e o probă că datele nu ating cazul.
- **condiția de deblocare**: `note_lunare` pasează tichetele către `calcul_salariu` (cauza A), și se stabilește — cu măsurătoare, nu prin alegere — dacă 641 trebuie să poarte salariul **realizat** sau altceva (cauza B). Se închide când `scripts/sonda_r34.py` întoarce **0** divergențe cu `fel = "verificare"` pe cele 30 de perechi măsurabile, iar cele 5 perechi de pe `tenant_001` rămân în eșantion.
- **[BLOC RR, 28.08.2026] CAUZA A — ÎNCHISĂ. Cheltuiala cu biletele de valoare intră acum în notă.**
  - **RR1, reparat unde era defectul, nu unde se vedea**: `pull()` punea înapoi în salariat doar **ieșirile** lui `calcul_salariu` (`cas`, `cass`, `impozit`, `tichete_nominal`, `e83_*`), niciodată **intrările** de tichete — zilele cu tichet (din pontaj), excesul peste plafonul anual de vacanță, cadoul taxabil. Cine chema `calcul_salariu` a doua oară pe același salariat n-avea de unde să le ia. Acum le pune înapoi, iar `note_lunare` le **pasează**, nu le recalculează — o a doua socoteală ar fi fost chiar clasa R34.
  - **e a treia instanță a aceleiași clase cu R85**: ce nu se pune înapoi în dicționar nu se poate reciti, iar **absența unei chei e indistinctă de valoarea zero**. La R85 e `cam`; aici erau zilele cu tichet. Amândouă au produs tăcere, nu eroare.
  - **RR2 — măsurat pe `tenant_001`, cele 5 luni**: `642 = 5328` era **0,00** pe toate cinci; acum e **800 · 800 · 25.140 · 920 · 840**, exact valorile declarate. **Zero divergențe rămase pe poziția asta.**
  - **și a reparat, din mers, cea mai mare parte din cauza B**: pe iunie, `641/421` sărise cu **+5.400** — excesul de tichete de vacanță peste plafonul anual, care intră în brutul impozabil și pe care nota nu-l vedea. După RR1, iunie intră în rând cu celelalte luni: abaterea ei devine **−300,00**, aceeași ca peste tot. **Divergențele totale pe cele două poziții: 10 → 5.**
- **[BLOC RR3, 28.08.2026] CAUZA B — DIAGNOSTICATĂ, NEREPARATĂ. Și diagnosticul spune că defectul NU e în notă.**
  - **măsurat salariat cu salariat**, pe `tenant_001`, 5 luni, 12 salariați: diverge **exact unul** — cel de la **salariul minim** — și diverge **exact cu facilitatea**: **300,00** în aprilie–iunie (minim 4.050, nivel de referință 3.750) și **200,00** în iulie–august (4.325 / 4.125). Tot restul e rotunjire **sub-leu** (±0,14 · ±0,48 · ±0,24), din rotunjirea D112 la leu **per salariat**: de-aia totalul lunii iese 199,04 și 200,24, nu 200 rotund.
  - **cauza, structural: cele două cifre nu sunt aceeași mărime.** `calc["brut"]` e salariul brut **realizat, întreg** — ce datorează angajatorul, deci ce intră pe 641. `B_brutSalarii` (= `C1_11` = suma `B2_5`) e **baza contributivă**, din care facilitatea de la salariul minim e **scăzută** (OUG 89/2025 art.III: nivelul de referință se diminuează cu 300 lei).
  - **și mai departe: D112 nu declară nicăieri „brutul realizat" ca atare.** Are trei mărimi apropiate, și **niciuna** nu înseamnă ce înseamnă 641: `B2_5` (baza contributivă, fără facilitate), `B4_3` (brutul **contractual** — pe o lună cu concediu medical diferă prin construcție; măsurat: ar fi produs 7 perechi „divergente" din 30, dintre care 2 fals pozitive curate) și `E1_1` (venitul brut **total**, care include tichetele). **Deci ce lipsește nu e o cifră corectă în notă — e o contrapartidă care nu există în declarație.**
  - **consecința pe instrument, spusă fiindcă e a mea**: ancora aleasă la QQ2 pentru `641/421` are o **abatere cunoscută**, egală cu facilitatea celor de la salariul minim. Gardul nu e curat și **nu s-a făcut să pară curat** — nu s-a lărgit toleranța ca să înghită 300 de lei. *O toleranță care ascunde exact singura clasă pe care instrumentul o mai poate găsi nu e o toleranță, e o oprire.*
...
- **ISTORIA lui, și de ce nu e o presupunere**: e **bugul nr.1 din antetul lui `core/salarii_contare.py`**, dovedit pe 15.07.2026 pe versiunea veche din `/opt/iconta`: *„`_sum("cam") = 0` — `pull()` nu setează «cam». Filtrul `if s > 0` sărea linia 646=436 → **CAM NU se contabiliza NICIODATĂ**."* Reconstrucția modulului a **ocolit** capcana (a chemat `calcul_salariu`, care produce `cam`); n-a **desființat-o**.
- **MĂSURAT azi, și măsurătoarea contrazice o parte din felul în care mi-a fost descrisă**: comanda spunea că descoperirea *„îl reconfirmă activ, doar ocolit"*. **Activ nu e.** Cei **3** cititori de `cam` din tot codul — `salarizare.monografie_salariu`, `stat_plata_api`, un test — îl iau toți din rezultatul lui `calcul_salariu`, nu de la `pull`. Deci azi **nimeni nu-l pierde**. Ce e adevărat e cealaltă jumătate: capcana **există**, și ea a fost cea care a forțat ocolul de la R34 — sursa celor patru poziții e artefactul D112, nu dicționarul lui `pull`, tocmai fiindcă a patra poziție nu e acolo.
...
- **ce blochează**: cum s-a găsit — Costin, 27.08, la 19:16:24, a apăsat *„Păstrez denumirea mea"*. Scrierea s-a făcut și e probată. **Ecranul n-a spus nimic.** Reușita s-a dedus din dispariția casetei, care dispare fiindcă `nav.acasa()` demontează tot ecranul — nu fiindcă actul ar fi confirmat ceva. *„A dispărut" și „a reușit" arată identic; la fel și „a dispărut" cu „s-a rupt ceva și ecranul s-a resetat".*
- **cifra**: în `static/js/ecrane/firme.js`, **4 din 6** acte de nivel firmă se termină fără nicio confirmare vizibilă — și sunt exact cele cu efectul cel mai mare: **alegerea de denumire** (l. 312), **scoaterea definitivă** (l. 2465), și **cele două dezactivări** (l. 2450 și l. 2493). Toate patru au aceeași formă: `api.post`/`api.del` → `nav.acasa()` → `setFirmaInLucru("")`, fără `arataMesaj`. Celelalte două **confirmă**: crearea (`_bannerFirmaCreata`, l. 107) și reactivarea (*„«X» e din nou în portofoliu."*, l. 228).
...
- **MĂSURAT DIN NOU 27.08.2026, seara, ÎNAINTE de orice decizie** *(Costin: „măsurătoarea decide dacă varianta (a) mai are temei")* — și măsurătoarea de dimineață era **contaminată**:
  - **cifra „4 din 17" e pe fixturi, nu pe comportament.** Toate patru firmele divergente — `ALFA MICRO` (8396), `BETA PROFIT` (8397), `GAMA DEFECT-MIGRARE` (8398), `DELTA DEFECT-LUNA` (8399) — sunt la cabinetul **4163, „CABINET TEST FIR INTRARE SRL"**, mediul de test izolat creat deliberat pe 09.08.2026 (`DECIZII.md`). **Excluzând cabinetele de test: 0 divergențe din 14 firme reale.** Portofoliul are azi 18 firme: 14 la cabinetul 1968, 4 la 4163.
  - **de unde vine divergența, în cod, nu în presupunere**: cele patru s-au adăugat **manual prin ecran** (CUI-uri de test, respinse de fluxul ANAF — vezi `DECIZII.md` 09.08), deci `tenants.nume` e ce a **tastat** omul: „ALFA MICRO". Denumirea fiscală a fost scrisă de semănător: `~/date_test_cabinet/genereaza.py` l. 239, `_seed_profil("ALFA MICRO SRL", …)` → `INSERT INTO firma_profil … ON CONFLICT (id) DO UPDATE SET nume=EXCLUDED.nume`. **Semănătorul scrie forma juridică; omul n-a scris-o.** Divergența e artefact de fixtură, nu comportament de utilizator.
  - **și semănătorul din repo dovedește contrariul**: `date_test/seed/transa2_coerenta_tva.py` (l. 80-105) trece **același** șir prin amândouă locurile — `provision_tenant(conn, nume, …)` și apoi `UPDATE "…".firma_profil SET nume=…`. De aceea cabinetul 1968 are **0** divergențe pe 14 firme.
  - **clichetul pe date la 4 e clichet pe fixturi**: `core/test_nume_firma_unic.py::_perechi_de_denumiri` citește `SELECT id, nume, schema_name FROM public.tenants` — **fără niciun filtru de cabinet**. Recalcularea cere ca populația să fie **declarată**, iar azi nu se poate: `public.accounting_firms` n-are nicio coloană care să spună „cabinet de test". Deci fie se adaugă un marker (ca la fixturile period-keyed), fie se scrie lângă clichet lista explicită de cabinete excluse. **Până atunci cifra 4 nu se poate coborî onest** — nu fiindcă divergența ar fi reală, ci fiindcă instrumentul nu poate spune despre cine vorbește.
- **CREAREA nu produce divergență — capcana oul-și-găina NU există**: `provision_tenant` scrie **același** `nume` în `public.tenants` (`core/tenant_provisioning.py` l. 172) **și** în `{schema}.firma_profil` (l. 190), în aceeași tranzacție; `precompleteaza_din_anaf(seteaza_nume=False)`, chemat imediat după la `POST /tenants` (`main.py` l. 1313), nu atinge niciuna. O firmă creată acum zece secunde ar afișa în listă exact ce s-a tastat, chiar dacă adevărul ar deveni cel fiscal — **lista n-ar fi goală**. Și nici scumpă: `auth_api.tenantii_userului` face **deja** o citire per schemă, cu savepoint, ca să aducă `tip_firma` (`core/auth_api.py` l. 385-395) — denumirea fiscală ar fi **o coloană în plus în aceeași interogare**, zero cereri noi. *(Prima formă a acestui rând spunea că `GET /tenants` face o singură interogare pe `public` și că o listă pe denumire fiscală ar costa o citire în fiecare schemă. **Fals, și e greșeala mea**: bucla per-schemă există de la `tip_firma_v1`. Corectat la a doua citire — iar corectura mută costul variantei (a) din „scump” în „nimic”.)*
- **PROBĂ, nu citire — care denumire pleacă efectiv pe hârtie** (generat pe `tenant_013`, unde cele două diferă, în tranzacție întoarsă la savepoint, fără să scrie nimic):

  | act | rezultat |
  |---|---|
  | D100 trim 3/2026 | `den="ALFA MICRO SRL"` |
  | D205 2026 | `den="ALFA MICRO SRL"` |
  | bilanț S1005 2026 și 2025 | `den="ALFA MICRO SRL"` |
...
- **REDESCHISĂ 27.08.2026, seara** *(Costin: „a fost închisă pe un criteriu mai îngust decât eticheta")*. Eticheta spune **ștergerea unei firme**; închiderea a măsurat **un rând de audit**. Ce n-a fost măsurat atunci, măsurat acum:
  - **`user_tenants` nu păstrează nimic.** Zero rânduri orfane azi: `SELECT * FROM public.user_tenants WHERE tenant_id NOT IN (SELECT id FROM public.tenants)` → **0**. Coloana e `tenant_id integer NOT NULL`, iar constrângerea e `FOREIGN KEY (tenant_id) REFERENCES tenants(id) ON DELETE CASCADE` — **referință prin id stabil**, nu prin numele schemei. În plus, `tenant_stergere.sterge` o golește **explicit**, în bucla peste cele 13 `TABELE_TENANT`, înainte de a atinge rândul firmei. Deci nu e nici reziduu prevăzut, nici orfan: **nu există**. Ce rămâne, și e proiectat să rămână, e rândul din `public.firme_scoase` — urma scoaterii.
  - **numele de schemă SE RECICLEAZĂ — confirmat pe date.** `public.firme_scoase` are trei rânduri, iar **două** poartă `schema_name = 'tenant_019'`: `PROBA PORTAL SRL` (CUI 2816464, scoasă la 13:44:40) și `BORG DESIGN SRL` (CUI 14837428, scoasă la 19:25:04). La fel `tenant_018`: schema unei firme scoase la 13:42:13, și **acum** schema firmei vii `Antibiotice Iasi` (33394, creată la 19:12:00).
  - **de ce se reciclează**: `urmator_schema_name` (`core/tenant_provisioning.py` l. 27-38) ia **max(NNN)+1** peste `SELECT schema_name FROM public.tenants` — adică peste firmele **vii**. Când cea mai mare e ștearsă, maximul coboară și numărul se refolosește. Nu e „prima gaură liberă" și nu e contor: e max+1 pe o populație care se poate micșora.
  - **schema chiar se DROP-uiește.** `core/tenant_stergere.py` l. 301: `DROP SCHEMA IF EXISTS "…" CASCADE`, după cele 13 tabele partajate și înaintea rândului firmei. Proba: azi există **18** scheme, `tenant_001`…`tenant_018`; **`tenant_019` nu mai există**. Deci întrebarea „ce mai e în ea, și de când" nu are obiect: nu mai e nimic.
  - **și că e re-creată, nu refolosită, se vede din OID.** `tenant_018` are `pg_namespace.oid = 203894825`, cu tabelele ei între `203894826` și `203895520` — peste **toate** celelalte scheme (cea mai nouă de dinainte, `tenant_017`, are oid `85624359`, iar maximul de oid pe tabelele tuturor celorlalte e `190688721`). Deci `tenant_018` a fost ștearsă la 13:42 și **creată din nou** la 19:12. `tenant_019` a trecut prin același ciclu de două ori în aceeași zi.
  - **crearea peste o schemă rămasă ar EȘUA, nu ar suprascrie**: `tenant_template.sql` l. 24 e `CREATE SCHEMA TENANT_PLACEHOLDER;` — **fără** `IF NOT EXISTS` —, parametrizat de `parametrizeaza_template`. Dacă `DROP`-ul n-ar fi rulat, provizionarea s-ar opri cu eroare; nu ar scrie peste date reziduale.
  - **jurnalele nu pot confirma nimic, și se spune**: pe instanța de producție `log_statement = none` și `log_min_duration_statement = -1`; nicio linie `CREATE SCHEMA`/`DROP SCHEMA` în `/var/log/postgresql/`. **GRI** pe partea de loguri — proba de mai sus e din `pg_namespace`/`pg_class`, nu din ele.
  - **un singur loc din `public` referă un tenant prin NUMELE schemei**: `firme_scoase.schema_name` (text, fără cheie străină; **2 din 3** rânduri trimit azi la o schemă care nu mai există). Celelalte 14 coloane de legătură sunt `tenant_id integer`. **Acolo istoricul devine ambiguu la reciclare**: pe `schema_name` singur, `tenant_019` trimite la două firme diferite. Rândul rămâne dezambiguizat de `tenant_id`, deci urma nu e pierdută — dar orice citire cheiată pe nume de schemă e greșită.
  - **numărătoarea orfanilor NU e cheiată pe nume de schemă**, deci „69 neschimbat" **nu** poate fi verde fals prin suprapunere: `test_niciun_orfan_NOU_dupa_ultima_stergere` numără `x.tenant_id IS NOT NULL AND NOT EXISTS (SELECT 1 FROM public.tenants p WHERE p.id = x.tenant_id)` peste cele 13 tabele — **id întreg, dintr-o secvență, care nu se reciclează niciodată**.
...
- **CE A RĂMAS DESCHIS DUPĂ ÎNCHIDERE, măsurat 27.08.2026 seara.** Restanța rămâne **REZOLVATĂ pe ce a fost măsurat** — scrierea, probată pe viu la 19:16:24 (`nume_ales='aplicatie'`, `nume_ales_de=1968`, un rând de audit cu amândouă denumirile). Astea sunt trei lucruri pe care închiderea nu le acoperea:
  1. **apăsarea nu confirmă nimic.** `static/js/ecrane/firme.js` l. 310-313: pe succes handlerul face `nav.acasa()` + `nav.setFirmaInLucru("")` și **niciun** `arataMesaj`. Caseta „dispare" fiindcă ecranul e demontat, nu fiindcă actul ar fi confirmat ceva. Pe eroare, în schimb, se adaugă un `msg-eroare` — **deci ecranul vorbește numai când eșuează.** Mutat în **R82**, fiindcă e o clasă, nu un buton.
...
- **DECIS 27.08.2026: varianta (b), supraveghere locală.** Costin: *„(a) cere un serviciu extern care devine el însuși ceva de întreținut. (c) declară acceptată o cădere pe care n-am putut-o explica. (b) e în aceeași formă cu ce merge deja: `cron.RITMURI` supraveghează 11 joburi; al 12-lea e procesul care le servește pe toate."* Cerința lui, textual: **„procesul e viu ȘI răspunde."**
- **CONSTRUIT, jumătatea mea** — `core/sonda_web.py`, al 12-lea nume din `RITMURI`:
  - **cere pagina**, nu întreabă systemd dacă unitatea e `active`: `GET /` local, cod 200 și corp nevid. Un proces `active` care nu mai răspunde e exact felul de cădere pe care *„viu"* n-o vede.
  - **compară ora de pornire a unității** cu cea de la sonda precedentă (memorată în `cron_batai.detalii`, coloană adăugată idempotent). Dacă s-a schimbat, **a repornit între cele două sonde** — și o spune, cu amândouă orele.
  - trece prin `cron.ruleaza`, deci lipsa sondei însăși devine vizibilă.
- **ȘI AFIRMAȚIA MAI SLABĂ, scrisă ca atare**, fiindcă Costin a cerut-o explicit: *„dacă o cădere de trei secunde nu se prinde cu ritmul supravegherii, spune — atunci ce se prinde e repornirea, nu căderea."* **Așa e.** Sonda prinde **repornirea**, nu **durata**: între două rulări la 15 minute, o cădere de trei secunde și una de paisprezece minute arată identic. Se poate spune *„a repornit între X și Y"*, nu *„a fost jos N secunde"*. Iar dacă mașina cade cu totul, nu rulează nici sonda — limita deja scrisă în `cron.verifica_batai`.
- **ȘI O REPARAȚIE DE DINAINTEA PRIMEI ALERTE, 27.08.2026**: sonda ar fi alertat la **fiecare deploy**. `post-commit` repornește serviciul la orice commit, deci ora de pornire se schimbă de zece ori pe zi — iar *„un semnal care se aprinde mereu nu mai e semnal"* e chiar doctrina scrisă în `core/cron.py`. Sonda deosebește acum: repornire cu **alt commit** = **deploy**, nu alertează; cu **același** commit = **cădere**, alertează; dacă un commit nu se poate citi, alertează **spunând** că nu poate deosebi.
...
- **LINIA DE `crontab` AM PUS-O EU, 27.08.2026 — și o spun fiindcă am schimbat ceva pe mașină.** Scrisesem că rămâne la Costin, presupunând că `crontab`-ul e al lui; el n-a spus asta, iar decizia (b) descrie o stare în care sonda **rulează**. Iar propria mea gardă din R74 (`test_RITMURI_nu_pastreaza_joburi_care_nu_mai_exista`) a respins commitul exact pe asta: un prag pentru un job care nu există nicăieri e o fantomă. Deci ori scoteam pragul, ori puneam jobul — am pus jobul.
  ...
  Linia:
  `*/15 * * * * cd /home/costin/iconta_nou && set -a && . /home/costin/.iconta/db.env && . /home/costin/.iconta/api_keys.env && set +a && /home/costin/iconta_nou/venv/bin/python3 -m core.sonda_web >> /home/costin/iconta_nou/sonda_web.log 2>&1`
  Până atunci sonda **nu rulează**, iar `verifica_batai` o va raporta ca job care n-a bătut niciodată — corect, chiar n-a.
- **de ce contorul de reluări e 2, și nu 0**: decizia asta a fost cerută de **trei** ori. Vezi mai jos, la instanță — e chiar clasa pe care Costin a numit-o întrebând *„de ce nu ajunge la restanță"*.
- **INSTANȚA CARE A PRODUS GARDA NOUĂ**: din restanțele DESCHISE deblocate de DECIZIE, **19 aveau `reluări: 0`** deși registrul se mișcase sub ele — **R9 de 133 de commituri**, R18 de 122, R26 de 90. Câmpul exista, garda lui exista (`test_trei_reluari_fara_rezultat_cer_rescrierea_conditiei`, care sare la ≥3), dar contorul nu urca niciodată — **deci garda aia n-a putut să se aprindă nici măcar o dată**. Iar clasa era **deja găsită și scrisă** în registrul ăsta (*„contorul de reluări e 0 pe TOATE cele 25 de restanțe […] regula era scrisă și nepăzită, deci se citea ca respectată"*) și lăsată ca **disciplină**. `core/test_reluari_decizie.py` o mută pe gardă.
- **condiția de deblocare**: *(rescrisă 27.08.2026, după decizie)* sonda rulează din `crontab` la 15 minute și **bate** — adică `cron.verifica_batai()` nu o mai listează. Se închide când o oprire a serviciului web lasă o urmă pe care o vede cineva fără să fi rulat din întâmplare altceva. *(Condiție rescrisă.)*
- **Condiția de atunci** *(păstrată ca istoric)*: decizia lui Costin între **(a)** o sondă **externă** (ping de la un serviciu terț care alertează la lipsa semnalului) — singura care acoperă și cazul „serverul e jos", limită deja scrisă în `cron.verifica_batai`; **(b)** o supraveghere **locală** (`OnFailure=` pe unitate + un job care compară `ActiveEnterTimestamp` cu ultima valoare știută) — mai ieftină, dar oarbă exact când contează; **(c)** se declară acceptat că o cădere scurtă trece nevăzută, cu motivul scris. Se închide când o oprire a serviciului web lasă o urmă pe care o vede cineva fără să fi rulat din întâmplare altceva.
...
- **[BLOC QQ, 28.08.2026] SCOPUL VERIFICĂRII, CORECTAT — decizia lui Costin.** *„Cele patru poziții din D112 (444, 4315, 4316, 436) nu mai sunt verificare de divergență — nu mai pot diverge, prin construcție (R34). Devin gardă de regresie: rămân comparate mecanic, dar un roșu acolo înseamnă «cineva a reintrodus un calcul independent», nu «dezacord fiscal real». Verificarea reală rămâne pe 641/421 și 642/5328."*
- **de ce era nevoie de reclasificare, și nu de ștergere**: după R34, comparația celor patru compară declarația **cu ea însăși**. A le **scoate** ar fi lăsat reintroducerea unui calcul independent să treacă neobservată; a le **lăsa nemarcate** ar fi făcut ca un roșu să trimită omul să caute în date, când ar trebui să caute în `note_lunare`. **Aceeași comparație, alt înțeles — deci se scrie înțelesul.**
- **QQ1 — cum se vede în cod**: fiecare divergență poartă acum `fel` ∈ {`regresie`, `verificare`}. `control_coerenta` o spune în docstring: *„nu poate diverge azi; un roșu aici înseamnă cablaj stricat, nu dezacord fiscal"*. Cine citește rezultatul și nu se uită la `fel` primește exact ce primea înainte; cine se uită află dacă semnalul e despre **bani** sau despre **cablaj**.
- **QQ2 — cele două poziții rămase au primit contra-valoarea lor DECLARATĂ**, altfel „verificarea reală" ar fi fost un nume, nu o măsurătoare:
  - **641 = 421** ↔ `angajatorB B_brutSalarii` (= `C1_11` = suma `B2_5`). **Nu** `B4_3`: generatorul spune explicit că `bazac` e *„DOAR salariul REALIZAT, EXCLUDE indemnizația"* — adică exact ce intră pe 641 —, pe când `B4_3` e brutul **contractual**. Pe o lună cu concediu medical, cele două diferă **prin construcție**, iar o ancoră care diferă prin construcție produce zgomot, nu măsurătoare. *(Ales după ce s-au măsurat amândouă, nu presupus: pe `B4_3` ar fi ieșit **7** perechi divergente din 30, dintre care **2** — `tenant_013` și `tenant_014`, august — sunt luni cu concediu medical în care nota și `bazac` coincid la bănuț: fals pozitiv curat.)*
  - **642 = 5328** ↔ secțiunea 8.3, `E3_10` (masă) + `E3_75` (vacanță) — exact cele două tipuri pe care nota le pune pe 5328.
- **gardat**: `core/test_coerenta_salarii.py` crește la **13** teste. Trei sunt noi și niciunul nu e decorativ: *fiecare divergență își spune felul* · **harta felurilor e completă și disjunctă** — conturile marcate `regresie` trebuie să fie **exact** cele citite din D112, iar cele `verificare` exact `{421, 5328}`; dacă un cont trece dintr-o clasă în alta fără să-i schimbe cineva **sursa**, semnalul începe să mintă despre ce înseamnă · **anti-vacuu pe cititorul de XML**: dacă `_suma_din_xml` n-ar găsi nimic ar întoarce 0, iar divergența ar apărea pe toată lumea — sau ar **dispărea**, dacă și nota e 0.
- **CE A GĂSIT VERIFICAREA REALĂ LA PRIMA RULARE: 10 divergențe pe 5 perechi — restanță nouă, R86.** Nu se repară azi (QQ3, cerut explicit).
- **ce NU s-a atins**: ecranul. Câmpul `fel` e **aditiv** — JS-ul nu-l citește, deci tabelul de divergențe arată exact ca ieri. Azi asta nu ascunde nimic (cele patru nu pot diverge), dar în ziua în care ar apărea un roșu de `regresie`, omul l-ar vedea la fel cu unul real. **Consemnat ca limită, nu reparat**: schimbarea ar fi o rearanjare de ecran.
- **[BLOC TT, 28.08.2026] ÎNCHISĂ — ultimul element, `core/compensare.py`, s-a SCOS. Decizia lui Costin: varianta (c).** *„Nu am nevoie de funcționalitatea de compensare. Modulul se scoate, nu se investighează, nu se păstrează ca bază pentru viitor."* **Motivul, în termenii lui: *funcționalitate neplanificată, nu justifică investiția de timp.*** Data: **28.08.2026**.
- **de ce (c) și nu (a) sau (b)**: **(a)** i se dă un apelant — ar fi cerut întâi să se decidă **unde** apare compensarea pe ecran, adică o cerință de produs pe care n-a cerut-o nimeni; *a lega un modul ca să iasă dintr-o listă e motivul greșit pentru o funcționalitate nouă*. **(b)** o declarație scrisă, ca `fisa_cont` — dar aia e forma pentru un **producător fără livrare**, un artefact care **va** fi livrat; compensarea nu e amânată, e **nedorită**, iar o declarație ar fi păstrat în registru o promisiune pe care n-o face nimeni. **(c)** codul mort care trece verde e mai scump decât absența lui: nimic nu-l semnalează, iar la fiecare măsurătoare de acoperire cineva îl numără.
- **TT2 — verificat ÎNAINTE de a atinge ceva, și arătat**: **zero** importatori în producție; singurul consumator era propriul lui test (`core/test_compensare.py`). Restul potrivirilor pe cuvântul *„compensare"* sunt **alt lucru**, verificate una câte una: `static/js/ecrane/operatiuni_ecran.js` (*achiziție de la agricultor, compensare 8%*), `date_test/C1` (*Fondul de compensare a investitorilor*), `date_test/C2` (*procent forfetar, art.315^1*) și **ghidul publicat** `ghid/compensare-datorii-terti.md`, care explică regula din Codul civil unui contabil și **nu depinde de modul**. Ghidul rămâne.
- **ce NU s-a șters, deliberat**: sursa din corpus, `anaf_surse/cod_civil_287_2009_art_1616_1623_compensare.txt` — act **ADUS**, cu amprentă, pe care se sprijină ghidul. *Un corpus nu se subțiază fiindcă un consumator de cod a plecat.* Verificat și că nu produce **declarație orfană**: fișierul rămâne pe disc, deci `test_provenienta` nu are ce reclama.
- **TT3 — ce a cerut curățenia, și n-am ținut-o eu minte**: blocul generat din `GARZI.md` cita `core/test_compensare.py`, iar **`test_registrul_nu_citeaza_teste_moarte` a oprit commitul** până la regenerare. `PIN` din `core/test_module_nelegate.py` nu mai conține **niciun** modul marcat `PRODUCTIE, nelegat` — exact condiția de închidere.
- **istoric — ce rămăsese DESCHIS din R33 până azi: `core/compensare.py`.** Condiția restanței cere ca `PIN` din `core/test_module_nelegate.py` să nu mai conțină niciun modul marcat `PRODUCTIE, nelegat` — și mai conține **exact unul**, măsurat azi pe cod: *„5 funcții publice, inclusiv `pull(conn, schema)` și `propune_compensari()`"*. `fisa_cont` are **declarație** scrisă (producător fără livrare, METODA §20), `echilibru_perioada` a ieșit din `PIN`, `salarii_contare` e legat din 25.08. **Deci restanța nu se închide azi** — nu fiindcă ar mai fi de lucru la ea, ci fiindcă ultimul ei element cere o decizie de produs, nu o cablare. Vezi cerința din raport.
...
- **ÎNCHISĂ 28.08.2026 (BLOC QQ4) — ambele jumătăți ale condiției, îndeplinite. Și una dintre ele era îndeplinită de trei zile, fără să știu.**
  - *„sonda întoarce 0 divergențe"* — **îndeplinită la BLOC OO**, în aceeași zi: forma nouă dă **0** pe cele patru poziții, forma veche **24** pe 7 perechi.
  - *„iar `control_coerenta` e legat"* — **era deja legat din 25.08.2026**, la R33: `POST /tenants/{}/salarii-contare/propunere` și `POST /tenants/{}/salarii-contare` cheamă `propunere`, care cheamă și `note_lunare`, și `control_coerenta`. **Verificat pe AST, nu presupus.**
- **CORECTAREA UNEI AFIRMAȚII DE-ALE MELE, din aceeași zi.** La BLOC OO am scris aici că restanța rămâne deschisă *„fiindcă propria ei condiție cere ȘI legarea lui `control_coerenta` — R33, amânată pe cerere"*. **Fals.** Legarea se făcuse pe 25.08 și e scrisă la R33, în registrul pe care îl aveam deschis. Am citit condiția, am recunoscut numele `control_coerenta` în ea, și am presupus starea în loc s-o verific — la trei paragrafe distanță de propria mea regulă despre asta. *Raționamentul care ține o restanță deschisă cere aceeași verificare ca cel care o închide; e mai ușor de ratat fiindcă rezultatul lui pare prudent.*
- **cum se citește „0 divergențe" acum, și de ce nu e o portiță:** sonda întoarce azi **10** divergențe — dar **toate pe alte poziții** (641/421 și 642/5328), care nu sunt subiectul lui R34 și care au restanță proprie, **R86**. Pe cele patru poziții ale ei — 444, 4315, 4316, 436 — sonda dă **0**, și de acum **nu mai poate da altceva** decât dacă cineva reintroduce un calcul independent. Condiția a fost scrisă când sonda măsura doar cele patru; se citește pe domeniul ei, iar restul se numește separat.
...
- **reluări**: 1
- **stare**: **REZOLVATĂ**
- **rezolvată pe commit**: `872526c`
- **deschisă pe commit**: `8a10080`
- **măsurat la**: 2026-08-28 · **pe commit**: `872526c`
...
- **condiția de deblocare**: un verdict nu poate fi **verde** cât timp `facturi_necontabilizate > 0` pe perioada lui
- **REZOLVATĂ 28.08.2026 (BLOC XX).**
- **XX1 — MECANISMUL, arătat înainte de orice reparație.** `compara_tva` decide culoarea pe **prima** ramură: `if abs(dif) <= TOLERANTA: → verde`. Ramura aia **nu se uită deloc** la `necontate` — deși lista e chiar în apelul curent, primită ca parametru. `necontate` se consultă abia **după**, ca să explice o diferență care există. Deci ordinea e inversă față de P6: se dă favorabilul, apoi se caută cauza. Când ambii termeni sunt **zero**, egalitatea se produce prin **absența amândurora** — o factură neînregistrată lipsește deopotrivă din rulaj **și** din decontul REGENERAT din evidență —, iar ecranul spune *„D300 și contul coincid"*.
- **XX2 — verdictul: e o CONDIȚIE greșită, se repară direct. Dar are DOUĂ locuri, iar al doilea nu se vede din primul.** *(a)* ramura verde din `compara_tva`; *(b)* agregarea din `verifica_tva`, care era `rosu if any(rosu) else verde` — **fără ramură de gri**. Fără *(b)*, reparația *(a)* ar fi **arătat făcută și n-ar fi fost**: constatarea gri exista, dar verdictul de sus rămânea verde. `verifica_d390` și `verifica_d112` aveau deja ramura; `verifica_tva` nu. *A doua instanță, în aceeași funcție.*
- **măsurat ÎNAINTE, pe date reale** (`scripts/sonda_r35.py`, instrument nou): domeniul e **toate schemele active × lunile cu cel puțin o factură = 27 de perechi**.

  | | înainte | după |
  |---|---|---|
  | verzi peste un necunoscut din propriul payload | **6** | **0** |
  | verzi peste o constatare gri (colapsul agregării) | 0 *(latent — niciun gri viu azi)* | 0 |

  Cele 6: `tenant_004` 2026-02 și 2026-06 · `tenant_017` 2026-03, 2026-08, 2026-09, 2026-11. **La 4 din 6, TVA-ul facturii necontabilizate e 0,00 sau necunoscut** — și asta contează: mesajul nu are voie să-l prezinte ca pe o cifră știută.
- **și un defect al PROPRIEI mele sonde, prins de ea însăși.** Prima formă întreba de coloana `data` pe `facturi` — care se numește `data_emitere`. Excepția era înghițită de `rollback`, iar sonda a raportat liniștit **„0 perechi · 0 verzi peste necunoscut"**: un rezultat **favorabil pe o lume pe care n-o vedea**. E chiar clasa [[gard-care-nu-se-verifica-pe-sine]], în instrumentul cu care măsuram tocmai clasa asta. Reparat cu **anti-vacuu**: domeniul gol e acum o **eroare**, nu un răspuns.
- **XX3 — ce spune ecranul acum**, când cifrele coincid dar există facturi în afara evidenței: *„TVA colectată: D300 și contul 4427 coincid (0,00), dar 1 facturi emise din lună nu sunt în evidență — TVA-ul lor e 0,00 sau nu se cunoaște. Coincidența NU spune nimic despre ele."* Cu remediu de tip **investigație** (cauza nu e dovedită) și cu **facturile numite**, ca omul să le găsească.
- **GRI, nu roșu, și motivul e P6:** nu se știe că cifrele sunt greșite — se știe că **nu se poate afirma** că sunt bune. Un roșu ar afirma o cauză pe care măsurătoarea n-o are.
- **gardat**: `core/test_verde_peste_necunoscut.py`, **8 teste**, cu domeniul cerut de condiție (**toate** schemele active, prin sondă). Include calibrarea în ambele direcții — *fără facturi necontabilizate rămâne VERDE*, iar *o factură pe cealaltă direcție nu atinge comparația asta* (altfel gardul ar produce gri peste tot și n-ar mai discrimina) — plus aserțiunea că agregarea are toate trei valorile, citită ca **nod de AST**, nu ca text.
- **RED-PROOF pe sursa REALĂ, ambele mutații**: culoarea întoarsă la verde → **roșu**; agregarea fără ramura de gri → **roșu**. Fișierul restaurat **identic** (verificat pe octeți), 8 verzi după restaurare.
- **ce NU acoperă, declarat**: nu verifică dacă facturile alea **chiar trebuiau** contabilizate în luna aia. Spune doar că, atât timp cât sunt în afara evidenței, egalitatea nu afirmă nimic despre ele. Și: clasa „verde peste gri" e azi **latentă** — n-a fost prinsă nicio instanță vie, fiindcă semnalul pe 4428 nu se aprinde pe datele curente. **Se repară pe o clasă, nu pe o instanță.** — devine **gri** (necunoaștere, cu domeniul ei) sau roșu, după caz. Se închide când sonda de mai sus întoarce **zero perechi verzi cu necontabilizate nenule**, iar gardul care o probează are în domeniu toate cele 17 scheme.
...
#### Lista 5 — ies și se validează, dar NU se pot verifica pe ecran — **12 poziții, din care 11 REPARATE (30.08.2026)**

*Reparată prin ORDINEA dată de Costin pe 30.08 (`DECIZII.md` 12): lista 4 → **lista 5** → lista 3.
Coloana din dreapta e ce era; a treia, ce s-a făcut. **Măsurat la**: 2026-08-30 · **pe commit**: `4b6d664` (arborele care devine commitul următor).*

| artefact | ce lipsea, măsurat | ce s-a făcut |
|---|---|---|
| **cele 9 declarații** | **0 din 92 de ieșiri își arată componentele.** Ruta întoarce un CONTOR de operațiuni, nu operațiunile. Iar cele 92 de `temei` sunt ale **validatorului**, nu ale cifrei. *Aici nici ruta nu trimite — deci e mai scump decât cele de mai jos.* | **8 din 9 REPARATE.** Prețul s-a dovedit mai mic decât se estimase, și **de ce** contează: componentele **existau deja** pe obiectele de rezultat ale motoarelor (`d100.obligatii`, `d205.beneficiari`, `d300.R`, `d101.P`, `d301.operatiuni`, `d390.ops`, `d394.op1`, cele trei liste ale lui `d406`). Nu lipsea calculul, lipsea **transportul**. O singură hartă (`core/declaratii_componente.py`), nu nouă serializatoare — nouă bucăți de cod ar fi produs nouă liste care se pot despica de motorul lor în tăcere. **D112 rămâne**, cu motiv scris: generatorul ei întoarce `(xml, avertismente)`, deci n-are obiect de rezultat de citit. **R105.** |
| **Balanța de verificare** | se închide pe toate cele 13 firme cu date (SI D=C, rulaje D=C, SF D=C), dar ecranul are titlu, navigare pe lună și **un buton „Descarcă PDF"** — atât. Ruta de cabinet întoarce **PDF**, nu date | **REPARATĂ.** Rută de date nouă (`GET /tenants/{id}/balanta`), înregistrată în T05, cu tabelul pe ecran și totalurile citite din **sursa unică** (`documente_api.totaluri_balanta` — erau calculate înăuntrul PDF-ului). Închiderea vine ca **obiect cu trei stări**, și a treia e cea care contează: pe o lună fără nicio înregistrare toate cele trei perechi sunt `0 = 0`, deci verdictul naiv ar fi „se închide" despre o lună în care **nu s-a verificat nimic**. Re-măsurat azi pe cele 19 scheme: **13 `se_inchide` · 6 `nimic_de_verificat` · 0 `nu_se_inchide`** — cele 13 sunt exact firmele cu date din 1c. |
| **netul de pe fluturaș** | ruta trimite componentele, ecranul nu randează **niciuna** — cuvintele apar în JS doar ca etichete pe formulare de introducere. **Adâncimea, măsurată la R97: 12 câmpuri tăcute**, nu 7 cât se știa; printre cele noi `cm_brut`/`cm_net`/`cm_zile` (**concediul medical**) și tichetele. Componentele se văd **numai în PDF-ul descărcat** | **REPARATĂ, 11 din 12.** Compoziția s-a mutat într-o **sursă unică** (`stat_plata_api.compozitie_fluturas`) pe care o citesc și hârtia, și ecranul; a doua scriere, în JS, ar fi produs exact clasa pe care `rand_fluturas` o descrie în docstring — *două calcule ale aceluiași lucru nu rămân egale*. Al 12-lea câmp, `retinut_tichete`, e **declarat neafișat cu motiv** (`CAMPURI_NEAFISATE_MOTIVATE`): e suma exactă a două rânduri deja arătate, iar motivul e verificat prin mutație, nu crezut pe cuvânt. |
| **Registrul-jurnal** (14-1-1) | **venit din lista 3**: ruta derivă `nr_curent`, `document` și `total_debit`/`total_credit` — cele trei elemente cerute de OMFP 2634 Anexa 1 pct. 45 — iar ecranul nu afișează **niciunul**. **Adâncimea, măsurată la R97: 2 câmpuri tăcute sigur** plus 2 candidate | **REPARAT.** `Nr. crt.` pe fiecare rând · documentul justificativ, iar când nu se poate deriva ecranul spune **„nederivat — nota n-are referință scrisă și nu e legată de o factură"**, nu „fără document": *un necunoscut nu se rotunjește la „știu că nu"* (interdicția 32) · totalizarea lunară în casetă, cu numărul notelor fără document derivabil. R97 pe ruta asta: **0 câmpuri tăcute**. |

**O DIVERGENȚĂ GĂSITĂ ÎN TIMPUL REPARAȚIEI, și reparată prin construcție:** `fluturas_pdf` calcula
`val_tichete = nominal + vacanță + cadou`, iar ruta trimitea `total_disponibil` cu **încă doi
termeni** — tichetele culturale și cele de creșă. Deci hârtia și ecranul arătau **totaluri diferite**
pentru același salariat, ori de câte ori avea culturale sau creșă; mai mult, poarta
`if are_masa or are_vac or are_cadou` sărea **întreaga secțiune de tichete** pentru un salariat care
are NUMAI culturale sau creșă. Sursa unică o închide prin construcție, iar
`test_salariat_doar_cu_tichete_culturale_isi_vede_tichetele` o ține închisă. *Nu era o ipoteză: era
acolo, și n-o vedea nimeni fiindcă cele două cifre nu se întâlneau niciodată pe același ecran.*

**CE NU ÎNSEAMNĂ „REPARAT" AICI, și se spune:** că artefactele **ies** și își **arată componentele**
pe ecran, nu că cifrele sunt corecte. Corectitudinea lor e altă întrebare, cu alt instrument.

**Cele două rânduri de jos sunt aceeași formă — „ruta livrează, ecranul tace" — și de-aceea NU mai
sunt o listă separată** (decizia de ordine din 30.08, `DECIZII.md` 12): *„nu sunt două șantiere […]
R97 pe artefacte nu e o listă separată, e adâncimea listei 5."* Clasa **R97** rămâne deschisă pentru
**restul** ei — **32 de câmpuri pe 13 rute, toate pe ecrane interne**, în afara setului de artefacte
pe care faza 1 îl măsoară.

**Iar despărțirea de preț rămâne, și e chiar ce ordonează munca:** pentru netul de pe fluturaș și
pentru registrul-jurnal, serverul **trimite deja** tot ce trebuie și se pierde la randare; pentru
cele 9 declarații, **nici ruta nu trimite**.
...
**ORDINEA, decisă de Costin pe 30.08.2026** (`DECIZII.md` 12): **lista 4 → lista 5 → lista 3**, iar faza 2 după ele. *Lista 4 nu e prima fiindcă e mică, ci fiindcă e cea despre care se credea că e goală: „o listă care iese din gol e semnul că măsurătoarea dinaintea ei se uita în altă parte — două poziții acolo cântăresc mai mult decât opt într-o listă care s-a comportat cum trebuia.”* Ordinea **nu contrazice** decizia din 23.08 (L5 înaintea lui L3) — o extinde, punând L4 în față.*
...
- **lista 5 (12 poziții)** — **11 REPARATE pe 30.08.2026**, rămâne **D112** (`R105`). Despărțirea de preț făcută la 1c a ținut doar pe jumătate, și partea greșită merită scrisă: netul și registrul-jurnal au fost într-adevăr ieftine, dar **cele 9 declarații n-au fost scumpe** — componentele existau pe obiectele de rezultat, deci lipsea transportul, nu calculul. *Estimarea „nici ruta nu trimite" era adevărată despre RĂSPUNS și falsă despre ce are motorul în mână; cele două nu fuseseră deosebite.* Singura care chiar e scumpă e D112, și e scumpă din alt motiv decât se credea: nu pentru că n-are componente, ci pentru că generatorul ei nu le întoarce.
...
> scrisă (`fel_ancorei`): **1222** ancorează pe ceva ce apare oricum, restul — sub o zecime — pe un
> semn de rău; iar clasificatorul supraevaluează semnul de rău, deci **partea de semn-de-rău e
> plafon SUPERIOR** și **1222 plafon INFERIOR**. *Doar 1222 are clichet; celelalte două cifre au
> îmbătrânit tăcut (1341/119 → 1342/120, 31.08) și nu se mai scriu — `core/scan_garzi_pe_text.pe_fel()`.*
>
...
## 77 — Un blocaj fără temei: aplicația oprește un act și nu spune pe ce se sprijină
...
- **limita pe care norma și-o scrie singură**: **UMBRA rămâne DELIBERAT în afara plafonului, și din 31.08.2026 rămâne NEMĂSURATĂ DEFINITIV** (Costin, `DECIZII.md` 15 — *«rămân nemăsurate, definitiv»*: populația nu se auditează, nu devine restanță și nu se mai deschide ca temă; ce continuă e derivarea cifrei, nu munca pe ea). *(Cifra ei se derivă cu `scripts/scan_refuzuri.umbra()` și **nu se scrie aici**: un număr care nu e plafonat n-are de ce să stea în proză, iar scris ar cere actualizare la fiecare modul nou — adică ar deveni de facto un clichet, cu costul unuia și fără protecția lui. Prima formă a acestei norme îl scria, iar garda care îl confrunta a căzut a doua zi, la primul modul adăugat.)* Sunt module care refuză și nu citează legea nicăieri: ori sunt generice (`db.py`, `bacsis.py`) și atunci e în regulă, ori aplică o regulă pe care n-o pot numi — și atunci e **mai grav** decât datoria, fiindcă nu se vede deloc. **Nu se poate deosebi mecanic** una de alta, iar un clichet pe cifra asta ar fi un plafon inventat: ar cădea la prima mutare de cod și ar fi ridicat fără să se fi reparat nimic. Se numără, se scrie, **nu se plafonează**.
- **cazul care intră sub normă ca FORMĂ, nu ca reparație**: `verificator_neconformitati.sh:28,29` tipărește *„FAIL NC-07 JWT_SECRET lipsește din proces!"* când `sudo` nu e disponibil neinteractiv. **Secretul e acolo**; ce lipsește e dreptul de a te uita. **Un blocaj care afirmă absența când de fapt n-a putut verifica e blocaj fără temei** — aceeași clasă cu „un necunoscut nu se rotunjește la «știu că nu»". Reparația e un **al treilea rezultat, `NU S-A PUTUT VERIFICA`**, distinct de `PASS` și `FAIL` — **nu** lărgirea setului îngust de `sudo`: un verificator care are nevoie de mai multe drepturi ca să spună adevărul cere să fie crezut pe încredere. **REPARAT 31.08.2026**, la cererea lui Costin (*«repară-l acum»*): al treilea rezultat există, cu **cod stabil** pe fiecare motiv, iar setul de `sudo` **n-a fost atins**. Măsurarea dinaintea reparației a scos **încă trei forme latente** ale aceleiași clase în același script — una dintre ele greșind în direcția OPUSĂ (un glob gol raporta `PASS`) — plus un cod de ieșire care întorcea `0` peste propriul `FAIL: 3`. Gard: `core/test_verificator_al_treilea_rezultat.py`, 18 teste, RED-proof 8 mutații / 8 roșii.
- **instanțe**: **cea care a deschis clasa e a mea, din tura precedentă** — `registre_art321.py` și `registru_inventar.py` refuză cu *„fel necunoscut"* / *„moment necunoscut"* fără temei, deși nomenclatorul e închis **pentru că norma numește exact două registre**. Amândouă module scrise ieri, cu `Temei` la un rând distanță. **Rămân în baseline, nereparate în tura măsurătorii** — altfel cifra de pornire ar fi una aleasă, nu una găsită. A doua instanță, de altă formă: `common.py` are **14**, cel mai mult din tot repo-ul, și e chiar fișierul unde stă `Temei`. A treia, în afara Python-ului: `verificator_neconformitati.sh` (vezi mai jos). *Clasa n-a fost descoperită azi — `scan_refuz_tacut` o atingea din 27.08 pe cealaltă față a ei (dacă refuzul **ajunge** la om); azi a primit cifră pe fața cu temeiul.*
- **calibrare**: în ambele direcții, pe cod sintetic. **Cazul cunoscut a fost GĂSIT**: refuzurile din `registre_art321.valideaza` care poartă `temei=` sunt clasificate `structurat`, iar cele două din același fișier care nu-l poartă intră în datorie — deci instrumentul deosebește două refuzuri **din aceeași funcție**, nu doar două fișiere. Vede temeiul în toate cele patru forme (`temei=`, cheie de dict, interpolare `TEMEI*`, `Temei(...)`); **nu** dă fals-pozitiv pe un refuz chiar gol; deosebește 401/403 de 404 de 400/409; și **nu numără** un `raise KeyError`, un `raise RuntimeError` sau o re-ridicare goală — altfel cifra ar crește din cod care n-are legătură cu norme.
- **ce nu vede**: **(1)** un refuz construit de un ajutor și ridicat în altă parte e numărat la locul lui `raise`, unde temeiul nu se vede → `DATORIE` e plafon **superior**; **(2)** nu deosebește *normativ* de *formă* înăuntrul unui modul care citează — un modul care citează legea o dată face candidate toate refuzurile lui; **(3)** vede doar Python, nu și refuzurile din ecran; **(4)** vede doar `raise` — cele **88** de locuri care `return {"eroare": ...}` sunt umbra instrumentului însuși, **numărate separat, niciodată adunate la datorie**. Instrumentul greșește în **amândouă** direcțiile, și de-aia cifrele lui **nu se însumează**.
- **unde ajunge efectul**: un refuz fără temei nu oprește contabilul din a face ceea ce voia — îl oprește din a înțelege **de ce nu poate**, și îl lasă cu o singură ieșire: să caute o cale ocolită. Iar la un control, un act refuzat de aplicație fără să spună sub ce normă nu se poate apăra nici de noi: nu putem arăta ce regulă am aplicat, doar că am aplicat una. Cele 64 nu produc cifre greșite; produc **decizii pe care nimeni nu le mai poate reconstitui**, nici măcar autorul lor.
