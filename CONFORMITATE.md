...
- **măsurat la**: 2026-08-24
- **pe commit**: `ad504f8`
- **cifra**: **50**, măsurat 24.08.2026 pe un domeniu declarat și **reproductibil** (`core/scan_garzi_pe_text.py`): din 397 de fișiere-gardă, **78** ating o sursă `.py`/`.js` și o caută cu `in`; **28** o curăță de proză înainte, **50** nu. *Cifra veche de aici — **14**, una exclusă prin natura ei → 13 — a fost măsurată ad-hoc pe 22.08 pe 369 de fișiere și n-a lăsat niciun instrument în urmă, deci nu se poate recalcula: era un plafon inferior al unui domeniu mai îngust, nu un inventar.* Clichetul stă pe 50 (`core/test_garzi_pe_text.py`) și **nu poate crește**. Una (`test_octeti_invizibili`) e **exclusă prin natura ei**: un octet invizibil dintr-un comentariu e tot un defect, deci trebuie să citească textul brut. Rămân **13 candidate**, din care **3 privite pe context**: 1 poate ASCUNDE o gaură, 2 pot doar să RAPORTEZE ÎN PLUS. Restul de 10, neprivite.
