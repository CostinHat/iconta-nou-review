...
- **pasul curent**: **faza 1 terminată; clasa R97 MĂSURATĂ** — 66 de câmpuri livrate și tăcute, pe 17 rute, din care doar 2 poartă artefacte cerute de lege. **Lista 5 rămâne la 10 poziții.** Urmează ordinea listelor, cu cifra pe masă. *(Cifrele se derivă.)*
...
- **ce blochează**: **CLASA ARE CIFRĂ, și nu e caz izolat.** Instrument: `scripts/scan_r97_livrat_tacut.py`, care ia fiecare `api.get(...)` din `static/js/` — **perechea o declară ecranul, nu o ghicesc eu** —, cheamă ruta **VIU** și confruntă cheile răspunsului, la toate adâncimile, cu ce apare în JS. **Domeniu:** 175 de apeluri, 129 de perechi distincte chemate, **106 măsurate** (restul: 36 cu interpolare nesubstituibilă, 23 refuzate — 19 rute de administrator, 2×404, 1×422, 1×405). **REZULTATUL, pe două praguri:** **TĂCUT SIGUR — numele nu apare NICĂIERI în `static/js` — 66 de câmpuri, pe 17 rute**; **candidați** (apar altundeva, dar nu în ecranul care le cere) — 268 de câmpuri, pe 41 de rute. *Primul e **plafon inferior** prin construcție: „randat" se caută pe nume și e supra-numărat, iar un câmp folosit doar în logică trece drept randat. Direcția e aleasă: o cifră care spune „cel puțin atâtea" poate dovedi că nu e izolat; una care supra-numără n-ar dovedi nimic.* **Cele mai mari instanțe:** `stat-plata` **12** (mai mult decât se știa — pe lângă cele 7 componente ale netului, tac și `cm_brut`/`cm_net`/`cm_zile`, adică **concediul medical**, plus `valoare_tichete`/`retinut_tichete`) · **`/tenants/{id}/scoatere` 11 din 31** — previzualizarea celui mai distructiv act al aplicației nu arată ce a fost decis (`confirmare_ceruta`, `evidenta.nedecis`) și nici ce anume s-ar șterge (`randuri_de_sters.*`) · `control-fiscal/{id}` **10 din 216** (printre ele `regim_tva_anaf` și `facturi_necontabilizate`) · `contracte/marcaje` **9 din 11** · registrul-jurnal **2 sigure** (`note[].nr_curent`, `note_fara_document`) plus 2 candidate (`total_debit`, `total_credit`, care apar în alt fișier JS, nu în cel al jurnalului). **89 din cele 106 rute măsurate n-au niciun câmp tăcut sigur** — deci clasa e reală și **concentrată**, nu difuză.
- **ce NU crește din asta, și e la fel de important**: **lista 5 rămâne la 10 poziții.** Din cele 17 rute, doar **două** poartă artefacte din lista lui 1a — statul de plată (netul) și registrul-jurnal —, iar amândouă erau **deja** în lista 5. A treia, `rip/inventar` (Registrul-inventar 14-1-2/b), rămâne în afara listelor: 0 firme de partidă simplă. Celelalte 14 sunt **ecrane interne**, nu artefacte cerute de lege. *Deci marginea despre care lista 5 spunea „nu se poate ști" e acum măsurată, și e **zero poziții noi**. Ce a crescut e **adâncimea** unei poziții: netul avea 7 câmpuri tăcute cunoscute, are 12.*
...
**Consecința pentru numărătoare — MĂSURATĂ pe 30.08.2026, și răspunsul e „zero”.** Cele **10
poziții** de mai sus erau declarate plafon inferior până se măsura clasa R97. S-a măsurat: din cele
**17 rute** cu câmpuri tăcute sigur, doar **două** poartă artefacte din lista lui 1a — statul de plată
(netul) și registrul-jurnal —, iar amândouă erau **deja** înăuntru. **Lista 5 rămâne la 10 poziții.**
Ce a crescut e **adâncimea** uneia: netul avea 7 câmpuri tăcute cunoscute, are **12**. *Clasa e mare — 66
de câmpuri pe 17 rute — dar cade aproape întreagă pe **ecrane interne**, nu pe artefacte cerute de lege.*
