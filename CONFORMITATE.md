...
- **felul**: VERIFICARE
- **cine deblochează**: DECIZIE
- **unde intră**: E1 · TRASEE T35 · interdicția 46 · **PRAG 2** *(măsurat, nu presupus: **0 din 17** firme au `principal_client_id` completat, iar toate trei rutele cer `principal_client_id = utilizatorul curent` — deci azi toate trei răspund **403**, pe orice firmă. Defectele sunt reale în cod și **neexercitabile** pe date)*
- **reluări**: 0
- **stare**: DESCHISĂ
...
- **condiția de deblocare**: **la primul client cu acces la portal** — decizia lui Costin, cu motivul: `principal_client_id` e **o coloană necompletată, nu una abandonată**; portalul e funcționalitate vie, iar rutele nu sunt cod mort, sunt *cod care așteaptă date*. Se închide când cele trei se exercită pe o firmă reală: un cont refuzat fiindcă e al altui cabinet, o schimbare de adresă care NU se aplică până la confirmare, și un cabinet care vede amândouă în `urme-portal`. Până atunci gardul e pe cod, iar asta se spune — e forma din [punctul orb e FIRMA, nu ecranul].
...
- **condiția de deblocare**: decizia lui Costin între **(a)** documentele operaționale produc mișcarea de stoc, iar `/stocuri/*` rămân pentru corecții; **(b)** rămân separate, dar există o **reconciliere** care arată divergența (e chiar instrumentul #5 din roadmap, nefăcut); **(c)** rămân separate și se **spune** că sunt, în aplicație, ca nimeni să nu creadă că balanța de stoc urmează contabilitatea. Se închide când cele două evidențe ori se leagă, ori divergența lor e vizibilă — cu gard. **Primul pas e făcut**: măsurătoarea de mai sus, cerută chiar de el.
