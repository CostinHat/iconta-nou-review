...
- **CREAREA nu produce divergență — capcana oul-și-găina NU există**: `provision_tenant` scrie **același** `nume` în `public.tenants` (`core/tenant_provisioning.py` l. 172) **și** în `{schema}.firma_profil` (l. 190), în aceeași tranzacție; `precompleteaza_din_anaf(seteaza_nume=False)`, chemat imediat după la `POST /tenants` (`main.py` l. 1313), nu atinge niciuna. O firmă creată acum zece secunde ar afișa în listă exact ce s-a tastat, chiar dacă adevărul ar deveni cel fiscal — **lista n-ar fi goală**. Ce s-ar schimba nu e conținutul, ci **costul citirii**: `GET /tenants` face azi o singură interogare pe `public`; o listă pe denumire fiscală ar cere o citire în **fiecare** schemă de firmă.
...
- **cine scrie în `public.tenants.nume` — toate locurile, nu doar `PUT`**: **patru**, toate în `core/tenant_provisioning.py`. (1) l. 172, `INSERT` la creare. (2) l. 227, `nume = COALESCE(NULLIF(%s,''), nume)` **numai** cu `seteaza_nume=True`, adică numai pe calea `POST /auth/register` (`main.py` l. 1165) — și scrie în `firma_profil`, nu în `tenants`. (3) l. 319, `alege_denumirea`, ramura `anaf`. (4) l. 361, `actualizeaza_tenant` (`PUT /tenants/{id}`). `grep` pe `UPDATE public.tenants` nu dă nimic altceva în afară de `activ`, `nume_anaf`, `nume_ales`.
