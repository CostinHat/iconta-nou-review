...
- **condiția de deblocare**: decizia lui Costin pe trei puncte, care pot merge separat — **(1)** schimbarea adresei cere confirmare pe adresa nouă și notifică vechea adresă, ori nu se poate face din portal deloc; **(2)** un cont de client dezactivat al **altui** cabinet se refuză, nu se reactivează (sau `accounting_firm_id` se mută explicit, cu urmă); **(3)** cabinetul primește o urmă pentru amândouă. Se închide când cele trei întrebări ale lui Costin au toate răspunsul *da*, cu gard — iar gardul are nevoie întâi de o firmă cu `principal_client_id` completat, altfel nu are ce exercita (e forma din [punctul orb e FIRMA, nu ecranul]).
