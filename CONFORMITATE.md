...
- **pasul curent**: **`TRASEE_VERIFICARI.md`** — 192 de locuri goale, unul per pas care schimbă ceva; le scrie Costin, în loturi de câte 30 (`scan_trasee.py --loturi N`, 7 loturi). Până atunci, inventarul de trasee e o hartă a codului, nu o listă de verificare.
...
- **decizii care blochează**: **una — R33, DESCHISĂ**, iar întrebarea ei s-a schimbat de două ori. Jumătatea de **prag 1** (semnalul notă-vs-D112) e decisă, aplicată și probată pe procesul viu. Ce rămâne e **prag 2**, și **nu mai e „care dintre cele două iese”**: rulate pe aceleași date (25.08.2026), `echilibru_perioada` și `verificatoare.verifica_balanta` au moduri de eșec **disjuncte** — prima prinde linia cu o parte lipsă și orfanii, a doua prinde soldurile inițiale dezechilibrate, iar fiecare o ratează pe cealaltă. **A alege una șterge o verificare.** Decizia cerută e **unde se leagă `echilibru_perioada`** și **ce se face cu jumătatea tautologică din `verifica_balanta`** (`BALANTA_INEGALA`, 0 din 2000 de seturi o pot face să pice). **Decizie cerută.**
...
- **condiția de deblocare**: la primul commit care atinge `core/gdpr_sterge.py`, `executa` (a) șterge și din `declaratii_coada` și `declaratii_depuse` pentru firmele cabinetului, (b) enumeră **numai** `table_type='BASE TABLE'`, ca să nu mai poată lovi un view, și (c) previzualizarea numără rândurile înainte, ca omul să vadă ce dispare. Se închide când o ștergere de cabinet nu mai lasă niciun rând pe firmele lui, probat pe o schemă efemeră.
...
- **condiția de deblocare**: fiecare dintre cele **patru** primește ori un apelant în producție, ori o declarație scrisă de ce rămâne nelegat. Se închide când `PIN` din `core/test_module_nelegate.py` nu mai conține niciun modul marcat `PRODUCTIE, nelegat`. **REFORMULATĂ 25.08.2026**, fiindcă măsurătoarea de mai sus a scos partea care spunea *„care dintre cele două implementări ale echilibrului rămâne, fiindcă a le păstra pe amândouă e chiar logica paralelă"* — cele două au moduri de eșec disjuncte, deci nu sunt două implementări, iar a alege una **șterge** o verificare. Ce rămâne de decis nu mai e *care iese*, ci **unde se leagă `echilibru_perioada` și ce se face cu jumătatea tautologică din `verifica_balanta`** — decizie de produs (schimbă ce vede contabilul pe `GET /firme/{tenant_id}/verificari`), deci §2.3 pct.2.
