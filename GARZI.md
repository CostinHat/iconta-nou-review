...
- **Neacoperit (raportat, regula 12) — toate depind de aplicaţie (model/cablaj), campanii proprii:**
  - **Deducerea suplimentară cod mort → impozit SUPRA-declarat.** `salarizare.deducere_personala` suportă `sub_26` (tineri) + `copii_scoala` (100 lei/copil, art.77 alin.10), dar NICIUN apelant de producţie le transmite (`d112.py`/`stat_plata_api` cheamă cu `persoane` doar) → tinerii sub 26 şi părinţii cu copii şcolarizaţi au impozit SUPRA-declarat în D112. Model `salariati` fără câmpuri distincte (vârstă/copii şcolarizaţi).
