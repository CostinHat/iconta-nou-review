...
- `core/test_rute_fiscale_lot3b.py` — Ultimele PATRU rute cu clichet fiscal, probate pana in cifra declaratiei.
...
| ratchet legacy | core/test_ghid_poarta.py::test_ghiduri_noi_poarta_verde + ghid/_legacy_pre_poarta.txt (clichet 202) | adăugarea unui ghid nou fără `poarta: v1` sau care nu trece poarta | ghid nou fără `poarta:` → `test_ghiduri_noi_poarta_verde` roșu (probat 21.09) | 202 ghiduri legacy grandfathered; 44 dintre ele citează 47 acte distincte încă neaduse în corpus (măsurat 21.09 DUPĂ reparația extractorului) |
