...
def dump_text(pg, tag):
    print("=== TEXT", tag, "===")
    seen=set()
    for sel in ["h1","h2","h3",".gol",".empty",".stare-goala",".avert",".eroare",".alerta","button","label","th"]:
        for e in pg.query_selector_all(sel):
            t = (e.inner_text() or "").strip().replace(chr(10)," | ")
            if t and 2 < len(t) < 140 and t not in seen:
                seen.add(t); print("  ", sel, "::", t[:130])

with sync_playwright() as pw:
    b, pg = new_page(pw)
    deschide_t005(pg)
    shot(pg, "00_meniu_firma")
    dump_text(pg, "00_meniu_firma")
