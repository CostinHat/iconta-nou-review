...
- **DEFAULT_FISCAL_TACIT** (v2.16, 23.07.2026). Un câmp fiscal decisiv (`regim_fiscal`, `tip_firma`, `platitor_tva`) **NU se defaultează pe LITERAL inline** — interzis `x or "micro"`, `x || "srl"`, `… else "pfa"`, `? : "micro"`. Faptul, normalizarea și default-ul trăiesc **într-un singur loc**: primitivele din `core/migrare_api.py` (`regim_contabil` = tip_firma→partidă simplă/dublă, `regim_efectiv` = regimul CIT efectiv cu partidă simplă⇒None, `tip_firma_nrm` = normalizare + default 'srl'). Consumatorii le importă; NU redau fallback-ul. Motiv: un `regim_fiscal or "micro"` inline în logica de obligații fabrică declarații pe firme la care nu se aplică (dovedit: termene emitea D100 pe un PFA, 23.07). Se prinde forma or/||/else (fallback pe CITIRE); atribuirea simplă `x = "srl"` (declarație, nu fallback) NU. Regula **DEFAULT_FISCAL_TACIT** în verificator scanează `.py`; EXCEPTAT: `migrare_api.py` (primitivele — singurul loc unde literalul e legitim). LIMITĂ: garda e pe `.py`; fallback-urile de AFIȘARE din frontend (`migrare.js`, `firme.js`: `tip_firma || "srl"`, `regim_fiscal || "micro"`) rămân un follow-up separat.
