...
- **[FISCAL, DESCHIS 27.07.2026] Rotunjirea din D390: bancara sau aritmetica?** `core/d390._int` foloseste `round()` = rotunjire BANCARA (112.5 -> 112). `core/d112._d112int` documenteaza ca ANAF cere rotunjire ARITMETICA si ca cea bancara a fost RESPINSA de validator (CAM calculat 112, cerut 113 - regula A91b). Nu se stie daca aceeasi regula se aplica bazelor din D390. **De verificat la sursa oficiala** (structura/instructiunile D390, OPANAF in vigoare) INAINTE de orice schimbare - e schimbare fiscala, nu tehnica. Azi neatins deliberat.
- **[DESCHIS 27.07.2026] `numere.numar()` intoarce float, folosit la importuri de SUME.** Float pe bani pierde precizie la insumare (0.1+0.2). Importurile de solduri/parteneri/salariati/mijloace fixe trec prin el. De evaluat trecerea la Decimal - schimbare cu suprafata mare (7 module), merita masurata intai: cat de mari sunt sumele importate si daca eroarea e observabila la rotunjirea finala.

- **[DE DECIS 27.07.2026] `main.py:1063` — register înghite eșecul de provisionare.** Blocul `try` din `register` acoperă `provision_tenant(...)`; dacă provisionarea crapă, înregistrarea răspunde SUCCES iar userul rămâne cu cont fără firmă. Azi a fost făcut **zgomotos** (log + alertă), dar **comportamentul nu a fost schimbat** — decizia e a lui Costin: (a) rămâne așa, contul e valid și firma se adaugă manual; (b) register eșuează explicit și userul reîncearcă; (c) contul se creează, dar UI-ul arată clar „firma nu s-a putut crea, reîncearcă". Necesită și verificarea a ce rămâne în DB după eșec parțial (schemă creată pe jumătate?).

- **[DESCHIS 27.07.2026] `SELECT *` pe date fiscale — restul locurilor.** `core/d112.pull` a primit gardă de prezență a coloanelor (`common.cere_coloane`) după ce s-a dovedit că o coloană dispărută producea o declarație cu salarii ZERO, tăcut. Aceeași construcție există în: `core/d394.py:650` (firma_profil), `core/bilant_api.py:59,90`, `core/rip_api.py:50`, `core/stocuri_cv_api.py` (articole, 6 locuri), `core/reconciliere_api.py:101,135`, `core/jurnal_api.py:9`, `core/salariati_api.py:221,224`. De evaluat fiecare: care ajung într-o valoare fiscală declarată (prioritar) și care sunt doar afișare. Gard mecanic posibil: interzice `SELECT *` fără `cere_coloane` pe tabelele care hrănesc declarații.
