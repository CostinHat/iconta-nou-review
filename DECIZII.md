...
## 18.08.2026 — Formular manual D107 in UI (scos din _DOAR_API) — declaratie 4/6

**Decizie.** D107 (informativa beneficiari sponsorizari/mecenat/burse) capata formular manual in ecranul Declaratii, ca LISTA (model d710/d307). Fiecare beneficiar: denumire/nume, cod de identificare fiscala (CUI/CNP), adresa, si TREI sume - Suma acordata (Val1), Suma reportata (Val2), Suma dedusa (Val3). Totalurile pe cele trei coloane (TVal1=Σval1, TVal2=Σval2+Val2_NI, TVal3=Σval3+Val3_NI) si suma de control (totalPlata_A=TVal1+TVal2+TVal3) se CALCULEAZA (afisate, recalculate la randare - coincid cu serverul, Regula 14.2). Beneficiarii stau IN MEMORIE (S.d107.beneficiari) -> body de generare (D107 n-are tabel DB). Optional: rectificativa (d_rec).

**Anexa neindividualizati (entit1) - decizie de forma.** Structura ANAF: entit1 (beneficiari neindividualizati, doar denumire/cod/adresa, fara sume pe entitate) exista DACA SI NUMAI DACA suma reportata a lor (Val2_NI) > 0. Caz rar. Ca sa nu incarce ecranul comun, l-am pus intr-o SUB-sectiune `<details>` colapsata: campurile Val2_NI/Val3_NI + o mini-lista care apare conditionat (doar cand Val2_NI>0). Backend garda simetrica: Val2_NI>0 fara lista -> refuz; lista fara Val2_NI -> refuz (mesaje de contabil, nu nume interne).

**Parametri (cititi din semnatura, Regula 1).** d107.genereaza(conn, schema, Perioada(an), {"beneficiari":[{den,cif,adresa,val1,val2,val3}], "val2_ni"?, "val3_ni"?, "neindividualizati":[{den,cif,adresa}]?, "d_rec"?}). Exercitiu pe an calendaristic (an modificat/dizolvare respinse). cod_oblig se alege automat din regimul firmei (micro->121, profit->103). d_PM=2 la micro, 1 la profit. cont bugetar 5503 cu X-uri literale.

**Temei.** OPANAF 355/2024; Cod fiscal (sponsorizare/mecenat/burse). Structura din D107Validator.jar (v1, an sfarsit exercitiu >=2024) + anaf_surse/structura_D107_2024_010726 (Val1/Val2/Val3 = Suma/Suma reportata/Suma dedusa, confirmat la sursa, Regula 2). Regula 14.4: erori_generare + valideaza_cerere rescrise in limba contabilului (nu mai expun denE/cifE/Val2_NI/entit1/cod_oblig/`manual.beneficiari`), test_d107 re-ancorat.

**Proba.** Backend: refuz pe gol cu mesaj de contabil; DUK valid pe 2 beneficiari (Asociatia Alfa 5000/2000/3000 + Fundatia Beta 3000/0/3000). Gard test_d107_formular RED(sed reintroduce Val2_NI in mesaj)->GREEN, + test suplimentar test_d107_erori_fara_nume_interne. Traseu live Playwright (ALFA MICRO, an 2024): D107 in selector, gol refuzat, completat -> DUK VALID (XML Val1=5000/TVal1=8000/2x<entit>, control 16.000 coincide), axe contrast+eticheta 0, mobil Pixel5 body=393 fara overflow. Captura privita.

**Lectie (a patra oara aplicata).** Flip CSV AMANAT->LIVE cere genereaza_grupe_functii --scrie + registre + CSV IN ACELASI commit cu codul; scoate DOAR tipul curent din _DOAR_API. In plus: valideaza_cerere (stratul de dispatch, INAINTEA generatorului) are propriul mesaj - trebuie rescris si el in limba contabilului, altfel formularul gol arata `manual.beneficiari` desi erori_generare e curat (prins de proba Playwright, nu de testul unit).

## 18.08.2026 — Formular manual D177 in UI (scos din _DOAR_API) — declaratie 5/6

**Decizie.** D177 (cerere de redirectionare a unei parti din impozitul pe profit catre entitati nonprofit / unitati de cult) capata formular manual in ecranul Declaratii. Forma = ANTET (plafoane + perioada, campuri fixe) + LISTA de beneficiari (1..n). Antet: suma maxima redirectionabila (contabilul o calculeaza: min(20% impozit profit, 0,75% cifra afaceri)), suma redirectionata anterior, suma ramasa (>0), perioada fiscala (dataInceput/dataSfarsit, prefill an calendaristic). Fiecare beneficiar: tip (1=cult/PJ fara scop lucrativ, 2=alti beneficiari, 3=act de mecenat/PF, 5=UNICEF si org. internationale), cod fiscal (CUI la tip 1/2/5, CNP la 3), denumire, IBAN (RO+22, cont din Romania), suma, acord de informare (0/1), contract (obligatoriu la tip<5). Suma de control = 0 (D177 informativa; formula veche round(sumaRest/100) era fals-verde pe validatorul vechi). tipB=4 (bursa privata) exista in act dar e NEPERMIS de validator -> nu se ofera in UI. Redirectionarea micro e abrogata din 2024 -> tipPlatitor fixat pe profit (1).

**Etichete la sursa (Regula 2).** tipB 1..5 + `acord` (informare entitate beneficiara: denumire+CIF contribuabil + suma) citite din anaf_surse/structura_D177_2026 + d177_structura_note, NU inventate.

**BUG REAL prins de DUK pe traseu (nu de formular).** Dispatch-ul _d177 trimitea Perioada(an, luna=b.get("luna") or 6). Validatorul (regula R4.1) cere ca atributul `luna` din XML sa coincida cu luna din dataSfarsit (12 la an calendaristic; 12 daca dataSfarsit e nul). luna=6 -> RESPINS. Reparat in d177.genereaza: luna se DERIVA din dataSfarsit (nu din perioada), default 12. Aserție de regresie in gard (perioada luna=6 -> XML luna=12). Confirma valoarea probei vizuale: unit-testul folosea luna=12 explicit si trecea; doar traseul real (dispatch cu default) a scos bug-ul la iveala.

**Temei.** CF art.25 alin.(4) lit.i/t + alin.(4^1); Legea 32/1994; OPANAF 3562/2024. Structura din D177Validator.jar (v1). Regula 14.4: erori_generare + valideaza_cerere rescrise (nu mai expun tipB/cuiB/denB/ibanB/sumaB/contractB/sumaMax/sumaRest/tipPlatitor/`manual.beneficiari`); test_d177 re-ancorat.
